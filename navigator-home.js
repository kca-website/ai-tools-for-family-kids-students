/* Homepage navigator v8.1 preview.
 * Scope: homepage only.
...
(function(){
  ...
  function ensureAccessibilityBadge(){
    ...
    // Reuse the badge id owned by home-accessibility.js so the two homepage
    // runtimes can never render the same GSL badge twice.
  ...
  function suppressLegacyInjectedBlocks(){
    // These elements are generated by older homepage/PWA helpers. They may be
    // injected after this file runs, so navigator-home.css also suppresses them
    // by id with !important. Adding the class here covers elements already present.
    [
      "pwaMobileLauncher",
  ...
  function init(){
    ...
    // Older homepage helpers add their panels on timers (0/40/180/500 ms),
    // while the PWA launcher is added after pwa-core loads. Keep the page clean
    // without removing those runtimes globally.
