/**
 * official-curriculum-data.js: Official Greek Curriculum Layer v2.1
 * School year 2026-2027; Panhellenic 2027 where explicitly noted.
 * Topic anchor != official chapter title unless status is exact/related-section-verified.
 */
(function(){
  "use strict";
  const META = {
  "version": "2.1.0",
  "schoolYear": "2026-2027",
  "lastVerified": "2026-08-29",
  "officialCatalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
  "iepAnnualGuidanceUrl": "https://www.iep.edu.gr/yli-odigies-tropos-axiologisis/",
  "gelExamSyllabusUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
  "panhellenic2027Url": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2",
  "sourcePolicyEl": "Μόνο ρητά καταχωρισμένες επίσημες πηγές επιτρέπουν ισχυρισμό επίσημης ευθυγράμμισης. Topic anchor ≠ επίσημος τίτλος ενότητας. Το layer διαχωρίζει βιβλίο, περιεχόμενα βιβλίου και ετήσια διδακτέα/εξεταστέα ύλη.",
  "sourcePolicyEn": "Only explicitly recorded official sources permit an official-alignment claim. A topic anchor is not an official section title. The layer separates textbook, textbook contents, and annual taught/examinable syllabus.",
  "coverageSummary": {
    "allQuizEntries": 54,
    "allGapEntries": 280,
    "quizCoverageByStatus": {
      "annual-syllabus-verified": 11,
      "book-index-verified": 7,
      "catalog-verified": 3,
      "official-book-verified": 33
    },
    "gapCoverageByStatus": {
      "catalog-topic-anchor": 8,
      "exact-section-verified": 31,
      "official-course-topic-anchor": 218,
      "related-section-verified": 23
    }
  },
  "limitationsEl": "Η v2 καλύπτει όλα τα 54 quiz και όλα τα 280 gapTags με source-aware status. Αυτό ΔΕΝ σημαίνει ότι 280/280 έχουν ακριβή αντιστοίχιση σε επίσημο κεφάλαιο. Τα περισσότερα είναι ασφαλή topic anchors μέχρι να γίνει section-level έλεγχος. Για ΓΕΛ έχει ενσωματωθεί η δημοσιευμένη εξεταστέα ύλη 2026–27 όπου επαληθεύτηκε. Για Δημοτικό/Γυμνάσιο οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί και τα βιβλία/ενότητες έχουν επαληθευτεί όπου υπάρχει section-level τεκμηρίωση, αλλά η ακριβής ετήσια αντιστοίχιση δεν έχει ολοκληρωθεί παντού.",
  "limitationsEn": "v2 covers all 54 quizzes and all 280 gapTags with source-aware status. This does NOT mean 280/280 are exact official chapter mappings. Most are safe topic anchors pending section-level verification. Published 2026–27 GEL examinable syllabus is encoded where verified; Official 2026–27 Primary/Middle guidance has been published, but exact annual section-level alignment is not yet complete everywhere."
};
  const BY_QUIZ = {
  "math-a-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Μαθηματικά, Α' Δημοτικού",
    "quizTitleEn": "Math, 1st Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Μαθηματικά, Α' Δημοτικού",
      "titleEn": "Math, 1st Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "math-a-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-a-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Α' Δημοτικού",
    "quizTitleEn": "Modern Greek Language, 1st Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Νεοελληνική Γλώσσα, Α' Δημοτικού",
      "titleEn": "Modern Greek Language, 1st Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "glossa-a-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "math-b-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Μαθηματικά, Β' Δημοτικού",
    "quizTitleEn": "Math, 2nd Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Μαθηματικά, Β' Δημοτικού",
      "titleEn": "Math, 2nd Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "math-b-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-b-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Β' Δημοτικού",
    "quizTitleEn": "Modern Greek Language, 2nd Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Νεοελληνική Γλώσσα, Β' Δημοτικού",
      "titleEn": "Modern Greek Language, 2nd Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "glossa-b-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "math-c-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Μαθηματικά, Γ' Δημοτικού",
    "quizTitleEn": "Math, 3rd Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Μαθηματικά, Γ' Δημοτικού",
      "titleEn": "Math, 3rd Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "math-c-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-c-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Γ' Δημοτικού",
    "quizTitleEn": "Modern Greek Language, 3rd Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Νεοελληνική Γλώσσα, Γ' Δημοτικού",
      "titleEn": "Modern Greek Language, 3rd Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "glossa-c-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-c-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Αγγλικά, Γ' Δημοτικού",
    "quizTitleEn": "English, 3rd Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Αγγλικά, Γ' Δημοτικού",
      "titleEn": "English, 3rd Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C107"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "english-c-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-c-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Ιστορία, Γ' Δημοτικού (Ελληνική Μυθολογία)",
    "quizTitleEn": "History, 3rd Grade (Greek Mythology)",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Ιστορία, Γ' Δημοτικού (Ελληνική Μυθολογία)",
      "titleEn": "History, 3rd Grade (Greek Mythology)",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C103"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "istoria-c-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "math-d-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Μαθηματικά, Δ' Δημοτικού",
    "quizTitleEn": "Math, 4th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Μαθηματικά, Δ' Δημοτικού",
      "titleEn": "Math, 4th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "math-d-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-d-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Δ' Δημοτικού",
    "quizTitleEn": "Modern Greek Language, 4th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Νεοελληνική Γλώσσα, Δ' Δημοτικού",
      "titleEn": "Modern Greek Language, 4th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "glossa-d-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-d-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Αγγλικά, Δ' Δημοτικού",
    "quizTitleEn": "English, 4th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Αγγλικά, Δ' Δημοτικού",
      "titleEn": "English, 4th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D101"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "english-d-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-d-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Ιστορία, Δ' Δημοτικού (Αρχαία Ελληνική Ιστορία)",
    "quizTitleEn": "History, 4th Grade (Ancient Greek History)",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Ιστορία, Δ' Δημοτικού (Αρχαία Ελληνική Ιστορία)",
      "titleEn": "History, 4th Grade (Ancient Greek History)",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D103"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "istoria-d-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "math-e-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Μαθηματικά, Ε' Δημοτικού",
    "quizTitleEn": "Math, 5th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Μαθηματικά, Ε' Δημοτικού",
      "titleEn": "Math, 5th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "math-e-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-e-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Ιστορία, Ε' Δημοτικού",
    "quizTitleEn": "History, 5th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Ιστορία, Ε' Δημοτικού",
      "titleEn": "History, 5th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E105"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "istoria-e-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-e-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Ε' Δημοτικού",
    "quizTitleEn": "Modern Greek Language, 5th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Νεοελληνική Γλώσσα, Ε' Δημοτικού",
      "titleEn": "Modern Greek Language, 5th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "glossa-e-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "science-e-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Φυσικές Επιστήμες, Ε' Δημοτικού",
    "quizTitleEn": "Natural Science, 5th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Φυσικές Επιστήμες, Ε' Δημοτικού",
      "titleEn": "Natural Science, 5th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E107"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "science-e-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-e-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Αγγλικά, Ε' Δημοτικού",
    "quizTitleEn": "English, 5th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Αγγλικά, Ε' Δημοτικού",
      "titleEn": "English, 5th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E103"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "english-e-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-st-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Αγγλικά, ΣΤ' Δημοτικού",
    "quizTitleEn": "English, 6th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Αγγλικά, ΣΤ' Δημοτικού",
      "titleEn": "English, 6th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F101"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "english-st-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "math-st-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Μαθηματικά, ΣΤ' Δημοτικού",
    "quizTitleEn": "Math, 6th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Μαθηματικά, ΣΤ' Δημοτικού",
      "titleEn": "Math, 6th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "math-st-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-st-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, ΣΤ' Δημοτικού",
    "quizTitleEn": "Modern Greek Language, 6th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Νεοελληνική Γλώσσα, ΣΤ' Δημοτικού",
      "titleEn": "Modern Greek Language, 6th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "glossa-st-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "science-st-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Φυσικές Επιστήμες, ΣΤ' Δημοτικού",
    "quizTitleEn": "Science, 6th Grade",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Φυσικές Επιστήμες, ΣΤ' Δημοτικού",
      "titleEn": "Science, 6th Grade",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL101"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "science-st-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-st-dimotikou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Ιστορία, ΣΤ' Δημοτικού (Νεότερη & Σύγχρονη Ελληνική Ιστορία)",
    "quizTitleEn": "History, 6th Grade (Modern & Contemporary Greek History)",
    "officialBook": {
      "titleEl": "Επίσημο διδακτικό πακέτο: Ιστορία, ΣΤ' Δημοτικού (Νεότερη & Σύγχρονη Ελληνική Ιστορία)",
      "titleEn": "History, 6th Grade (Modern & Contemporary Greek History)",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F114"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/protovathmia/dimotiko",
    "scopeNoteEl": "Το επίσημο διδακτικό πακέτο της τάξης/του μαθήματος είναι καταχωρισμένο για το 2026–27. Τα topic anchors του layer προέρχονται από τα υπάρχοντα μαθησιακά κενά του site και δεν δηλώνονται ως επίσημοι τίτλοι κεφαλαίων, εκτός αν υπάρχει ξεχωριστή section-level επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "istoria-st-dimotikou",
    "zone": "primary",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Γ' Γυμνασίου",
    "quizTitleEn": "Modern Greek Language, 3rd Grade Middle School",
    "officialBook": {
      "titleEl": "Νεοελληνική Γλώσσα Γ΄ Γυμνασίου",
      "titleEn": "Νεοελληνική Γλώσσα Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C107/675/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "glossa-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "physics-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Φυσική, Β' Γυμνασίου",
    "quizTitleEn": "Physics, 2nd Grade Middle School",
    "officialBook": {
      "titleEl": "Φυσική Β΄ Γυμνασίου",
      "titleEn": "Φυσική Β΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B200/455/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "physics-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "mathimatika-g-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Μαθηματικά, Γ' Γυμνασίου",
    "quizTitleEn": "Mathematics, 3rd Grade Middle School",
    "officialBook": {
      "titleEl": "Μαθηματικά Γ΄ Γυμνασίου",
      "titleEn": "Μαθηματικά Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C104/68/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "mathimatika-g-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-g-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Ιστορία, Γ' Γυμνασίου",
    "quizTitleEn": "History, 3rd Grade Middle School",
    "officialBook": {
      "titleEl": "Νεότερη και Σύγχρονη Ιστορία Γ΄ Γυμνασίου",
      "titleEn": "Νεότερη και Σύγχρονη Ιστορία Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C105/65/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "istoria-g-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-g-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Αγγλικά, Γ' Γυμνασίου",
    "quizTitleEn": "English, 3rd Grade Middle School",
    "officialBook": {
      "titleEl": "Think Teen: 3rd Grade / Αγγλικά Γ΄ Γυμνασίου",
      "titleEn": "Think Teen: 3rd Grade / Αγγλικά Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-C109"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "quizId": "english-g-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "mathimatika-a-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Μαθηματικά, Α' Γυμνασίου",
    "quizTitleEn": "Math, 7th Grade",
    "officialBook": {
      "titleEl": "Μαθηματικά Α΄ Γυμνασίου",
      "titleEn": "Μαθηματικά Α΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A200/293/"
    },
    "officialSectionsEl": [
      "Κεφ. 1: Φυσικοί αριθμοί: διάταξη, πράξεις, δυνάμεις, διαιρετότητα, ΜΚΔ/ΕΚΠ",
      "Κεφ. 2: Κλάσματα: έννοια, ισοδύναμα, σύγκριση, πράξεις",
      "Κεφ. 3: Δεκαδικοί αριθμοί: διάταξη, πράξεις, στρογγυλοποίηση, μονάδες μέτρησης",
      "Κεφ. 4: Εξισώσεις και προβλήματα",
      "Κεφ. 5: Ποσοστά",
      "Κεφ. 6: Λόγος, αναλογία, ανάλογα και αντιστρόφως ανάλογα ποσά",
      "Κεφ. 7: Θετικοί/αρνητικοί (ρητοί): ευθεία ρητών, απόλυτη τιμή, αντίθετοι, σύγκριση και πράξεις",
      "Γεωμετρία: βασικές έννοιες, γωνίες, κύκλος, συμμετρία, παράλληλες, τρίγωνα/τετράπλευρα (σύμφωνα με το βιβλίο)"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Ο αναλυτικός δείκτης βασίζεται στα επίσημα περιεχόμενα του βιβλίου Μαθηματικών Α΄ Γυμνασίου. Οι ετήσιες οδηγίες 2026–27 παραμένουν ξεχωριστή πηγή και δεν θεωρούνται ακόμη πλήρως ενσωματωμένες.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "mathimatika-a-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-a-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Α' Γυμνασίου",
    "quizTitleEn": "Modern Greek Language, 7th Grade",
    "officialBook": {
      "titleEl": "Νεοελληνική Γλώσσα Α΄ Γυμνασίου",
      "titleEn": "Νεοελληνική Γλώσσα Α΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A112/337/"
    },
    "officialSectionsEl": [
      "Εν. 1: Επικοινωνία, κώδικες επικοινωνίας, είδη προτάσεων, λεξιλόγιο, παραγωγή λόγου",
      "Εν. 2: Προφορικός/γραπτός λόγος, γλωσσική ποικιλία, παράγραφος",
      "Εν. 3: Περιγραφή, αφήγηση, επιχειρηματολογία, πολυτροπικότητα",
      "Εν. 4: Ονοματική φράση, επιθετικός προσδιορισμός, κλίση ουσιαστικών/επιθέτων, παραγωγή/σύνθεση",
      "Εν. 5: Ρήμα, παράγωγα ρήματα, ρήμα στην αφήγηση",
      "Εν. 6: Λειτουργίες/χρήσεις πτώσεων, παράγωγα ουσιαστικά, παράγραφος/πλαγιότιτλοι",
      "Εν. 7: Άρθρο, παράγωγα επίθετα, περιγραφή",
      "Εν. 8: Παρατακτική σύνδεση, ασύνδετο σχήμα, αφήγηση",
      "Εν. 9: Συνταγματικός και παραδειγματικός άξονας",
      "Εν. 10: Υποτακτική σύνδεση, σημεία στίξης, χρήση λεξικών"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Ο δείκτης βασίζεται στα επίσημα περιεχόμενα της Νεοελληνικής Γλώσσας Α΄ Γυμνασίου. Δεν πρέπει να θεωρείται ότι κάθε γενική γλωσσική έννοια που γνωρίζει το μοντέλο έχει διδαχθεί ήδη στην Α΄ Γυμνασίου.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "glossa-a-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-a-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Ιστορία, Α' Γυμνασίου (Προϊστορία - Κλασικά Χρόνια)",
    "quizTitleEn": "History, 7th Grade (Prehistory - Classical Period)",
    "officialBook": {
      "titleEl": "Αρχαία Ιστορία Α΄ Γυμνασίου",
      "titleEn": "Αρχαία Ιστορία Α΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A105/29/"
    },
    "officialSectionsEl": [
      "Η Προϊστορία: Εποχή του Λίθου",
      "Εποχή του Χαλκού: Εγγύς Ανατολή, Κυκλαδικός, Μινωικός και Μυκηναϊκός πολιτισμός",
      "Ελληνικός κόσμος 1100–800 π.Χ.",
      "Αρχαϊκή εποχή: αποικισμός, πόλη-κράτος, Σπάρτη, Αθήνα και πορεία προς τη δημοκρατία, Περσικοί πόλεμοι",
      "Ηγεμονία της Αθήνας",
      "Πελοποννησιακός πόλεμος και ηγεμονικοί ανταγωνισμοί",
      "Ανάπτυξη της Μακεδονίας: Φίλιππος και Αλέξανδρος",
      "Τέχνες και γράμματα της κλασικής εποχής",
      "Ελληνιστικοί και ρωμαϊκοί χρόνοι",
      "Από τη Ρώμη στο Βυζάντιο"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Ο αναλυτικός δείκτης αντιστοιχεί στα επίσημα περιεχόμενα της Αρχαίας Ιστορίας Α΄ Γυμνασίου. Ο Tutor πρέπει να μένει στην ορολογία και στο επίπεδο του σχολικού βιβλίου όταν το ερώτημα αφορά αυτές τις ενότητες.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "istoria-a-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-a-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "catalog-verified",
    "coverageLabelEl": "Επιβεβαιωμένο στον επίσημο κατάλογο βιβλίων 2026–27",
    "coverageLabelEn": "Verified in official 2026–27 textbook catalog",
    "quizTitleEl": "Αγγλικά, Α' Γυμνασίου (A2)",
    "quizTitleEn": "English, 7th Grade (A2)",
    "officialBook": null,
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Το μάθημα και οι επίσημες επιλογές διδακτικών πακέτων είναι επιβεβαιωμένα, αλλά το επίπεδο που δηλώνει το quiz (A2/B1) δεν ταυτοποιεί με ασφάλεια ένα και μόνο επίσημο βιβλίο. Ο Tutor δεν πρέπει να ισχυρίζεται συγκεκριμένο unit χωρίς επιπλέον επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "officialCourseOptions": [
      {
        "titleEl": "Αγγλικά Α΄ Γυμνασίου: Beginners",
        "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-A111"
      },
      {
        "titleEl": "Αγγλικά Α΄ Γυμνασίου: Advanced",
        "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-A114"
      }
    ],
    "quizId": "english-a-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "mathimatika-b-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Μαθηματικά, Β' Γυμνασίου",
    "quizTitleEn": "Math, 8th Grade",
    "officialBook": {
      "titleEl": "Μαθηματικά Β΄ Γυμνασίου",
      "titleEn": "Μαθηματικά Β΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B105/372/"
    },
    "officialSectionsEl": [
      "Άλγεβρα: εξισώσεις/ανισώσεις α΄ βαθμού, πραγματικοί αριθμοί, συναρτήσεις, περιγραφική στατιστική",
      "Γεωμετρία: εμβαδά και Πυθαγόρειο θεώρημα, τριγωνομετρία/διανύσματα, μέτρηση κύκλου, γεωμετρικά στερεά"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Το μάθημα/η τάξη έχει επιβεβαιωθεί στον επίσημο κατάλογο σχολικών βιβλίων 2026–27. Δεν έχει ακόμη καταχωριστεί αναλυτικός δείκτης κεφαλαίων για αυτό το quiz στο δικό μας curriculum layer.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "mathimatika-b-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "glossa-b-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα, Β' Γυμνασίου",
    "quizTitleEn": "Modern Greek Language, 8th Grade",
    "officialBook": {
      "titleEl": "Νεοελληνική Γλώσσα Β΄ Γυμνασίου",
      "titleEn": "Νεοελληνική Γλώσσα Β΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B110/676/"
    },
    "officialSectionsEl": [
      "Υποκείμενο, παραγωγή με αχώριστα μόρια, τρόποι ανάπτυξης παραγράφου",
      "Εγκλίσεις/χρόνοι, σύνθετες λέξεις, περίληψη και πλαγιότιτλοι",
      "Ενεργητική/παθητική φωνή, διαθέσεις ρήματος, συνοχή κειμένου",
      "Βαθμοί επιθέτων, οργάνωση περιγραφής, αντωνυμίες, επιρρηματικοί προσδιορισμοί",
      "Μετοχές, επιχειρηματολογία, ορισμός, χρήση λεξικών και παραγωγή λόγου"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Το μάθημα/η τάξη έχει επιβεβαιωθεί στον επίσημο κατάλογο σχολικών βιβλίων 2026–27. Δεν έχει ακόμη καταχωριστεί αναλυτικός δείκτης κεφαλαίων για αυτό το quiz στο δικό μας curriculum layer.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "glossa-b-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-b-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Ιστορία, Β' Γυμνασίου (Ελληνιστική - Μεσαιωνική Περίοδος)",
    "quizTitleEn": "History, 8th Grade (Hellenistic - Medieval Period)",
    "officialBook": {
      "titleEl": "Μεσαιωνική και Νεότερη Ιστορία Β΄ Γυμνασίου",
      "titleEn": "Μεσαιωνική και Νεότερη Ιστορία Β΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B107/371/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "istoria-b-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-b-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "catalog-verified",
    "coverageLabelEl": "Επιβεβαιωμένο στον επίσημο κατάλογο βιβλίων 2026–27",
    "coverageLabelEn": "Verified in official 2026–27 textbook catalog",
    "quizTitleEl": "Αγγλικά, Β' Γυμνασίου (B1)",
    "quizTitleEn": "English, 8th Grade (B1)",
    "officialBook": null,
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Το μάθημα και οι επίσημες επιλογές διδακτικών πακέτων είναι επιβεβαιωμένα, αλλά το επίπεδο που δηλώνει το quiz (A2/B1) δεν ταυτοποιεί με ασφάλεια ένα και μόνο επίσημο βιβλίο. Ο Tutor δεν πρέπει να ισχυρίζεται συγκεκριμένο unit χωρίς επιπλέον επαλήθευση.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "official-annual-guidance-published",
    "annualInstructionsNoteEl": "Οι επίσημες οδηγίες 2026–27 έχουν δημοσιευτεί για το μάθημα. Η ακριβής section-level αντιστοίχιση δεν έχει ολοκληρωθεί παντού· το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ή εξεταστέα ύλη.",
    "annualInstructionsNoteEn": "Official 2026–27 guidance has been published for this subject. Exact section-level alignment is not complete everywhere; an official textbook does not automatically equal the full taught or examinable scope for this year.",
    "officialCourseOptions": [
      {
        "titleEl": "Αγγλικά Β΄ Γυμνασίου: επίσημο πακέτο (μία από τις διαθέσιμες σειρές)",
        "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-B114"
      }
    ],
    "quizId": "english-b-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "fysiki-g-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Φυσική, Γ' Γυμνασίου",
    "quizTitleEn": "Physics, 9th Grade",
    "officialBook": {
      "titleEl": "Φυσική Γ΄ Γυμνασίου",
      "titleEn": "Φυσική Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C201/296/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "fysiki-g-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "chimeia-g-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Χημεία, Γ' Γυμνασίου",
    "quizTitleEn": "Chemistry, 9th Grade",
    "officialBook": {
      "titleEl": "Χημεία Γ΄ Γυμνασίου",
      "titleEn": "Χημεία Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C102/223/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "chimeia-g-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "biologia-a-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Βιολογία, Α' Γυμνασίου",
    "quizTitleEn": "Biology, 7th Grade",
    "officialBook": {
      "titleEl": "Βιολογία Α΄ Γυμνασίου",
      "titleEn": "Βιολογία Α΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A103/477/"
    },
    "officialSectionsEl": [
      "Επιστήμη της Βιολογίας και εισαγωγή στην επιστημονική μέθοδο",
      "Κεφ. 1: Οργάνωση της ζωής: χαρακτηριστικά οργανισμών, κύτταρο, πολυκύτταρη οργάνωση, ταξινόμηση, προσαρμογές",
      "Κεφ. 2: Πρόσληψη ουσιών και πέψη: φωτοσύνθεση, μονοκύτταροι, ζωικοί οργανισμοί, άνθρωπος",
      "Κεφ. 3: Μεταφορά και αποβολή ουσιών",
      "Κεφ. 4: Αναπνοή",
      "Κεφ. 5: Στήριξη και κίνηση",
      "Κεφ. 6: Αναπαραγωγή",
      "Κεφ. 7: Ερεθιστικότητα: νευρικό σύστημα, αισθητήρια όργανα, ενδοκρινικό σύστημα"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Ο δείκτης βασίζεται στα επίσημα περιεχόμενα της Βιολογίας Α΄ Γυμνασίου. Ο Tutor πρέπει να διακρίνει τι αναφέρει το βασικό κείμενο από συμπληρωματικό υλικό και να μην επεκτείνει το επίπεδο πέρα από την τάξη χωρίς να το δηλώνει.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "biologia-a-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "biologia-b-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official textbook verified",
    "quizTitleEl": "Βιολογία, Β' Γυμνασίου",
    "quizTitleEn": "Biology, 8th Grade",
    "officialBook": {
      "titleEl": "Βιολογία Β΄ και Γ΄ Γυμνασίου",
      "titleEn": "Βιολογία Β΄ και Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Έχει επιβεβαιωθεί το επίσημο σχολικό βιβλίο. Δεν έχει ακόμη ολοκληρωθεί η αντιστοίχιση όλων των quiz topics με συγκεκριμένες ενότητες του βιβλίου.",
    "scopeNoteEn": "The official textbook is verified. Full mapping of all quiz topics to exact textbook sections is not complete yet.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "biologia-b-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "biologia-g-gymnasiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "book-index-verified",
    "coverageLabelEl": "Επίσημο σχολικό βιβλίο και περιεχόμενα επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and contents verified",
    "quizTitleEl": "Βιολογία, Γ' Γυμνασίου",
    "quizTitleEn": "Biology, 9th Grade",
    "officialBook": {
      "titleEl": "Βιολογία Β΄ και Γ΄ Γυμνασίου",
      "titleEn": "Βιολογία Β΄ και Γ΄ Γυμνασίου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
    },
    "officialSectionsEl": [
      "Κεφ. 1: Οργάνωση της ζωής: μόρια, κύτταρο, επίπεδα οργάνωσης",
      "Κεφ. 2: Οργανισμοί στο περιβάλλον: οικοσύστημα, ενέργεια, ανακύκλωση ύλης, ανθρώπινες παρεμβάσεις",
      "Κεφ. 3: Μεταβολισμός και ένζυμα",
      "Κεφ. 4: Ομοιόσταση, ασθένειες, άμυνα οργανισμού, τρόπος ζωής",
      "Κεφ. 5: Γενετικό υλικό, ροή πληροφορίας, αλληλόμορφα, κυτταρική διαίρεση, κληρονομικότητα, μεταλλάξεις",
      "Κεφ. 6: Γενετική μηχανική, βιοτεχνολογία, βιοηθική",
      "Κεφ. 7: Εξέλιξη και εξέλιξη του ανθρώπου"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "ministryUrl": "https://www.minedu.gov.gr/defterovathmia/gymnasio",
    "scopeNoteEl": "Το επίσημο βιβλίο δηλώνει ότι τα παραθέματα που συνοδεύουν το βασικό κείμενο δεν αποτελούν εξεταστέα ύλη. Ο Tutor δεν πρέπει να τα παρουσιάζει ως απαιτούμενη εξεταστέα γνώση.",
    "scopeNoteEn": "The grade/subject is verified in the official 2026–27 textbook catalog. A chapter-level index has not yet been added for this quiz in our curriculum layer.",
    "annualInstructionsStatus": "2026-27-not-yet-indexed",
    "annualInstructionsNoteEl": "Μην θεωρείς ότι όλο το βιβλίο είναι αυτομάτως διδακτέα ή εξεταστέα ύλη. Οι αναλυτικές ετήσιες οδηγίες 2026–27 για το συγκεκριμένο μάθημα δεν έχουν ακόμη επιβεβαιωθεί στο layer.",
    "annualInstructionsNoteEn": "Do not assume the whole textbook is automatically taught or examinable. The detailed 2026–27 annual teaching instructions for this subject have not yet been verified in this layer.",
    "quizId": "biologia-g-gymnasiou",
    "zone": "middle",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "ekthesi-a-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα & Έκθεση, Α' Λυκείου",
    "quizTitleEn": "Modern Greek Language & Essay, 10th Grade",
    "officialBook": {
      "titleEl": "Έκφραση - Έκθεση Α΄ Λυκείου / Θεματικοί Κύκλοι / Γλωσσικές Ασκήσεις",
      "titleEn": "Έκφραση - Έκθεση Α΄ Λυκείου / Θεματικοί Κύκλοι / Γλωσσικές Ασκήσεις",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A110"
    },
    "officialSectionsEl": [
      "Επίσημο πλαίσιο εξέτασης: κατανόηση και κριτική προσέγγιση κειμένων",
      "Οργάνωση/δομή/γλώσσα κειμένου, μετασχηματισμός νοημάτων, περίληψη και διατύπωση τεκμηριωμένης άποψης"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "ekthesi-a-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "ekthesi-b-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα & Έκθεση, Β' Λυκείου",
    "quizTitleEn": "Modern Greek Language & Essay, 11th Grade",
    "officialBook": {
      "titleEl": "Έκφραση - Έκθεση Β΄ Λυκείου",
      "titleEn": "Έκφραση - Έκθεση Β΄ Λυκείου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSB105/435/"
    },
    "officialSectionsEl": [
      "Επίσημο πλαίσιο εξέτασης: κριτική κατανόηση, οργάνωση και τεκμηρίωση",
      "Βιβλίο: ανάπτυξη παραγράφου, αντίθεση/συνοχή, ορισμός/διαίρεση, σημειώσεις/περίληψη"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "ekthesi-b-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "ekthesi-g-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Νεοελληνική Γλώσσα & Έκθεση, Γ' Λυκείου",
    "quizTitleEn": "Modern Greek Language & Essay, 12th Grade",
    "officialBook": {
      "titleEl": "Έκφραση - Έκθεση Γ΄ Λυκείου",
      "titleEn": "Έκφραση - Έκθεση Γ΄ Λυκείου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C130/652/"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2027-panhellenic-syllabus-published-separate-decision",
    "annualInstructionsNoteEl": "Για τη Γ΄ Λυκείου η πανελλαδικά εξεταζόμενη ύλη/πλαίσιο 2027 καθορίζεται με ξεχωριστή επίσημη απόφαση. Το layer δεν μετατρέπει γενικές δεξιότητες του quiz σε υποτιθέμενους επίσημους τίτλους κεφαλαίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2",
    "quizId": "ekthesi-g-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "mathimatika-a-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Μαθηματικά, Α' Λυκείου (Γενικής Παιδείας)",
    "quizTitleEn": "Mathematics, 1st Grade Lyceum (General Education)",
    "officialBook": {
      "titleEl": "Άλγεβρα και Στοιχεία Πιθανοτήτων Α΄ Γενικού Λυκείου",
      "titleEn": "Άλγεβρα και Στοιχεία Πιθανοτήτων Α΄ Γενικού Λυκείου",
      "url": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-A100/490/3186,12916/"
    },
    "officialSectionsEl": [
      "2.1 Πράξεις και ιδιότητες πραγματικών αριθμών",
      "2.2 Διάταξη πραγματικών αριθμών",
      "2.3 Απόλυτη τιμή",
      "2.4 Ρίζες",
      "3.1–3.3 Εξισώσεις",
      "4.1–4.2 Ανισώσεις",
      "5.1–5.3 Πρόοδοι",
      "6.1 Έννοια συνάρτησης",
      "6.2 Γραφική παράσταση",
      "6.3 f(x)=αx+β"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "mathimatika-a-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "fysiki-a-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Φυσική, Α' Λυκείου (Γενικής Παιδείας)",
    "quizTitleEn": "Physics, 10th Grade Lyceum (General Education)",
    "officialBook": {
      "titleEl": "Φυσική Γενικής Παιδείας Α΄ Γενικού Λυκείου",
      "titleEn": "Φυσική Γενικής Παιδείας Α΄ Γενικού Λυκείου",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A103"
    },
    "officialSectionsEl": [
      "1.1 Ευθύγραμμη κίνηση",
      "1.2 Δυναμική σε μία διάσταση: 1ος/2ος νόμος Νεύτωνα",
      "1.3 Δυναμική στο επίπεδο: 3ος νόμος Νεύτωνα",
      "2.1 Διατήρηση μηχανικής ενέργειας"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "fysiki-a-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "fysiki-b-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημη ετήσια ύλη/εξεταστέα ύλη επαληθευμένη",
    "coverageLabelEn": "Official annual/examinable syllabus verified",
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment",
    "quizTitleEl": "Φυσική, Β' Λυκείου (Προσανατολισμού)",
    "quizTitleEn": "Physics, 11th Grade (Science Track)",
    "officialBook": {
      "titleEl": "Φυσική Ομάδας Προσανατολισμού Θετικών Σπουδών Β΄ Γενικού Λυκείου",
      "titleEn": "Physics, Grade 11 Science Track: official textbook",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-B134"
    },
    "officialSectionsEl": [
      "Κεφάλαιο 1: Καμπυλόγραμμες κινήσεις: οριζόντια βολή, ομαλή κυκλική κίνηση, κεντρομόλος δύναμη",
      "Κεφάλαιο 2: Ορμή και κρούσεις",
      "Κεφάλαιο 5: Ηλεκτρικό πεδίο (επιλεγμένες ενότητες)",
      "Κεφάλαιο 3: Κινητική θεωρία αερίων",
      "Κεφάλαιο 4: Θερμοδυναμική (επιλεγμένες ενότητες)"
    ],
    "officialSectionsEn": [],
    "scopeNoteEl": "Το quiz διορθώθηκε ώστε να ελέγχει μόνο θέματα που ανήκουν στον επαληθευμένο επίσημο πυρήνα της Φυσικής Προσανατολισμού Β΄ ΓΕΛ 2026–27.",
    "scopeNoteEn": "The quiz was corrected to cover only topics in the verified official 2026–27 Grade 11 science-track Physics scope.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Επίσημη απόφαση Υπουργείου για 2026–27.",
    "annualInstructionsNoteEn": "Official Ministry 2026–27 decision.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "fysiki-b-lykeiou"
  },
  "istoria-a-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Ιστορία, Α' Λυκείου (Αρχαία Ιστορία)",
    "quizTitleEn": "History, 10th Grade (Ancient History)",
    "officialBook": {
      "titleEl": "Ιστορία του Αρχαίου Κόσμου: Α΄ Γενικού Λυκείου",
      "titleEn": "Ιστορία του Αρχαίου Κόσμου: Α΄ Γενικού Λυκείου",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A102"
    },
    "officialSectionsEl": [
      "Ι. Πολιτισμοί Εγγύς Ανατολής (επιλεγμένες ενότητες)",
      "ΙΙ. Αρχαίοι Έλληνες: 1.2 Μυκηναϊκός, 2.1 Ομηρική, 2.2 Αρχαϊκή, 2.3 Κλασική εποχή",
      "ΙΙΙ. Ελληνιστικοί χρόνοι (επιλεγμένες ενότητες)",
      "IV:VII επιλεγμένες ενότητες Ρώμης/Ύστερης Αρχαιότητας"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "istoria-a-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-b-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Ιστορία, Β' Λυκείου (Βυζάντιο & Απαρχές Νεότερης Ιστορίας)",
    "quizTitleEn": "History, 11th Grade (Byzantium & Early Modern History)",
    "officialBook": {
      "titleEl": "Ιστορία του Μεσαιωνικού και του Νεότερου Κόσμου 565–1815: Β΄ ΓΕΛ",
      "titleEn": "Ιστορία του Μεσαιωνικού και του Νεότερου Κόσμου 565–1815: Β΄ ΓΕΛ",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-B131"
    },
    "officialSectionsEl": [
      "Κεφ. 1–4: Βυζάντιο/Δυτική Ευρώπη, Σταυροφορίες, Άλωση 1453 (επιλεγμένες ενότητες)",
      "Κεφ. 6: Αναγέννηση και Ανθρωπισμός, Ανακαλύψεις, Μεταρρύθμιση (επιλεγμένες ενότητες)",
      "Κεφ. 7: Διαφωτισμός, Αμερικανική/Γαλλική Επανάσταση (επιλεγμένες ενότητες)"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "istoria-b-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "istoria-g-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Ιστορία, Γ' Λυκείου (Νεότερη & Σύγχρονη Ιστορία)",
    "quizTitleEn": "History, 12th Grade (Modern & Contemporary History)",
    "officialBook": {
      "titleEl": "Ιστορία του Νεότερου και του Σύγχρονου Κόσμου (1815 έως σήμερα): Γ΄ ΓΕΛ",
      "titleEn": "Ιστορία του Νεότερου και του Σύγχρονου Κόσμου (1815 έως σήμερα): Γ΄ ΓΕΛ",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL106"
    },
    "officialSectionsEl": [
      "2026–27 γραπτή ύλη γενικής Ιστορίας Γ΄: επιλεγμένες ενότητες 19ου:20ού αιώνα",
      "Κεφ. Γ΄: Α΄ Παγκόσμιος Πόλεμος και άμεσες επιπτώσεις: περιλαμβάνεται ο Μικρασιατικός Πόλεμος 1919–1922"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Το site quiz περιέχει και θέματα που μπορεί να συνδέονται με διαφορετική επίσημη διαδρομή/βιβλίο Ιστορίας Γ΄ (γενική έναντι Ομάδας Προσανατολισμού). Μόνο όσα έχουν ειδική gap-to-section επαλήθευση πρέπει να παρουσιάζονται ως ακριβώς ευθυγραμμισμένα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "istoria-g-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-b-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημο βιβλίο και εξεταστέα ύλη 2026–27 επιβεβαιωμένα",
    "coverageLabelEn": "Official textbook and 2026–27 examinable syllabus verified",
    "quizTitleEl": "Αγγλικά, Β' Λυκείου (B2)",
    "quizTitleEn": "English, 11th Grade (B2)",
    "officialBook": {
      "titleEl": "Αγγλικά Γενικού Λυκείου 2",
      "titleEn": "Αγγλικά Γενικού Λυκείου 2",
      "url": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
    },
    "officialSectionsEl": [
      "Unit 2: Do we all live in the same world?",
      "Unit 3: Renaissance arts and artists",
      "Unit 4: Learning to fly",
      "Unit 5: Addictions"
    ],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Η επίσημη απόφαση του Υπουργείου για την εξεταστέα ύλη των γραπτώς εξεταζόμενων μαθημάτων ΓΕΛ 2026–27 έχει δημοσιευτεί και χρησιμοποιείται ξεχωριστά από τον κατάλογο βιβλίων.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "english-b-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "english-g-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "catalog-verified",
    "coverageLabelEl": "Επιβεβαιωμένο στον επίσημο κατάλογο βιβλίων 2026–27",
    "coverageLabelEn": "Verified in official 2026–27 textbook catalog",
    "quizTitleEl": "Αγγλικά, Γ' Λυκείου (C1)",
    "quizTitleEn": "English, 12th Grade (C1)",
    "officialBook": {
      "titleEl": "Αγγλικά Γ΄ Γενικού Λυκείου: επίσημο διδακτικό πακέτο",
      "titleEn": "English, Grade C Lyceum: official course package",
      "url": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η ύπαρξη επίσημου πακέτου Αγγλικών Γ΄ ΓΕΛ επιβεβαιώνεται στον κατάλογο 2026–27. Η απόφαση 29/07/2026 που ενσωματώθηκε στο layer δίνει συγκεκριμένη γραπτή εξεταστέα ύλη για Αγγλικά Α΄ και Β΄, όχι για Γ΄. Τα C1 language gaps παραμένουν topic anchors, όχι επίσημα units.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-annual-instructions-not-indexed",
    "annualInstructionsNoteEl": "Η ετήσια διδακτέα/εξεταστέα ύλη 2026–27 δεν έχει καταχωριστεί αναλυτικά για αυτό το μάθημα στο layer. Το επίσημο βιβλίο δεν ισοδυναμεί αυτομάτως με όλη τη φετινή διδακτέα ύλη.",
    "annualInstructionsNoteEn": "Detailed 2026–27 annual taught/examinable scope has not been encoded for this subject. An official textbook does not automatically mean its entire contents are taught this year.",
    "quizId": "english-g-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "biologia-a-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "official-book-verified",
    "coverageLabelEl": "Επίσημο διδακτικό πακέτο/βιβλίο επιβεβαιωμένο",
    "coverageLabelEn": "Official course package/textbook verified",
    "quizTitleEl": "Βιολογία, Α' Λυκείου (Γενικής Παιδείας)",
    "quizTitleEn": "Biology, 10th Grade (General Education)",
    "officialBook": {
      "titleEl": "Βιολογία Α΄ Γενικού Λυκείου",
      "titleEn": "Βιολογία Α΄ Γενικού Λυκείου",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A105"
    },
    "officialSectionsEl": [],
    "officialSectionsEn": [],
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "scopeNoteEl": "Η τάξη/το μάθημα έχει επιβεβαιωθεί σε επίσημη πηγή. Όπου δεν υπάρχει section-level αντιστοίχιση, ο Tutor δεν πρέπει να επινοεί κεφάλαιο ή να παρουσιάζει το topic anchor ως επίσημη ενότητα.",
    "scopeNoteEn": "The grade/subject is verified in an official source. Where no section-level mapping exists, the Tutor must not invent a chapter or present a topic anchor as an official section.",
    "annualInstructionsStatus": "2026-27-course-book-verified-annual-scope-not-indexed",
    "annualInstructionsNoteEl": "Το επίσημο βιβλίο/πακέτο της Α΄ Λυκείου είναι επαληθευμένο, αλλά δεν έχει γίνει στο v2 πλήρης chapter-level κωδικοποίηση της ετήσιας εξεταστέας ύλης για το συγκεκριμένο quiz.",
    "annualInstructionsNoteEn": "The official Ministry decision for the 2026–27 examinable syllabus is recorded separately from the textbook catalog.",
    "annualInstructionsUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "quizId": "biologia-a-lykeiou",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment"
  },
  "biologia-b-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημη ετήσια ύλη/εξεταστέα ύλη επαληθευμένη",
    "coverageLabelEn": "Official annual/examinable syllabus verified",
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment",
    "quizTitleEl": "Βιολογία, Β' Λυκείου (Γενικής Παιδείας)",
    "quizTitleEn": "Biology, 11th Grade (General Education)",
    "officialBook": {
      "titleEl": "Βιολογία Γενικής Παιδείας Β΄ ΓΕΛ",
      "titleEn": "Biology, Grade 11 General Education: official textbook",
      "url": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
    },
    "officialSectionsEl": [
      "Κεφάλαιο 1: Άνθρωπος και Υγεία",
      "Κεφάλαιο 2: Άνθρωπος και Περιβάλλον",
      "Κεφάλαιο 3: Εξέλιξη (επιλεγμένες ενότητες)"
    ],
    "officialSectionsEn": [],
    "scopeNoteEl": "Το quiz διορθώθηκε από «Προσανατολισμού» σε Γενικής Παιδείας και πλέον ελέγχει θέματα από την επαληθευμένη επίσημη ύλη Β΄ ΓΕΛ 2026–27.",
    "scopeNoteEn": "The quiz was corrected from science-track to General Education and now covers the verified 2026–27 Grade 11 Biology scope.",
    "annualInstructionsStatus": "2026-27-exam-syllabus-verified",
    "annualInstructionsNoteEl": "Επίσημη απόφαση Υπουργείου για 2026–27.",
    "annualInstructionsNoteEn": "Official Ministry 2026–27 decision.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "quizId": "biologia-b-lykeiou"
  },
  "biologia-g-lykeiou": {
    "schoolYear": "2026-2027",
    "verificationDate": "2026-08-29",
    "coverageStatus": "annual-syllabus-verified",
    "coverageLabelEl": "Επίσημη ετήσια ύλη/εξεταστέα ύλη επαληθευμένη",
    "coverageLabelEn": "Official annual/examinable syllabus verified",
    "catalogUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp",
    "zone": "high",
    "sourceDiscipline": "official-source-only; no inference from topic label to official section without explicit gap alignment",
    "quizTitleEl": "Βιολογία, Γ' Λυκείου (Προσανατολισμού)",
    "quizTitleEn": "Biology, 12th Grade (Science Track)",
    "officialBook": {
      "titleEl": "Βιολογία Ομάδας Προσανατολισμού Γ΄ ΓΕΛ",
      "titleEn": "Biology, Grade 12 Science Track: official textbook",
      "url": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-C112"
    },
    "officialSectionsEl": [
      "Τεύχος Α: επιλεγμένες ενότητες για πρωτεΐνες, πυρήνα/ενδομεμβρανικό σύστημα, μιτοχόνδρια/χλωροπλάστες, ένζυμα, κυτταρικό κύκλο/διαίρεση",
      "Τεύχος Β, Κεφάλαιο 1: Γενετικό υλικό",
      "Κεφάλαιο 2: Αντιγραφή, έκφραση και ρύθμιση γενετικής πληροφορίας",
      "Κεφάλαιο 4: Τεχνολογία ανασυνδυασμένου DNA",
      "Κεφάλαιο 5: Μενδελική κληρονομικότητα",
      "Κεφάλαιο 6: Μεταλλάξεις",
      "Κεφάλαια 7–9: Βιοτεχνολογία και εφαρμογές"
    ],
    "officialSectionsEn": [],
    "scopeNoteEl": "Το quiz ανασχεδιάστηκε ώστε να ελέγχει βασικές έννοιες που ανήκουν στην επίσημη εξεταστέα ύλη Βιολογίας Προσανατολισμού για τις Πανελλαδικές 2027.",
    "scopeNoteEn": "The quiz was redesigned to cover core concepts included in the official 2027 Panhellenic Biology science-track syllabus.",
    "annualInstructionsStatus": "2027-panhellenic-syllabus-published-separate-decision",
    "annualInstructionsNoteEl": "Επίσημη απόφαση Υπουργείου για Πανελλαδικές 2027.",
    "annualInstructionsNoteEn": "Official Ministry decision for the 2027 Panhellenic examinations.",
    "annualInstructionsUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2",
    "quizId": "biologia-g-lykeiou"
  }
};
  const GAP_ALIGNMENT = {
  "math-a-gym.rational-number-order": {
    "sectionEl": "Κεφ. 7.2: Απόλυτη τιμή ρητού, αντίθετοι ρητοί, σύγκριση ρητών",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A200/293/"
  },
  "math-a-gym.absolute-value": {
    "sectionEl": "Κεφ. 7.2: Απόλυτη τιμή ρητού, αντίθετοι ρητοί, σύγκριση ρητών",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A200/293/"
  },
  "math-a-gym.gcd-lcm-confusion": {
    "sectionEl": "Κεφ. 1.5: Χαρακτήρες διαιρετότητας, ΜΚΔ, ΕΚΠ, πρώτοι παράγοντες",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A200/293/"
  },
  "math-a-gym.proportion-intro": {
    "sectionEl": "Κεφ. 6.2–6.3: Λόγος/αναλογία και ανάλογα ποσά",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A200/293/"
  },
  "istoria-a-gym.bronze-age-civilizations": {
    "sectionEl": "Κεφ. Β΄: Εποχή του Χαλκού, §§1–6",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A105/29/"
  },
  "istoria-a-gym.archaic-democracy-path": {
    "sectionEl": "Κεφ. Δ΄: Αθήνα: από τη βασιλεία στην αριστοκρατία και πορεία προς τη δημοκρατία",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A105/29/"
  },
  "istoria-a-gym.peloponnesian-war-sides": {
    "sectionEl": "Κεφ. ΣΤ΄ §§1–2: Αίτια/Αρχιδάμειος πόλεμος, Σικελική εκστρατεία/Δεκελεικός πόλεμος",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A105/29/"
  },
  "istoria-a-gym.alexander-legacy": {
    "sectionEl": "Κεφ. Ζ΄ §§3–4: Αλέξανδρος: κατάκτηση της Ανατολής και έργο",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A105/29/"
  },
  "biologia-a-gym.plant-animal-cell": {
    "sectionEl": "Κεφ. 1.2: Κύτταρο: η μονάδα της ζωής",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-a-gymnasiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Related mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A103/477/"
  },
  "biologia-a-gym.unicellular-nutrition": {
    "sectionEl": "Κεφ. 2.2: Πρόσληψη ουσιών και πέψη στους μονοκύτταρους οργανισμούς",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A103/477/"
  },
  "biologia-a-gym.adaptation-misconception": {
    "sectionEl": "Κεφ. 1.4: Αλληλεπιδράσεις και προσαρμογές",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-a-gymnasiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Exact mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A103/477/"
  },
  "biologia-a-gym.plant-vs-animal-digestion": {
    "sectionEl": "Κεφ. 2.1–2.3: Παραγωγή θρεπτικών ουσιών στα φυτά και πρόσληψη/πέψη στους οργανισμούς",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-a-gymnasiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Related mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A103/477/"
  },
  "biologia-a-gym.cell-membrane-function": {
    "sectionEl": "Κεφ. 1.2: Κύτταρο: η μονάδα της ζωής",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-a-gymnasiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Related mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A103/477/"
  },
  "glossa-a-gym.paragraph-argument-basic": {
    "sectionEl": "Εν. 2: Η παράγραφος · Εν. 3: Επιχειρηματολογία",
    "status": "related-section-verified",
    "sourceQuizId": "glossa-a-gymnasiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Related mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A112/337/"
  },
  "glossa-a-gym.verb-mood-basic": {
    "sectionEl": "Εν. 5: Το ρήμα",
    "status": "related-section-verified",
    "sourceQuizId": "glossa-a-gymnasiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη ενότητα",
    "statusLabelEn": "Related mapping to official section",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A112/337/"
  },
  "functions.one-formula-only": {
    "sectionEl": "Κεφ. 6.1: Η Έννοια της Συνάρτησης",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "functions.quadratic-always-two-roots": {
    "sectionEl": "Κεφ. 3.3: Εξισώσεις 2ου βαθμού",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "functions.sqrt-drops-absolute-value": {
    "sectionEl": "Κεφ. 2.3–2.4: Απόλυτη τιμή και Ρίζες Πραγματικών Αριθμών",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "functions.inequality-no-sign-flip": {
    "sectionEl": "Κεφ. 4.1–4.2: Ανισώσεις 1ου και 2ου βαθμού",
    "status": "exact-section-verified",
    "sourceQuizId": "mathimatika-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "functions.vertex-value-confusion": {
    "topicAnchorEl": "Μέγιστο/ελάχιστο συνάρτησης",
    "topicAnchorEn": "Function max/min",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-a-lykeiou",
    "statusLabelEl": "Επίσημο μάθημα/βιβλίο, όχι ακριβής ενότητα 2026–27",
    "statusLabelEn": "Official course/book, not an exact 2026–27 section mapping",
    "noteEl": "Το v2 δεν έχει επαληθεύσει section-level ότι το συγκεκριμένο max/min/vertex skill βρίσκεται στην καταχωρισμένη ετήσια εξεταστέα ενότητα. Μην το παρουσιάζεις ως επίσημη § χωρίς νέα πηγή.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-A100/490/3186,12916/"
  },
  "physics-lyk.motion-implies-force": {
    "sectionEl": "1.2.3: Ο πρώτος νόμος του Νεύτωνα",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-lyk.constant-velocity-needs-force": {
    "sectionEl": "1.2.3: Ο πρώτος νόμος του Νεύτωνα",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-lyk.newton-third-law-bigger-force": {
    "sectionEl": "1.3.1: Τρίτος νόμος του Νεύτωνα, Δράση:Αντίδραση",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-lyk.force-energy-same": {
    "sectionEl": "1.2.1 (έννοια δύναμης) σε συσχετισμό με 2.1.1 (έννοια έργου)",
    "status": "related-section-verified",
    "sourceQuizId": "fysiki-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "noteEl": "Η διάκριση δύναμης/ενέργειας είναι εννοιολογική γέφυρα ανάμεσα στις επαληθευμένες ενότητες, όχι τίτλος μίας μοναδικής παραγράφου."
  },
  "physics-lyk.energy-used-up": {
    "sectionEl": "2.1.4: Η μηχανική ενέργεια / διατήρηση μηχανικής ενέργειας",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-a-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "istoria-a-lyk.polis-definition": {
    "sectionEl": "ΙΙ.2.2: Αρχαϊκή εποχή (πόλη-κράτος, με τις ρητές εξαιρέσεις της απόφασης)",
    "status": "related-section-verified",
    "sourceQuizId": "istoria-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "noteEl": "Η απόφαση εξαιρεί συγκεκριμένες υποενότητες της §2.2. Ο Tutor δεν πρέπει να χρησιμοποιεί εξαιρούμενο κείμενο ως εξεταστέο."
  },
  "istoria-a-lyk.persian-wars-cause": {
    "sectionEl": "ΙΙ.2.2: Αρχαϊκή εποχή / μετάβαση προς τους Περσικούς Πολέμους",
    "status": "related-section-verified",
    "sourceQuizId": "istoria-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "istoria-a-lyk.direct-democracy-athens": {
    "sectionEl": "ΙΙ.2.3: Κλασική εποχή, Η εποχή του Περικλή",
    "status": "related-section-verified",
    "sourceQuizId": "istoria-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "istoria-a-lyk.alexander-empire-unity": {
    "topicAnchorEl": "Ελληνιστικοί χρόνοι: διάλυση της αυτοκρατορίας και Διάδοχοι",
    "topicAnchorEn": "Hellenistic period: breakup of the empire and Successors",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-a-lykeiou",
    "statusLabelEl": "Επίσημο βιβλίο/θεματική, όχι ακριβής εξεταστέα §",
    "statusLabelEn": "Official book/topic, not exact examinable section",
    "noteEl": "Η απόφαση 2026–27 περιλαμβάνει Ελληνιστικούς χρόνους αλλά εξαιρεί ορισμένες υποενότητες για τα βασίλεια. Το συγκεκριμένο gap δεν χαρακτηρίζεται exact-section-verified.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A102"
  },
  "istoria-b-lyk.fall-constantinople-1453": {
    "sectionEl": "Κεφ. 4 §7: Η άλωση της Κωνσταντινούπολης",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-b-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "istoria-b-lyk.renaissance-cause": {
    "sectionEl": "Κεφ. 6 §2: Αναγέννηση και Ανθρωπισμός",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-b-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "history-lyk.1922-military-defeat-only": {
    "sectionEl": "Κεφ. Γ΄ §5: Ο Μικρασιατικός Πόλεμος (1919–1922)",
    "status": "exact-section-verified",
    "sourceQuizId": "istoria-g-lykeiou",
    "statusLabelEl": "Ακριβής αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Exact mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-a.topic-sentence-missing": {
    "sectionEl": "Νεοελληνική Γλώσσα Α΄: επίσημο πλαίσιο οργάνωσης/δομής κειμένου και Γλωσσικές Ασκήσεις: παράγραφος",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-a.development-method-confusion": {
    "sectionEl": "Νεοελληνική Γλώσσα Α΄: Γλωσσικές Ασκήσεις: ανάπτυξη παραγράφου",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-a.unity-coherence": {
    "sectionEl": "Νεοελληνική Γλώσσα Α΄: επίσημο πλαίσιο δομής/συνοχής κειμένου",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-a.summary-length": {
    "sectionEl": "Νεοελληνική Γλώσσα Α΄: δεξιότητα περίληψης στο επίσημο πλαίσιο εξέτασης",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-a-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027",
    "noteEl": "Το ακριβές όριο λέξεων εξαρτάται από την εκφώνηση· ο Tutor δεν πρέπει να επινοεί σταθερό γενικό όριο."
  },
  "ekthesi.unsupported-argument": {
    "sectionEl": "Νεοελληνική Γλώσσα Β΄: επίσημο πλαίσιο κριτικής κατανόησης, οργάνωσης και τεκμηρίωσης λόγου / Έκφραση-Έκθεση Β΄",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-b.persuasion-modes-confusion": {
    "sectionEl": "Νεοελληνική Γλώσσα Β΄: επίσημο πλαίσιο κριτικής κατανόησης, οργάνωσης και τεκμηρίωσης λόγου / Έκφραση-Έκθεση Β΄",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-b.thesis-antithesis-missing": {
    "sectionEl": "Νεοελληνική Γλώσσα Β΄: επίσημο πλαίσιο κριτικής κατανόησης, οργάνωσης και τεκμηρίωσης λόγου / Έκφραση-Έκθεση Β΄",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-b.essay-structure-imbalance": {
    "sectionEl": "Νεοελληνική Γλώσσα Β΄: επίσημο πλαίσιο κριτικής κατανόησης, οργάνωσης και τεκμηρίωσης λόγου / Έκφραση-Έκθεση Β΄",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "ekthesi-b.evidence-generic": {
    "sectionEl": "Νεοελληνική Γλώσσα Β΄: επίσημο πλαίσιο κριτικής κατανόησης, οργάνωσης και τεκμηρίωσης λόγου / Έκφραση-Έκθεση Β΄",
    "status": "related-section-verified",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Σχετική αντιστοίχιση σε επίσημη εξεταστέα ενότητα 2026–27",
    "statusLabelEn": "Related mapping to official 2026–27 examinable section",
    "annualScopeVerified": true,
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "fractions.whole_number_bias": {
    "topicAnchorEl": "Σύγκριση κλασμάτων",
    "topicAnchorEn": "Comparing fractions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "fractions.add_across": {
    "topicAnchorEl": "Πρόσθεση κλασμάτων",
    "topicAnchorEn": "Adding fractions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "decimals.longer_is_larger": {
    "topicAnchorEl": "Σύγκριση δεκαδικών",
    "topicAnchorEn": "Comparing decimals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "division.remainder": {
    "topicAnchorEl": "Διαίρεση με υπόλοιπο",
    "topicAnchorEn": "Division with remainder",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "percent.as_fraction": {
    "topicAnchorEl": "Ποσοστά",
    "topicAnchorEn": "Percentages",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "measurement.area_perimeter_confusion": {
    "topicAnchorEl": "Εμβαδόν και περίμετρος",
    "topicAnchorEn": "Area and perimeter",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "history.athens-sparta-confusion": {
    "topicAnchorEl": "Αθήνα vs Σπάρτη",
    "topicAnchorEn": "Athens vs Sparta",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E105"
  },
  "history.byzantine-confusion": {
    "topicAnchorEl": "Βυζαντινή Αυτοκρατορία",
    "topicAnchorEn": "Byzantine Empire",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E105"
  },
  "grammar.subject-verb-agreement": {
    "topicAnchorEl": "Συμφωνία υποκειμένου-ρήματος",
    "topicAnchorEn": "Subject-verb agreement",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C107/675/"
  },
  "grammar.comma-usage": {
    "topicAnchorEl": "Χρήση κομμάτων",
    "topicAnchorEn": "Comma usage",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C107/675/"
  },
  "physics.force-motion-confusion": {
    "topicAnchorEl": "Δύναμη και κίνηση",
    "topicAnchorEn": "Force and motion",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "physics-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B200/455/"
  },
  "history.olympics-location": {
    "topicAnchorEl": "Τόπος Αρχαίων Ολυμπιακών",
    "topicAnchorEn": "Ancient Olympics location",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E105"
  },
  "history.philosophers-confusion": {
    "topicAnchorEl": "Αρχαίοι Φιλόσοφοι",
    "topicAnchorEn": "Ancient philosophers",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E105"
  },
  "physics.density-mass-confusion": {
    "topicAnchorEl": "Πυκνότητα vs Μάζα",
    "topicAnchorEn": "Density vs Mass",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "physics-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B200/455/"
  },
  "spelling.homophones": {
    "topicAnchorEl": "Ομόηχες λέξεις",
    "topicAnchorEn": "Homophones",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "spelling.oti-otidipote": {
    "topicAnchorEl": "«Ό,τι» και «ότι»",
    "topicAnchorEn": "'Ó,ti' vs 'óti'",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "spelling.final-n": {
    "topicAnchorEl": "Τελικό -ν",
    "topicAnchorEn": "Final -n rule",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "spelling.verb-ending": {
    "topicAnchorEl": "Κατάληξη ρήματος -ει",
    "topicAnchorEn": "Verb ending -ei",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "spelling.stem-confusion": {
    "topicAnchorEl": "Θέμα λέξης",
    "topicAnchorEn": "Word stem spelling",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "science-primary.seasons-distance": {
    "topicAnchorEl": "Αιτία των εποχών",
    "topicAnchorEn": "Cause of the seasons",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E107"
  },
  "science-primary.day-night-sun-moves": {
    "topicAnchorEl": "Αιτία μέρας/νύχτας",
    "topicAnchorEn": "Cause of day/night",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E107"
  },
  "science-primary.matter-disappears": {
    "topicAnchorEl": "Διατήρηση ύλης",
    "topicAnchorEn": "Conservation of matter",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E107"
  },
  "science-primary.heavier-falls-faster": {
    "topicAnchorEl": "Ελεύθερη πτώση",
    "topicAnchorEn": "Free fall",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E107"
  },
  "science-primary.moon-own-light": {
    "topicAnchorEl": "Φως του φεγγαριού",
    "topicAnchorEn": "Moonlight",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E107"
  },
  "efl-primary.third-person-s": {
    "topicAnchorEl": "Γ' πρόσωπο Ενεστώτα (-s)",
    "topicAnchorEn": "3rd person -s",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E103"
  },
  "efl-primary.age-construction": {
    "topicAnchorEl": "Έκφραση ηλικίας",
    "topicAnchorEn": "Age expressions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E103"
  },
  "efl-primary.definite-article-generic": {
    "topicAnchorEl": "Άρθρο σε γενικές έννοιες",
    "topicAnchorEn": "Article with generics",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E103"
  },
  "efl-primary.phonetic-spelling": {
    "topicAnchorEl": "Φωνητική γραφή αγγλικών",
    "topicAnchorEn": "Phonetic spelling",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E103"
  },
  "efl-primary.false-friend-sympathetic": {
    "topicAnchorEl": "False friend: sympathetic",
    "topicAnchorEn": "False friend: sympathetic",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E103"
  },
  "algebra.minus-sign": {
    "topicAnchorEl": "Πρόσημο πλην",
    "topicAnchorEn": "The minus sign",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C104/68/"
  },
  "algebra.equals-balance": {
    "topicAnchorEl": "Ισότητα ως ισορροπία",
    "topicAnchorEn": "Equals as balance",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C104/68/"
  },
  "algebra.distribute-negative": {
    "topicAnchorEl": "Επιμεριστική με αρνητικό",
    "topicAnchorEn": "Distributing a negative",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C104/68/"
  },
  "algebra.pythagorean-add": {
    "topicAnchorEl": "Πυθαγόρειο με κάθετη πλευρά",
    "topicAnchorEn": "Pythagorean theorem, finding a leg",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C104/68/"
  },
  "history-gym.agia-lavra": {
    "topicAnchorEl": "Έναρξη της Επανάστασης",
    "topicAnchorEn": "Start of the Revolution",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C105/65/"
  },
  "history-gym.greeks-united": {
    "topicAnchorEl": "Ενότητα κατά την Επανάσταση",
    "topicAnchorEn": "Unity during the Revolution",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C105/65/"
  },
  "history-gym.filiki-etaireia-leaders": {
    "topicAnchorEl": "Ίδρυση Φιλικής Εταιρείας",
    "topicAnchorEn": "Founding of the Filiki Etaireia",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C105/65/"
  },
  "history-gym.kryfo-sxoleio": {
    "topicAnchorEl": "Εκπαίδευση επί Τουρκοκρατίας",
    "topicAnchorEn": "Education under Ottoman rule",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C105/65/"
  },
  "efl-gym.present-perfect-past-simple": {
    "topicAnchorEl": "Present Perfect vs Past Simple",
    "topicAnchorEn": "Present Perfect vs Past Simple",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-C109"
  },
  "efl-gym.second-conditional-would": {
    "topicAnchorEl": "Δεύτερος Υποθετικός",
    "topicAnchorEn": "Second Conditional",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-C109"
  },
  "efl-gym.preposition-interference": {
    "topicAnchorEl": "Προθέσεις",
    "topicAnchorEn": "Prepositions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-C109"
  },
  "efl-gym.uncountable-nouns": {
    "topicAnchorEl": "Μη μετρήσιμα ουσιαστικά",
    "topicAnchorEn": "Uncountable nouns",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGYM-C109"
  },
  "ekthesi.text-type-ignore": {
    "topicAnchorEl": "Κειμενικά είδη",
    "topicAnchorEn": "Text types",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSB105/435/"
  },
  "ekthesi.register-confusion": {
    "topicAnchorEl": "Επίσημο ύφος",
    "topicAnchorEn": "Formal register",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSB105/435/"
  },
  "ekthesi.intro-conclusion-function": {
    "topicAnchorEl": "Δομή κειμένου",
    "topicAnchorEn": "Text structure",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSB105/435/"
  },
  "ekthesi.summary-copying": {
    "topicAnchorEl": "Περίληψη",
    "topicAnchorEn": "Summarizing",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C130/652/"
  },
  "history-lyk.kryfo-sxoleio-ban": {
    "topicAnchorEl": "Εκπαίδευση επί Τουρκοκρατίας",
    "topicAnchorEn": "Education under Ottoman rule",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL106"
  },
  "history-lyk.refugee-spontaneous": {
    "topicAnchorEl": "Συνθήκη της Λωζάνης",
    "topicAnchorEn": "Treaty of Lausanne",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL106"
  },
  "history-lyk.greeks-majority-asia-minor": {
    "topicAnchorEl": "Πληθυσμοί Μικράς Ασίας",
    "topicAnchorEn": "Asia Minor populations",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL106"
  },
  "efl-lyk.present-perfect-discourse": {
    "topicAnchorEl": "Χρόνοι σε αφήγηση",
    "topicAnchorEn": "Tenses in narrative",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-lyk.reported-speech-backshift": {
    "topicAnchorEl": "Πλάγιος λόγος",
    "topicAnchorEn": "Reported speech",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-lyk.false-friends-advanced": {
    "topicAnchorEl": "Προχωρημένα False Friends",
    "topicAnchorEn": "Advanced false friends",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-lyk.since-for-confusion": {
    "topicAnchorEl": "Since vs For",
    "topicAnchorEn": "Since vs For",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-lyk.preposition-collocation": {
    "topicAnchorEl": "Collocations με προθέσεις",
    "topicAnchorEn": "Preposition collocations",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "ekthesi-g.synthesis-sources-missing": {
    "topicAnchorEl": "Σύνθεση πηγών",
    "topicAnchorEn": "Synthesizing sources",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C130/652/"
  },
  "ekthesi-g.proposal-vague": {
    "topicAnchorEl": "Προτάσεις αντιμετώπισης",
    "topicAnchorEn": "Proposed solutions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C130/652/"
  },
  "ekthesi-g.critical-stance-missing": {
    "topicAnchorEl": "Κριτική αποτίμηση",
    "topicAnchorEn": "Critical evaluation",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C130/652/"
  },
  "ekthesi-g.counterargument-superficial": {
    "topicAnchorEl": "Βάθος αντίκρουσης",
    "topicAnchorEn": "Depth of rebuttal",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "ekthesi-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGL-C130/652/"
  },
  "istoria-b-lyk.byzantium-continuation": {
    "topicAnchorEl": "Βυζάντιο ως συνέχεια της Ρώμης",
    "topicAnchorEn": "Byzantium as Rome's continuation",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-B131"
  },
  "istoria-b-lyk.ottoman-administration": {
    "topicAnchorEl": "Οθωμανική διοίκηση Ελλήνων",
    "topicAnchorEn": "Ottoman rule over Greeks",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-b-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-B131"
  },
  "efl-c-lyk.mixed-conditionals": {
    "topicAnchorEl": "Μεικτοί υποθετικοί λόγοι",
    "topicAnchorEn": "Mixed conditionals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-c-lyk.inversion-emphasis": {
    "topicAnchorEl": "Αντεστραμμένη σύνταξη",
    "topicAnchorEn": "Inversion for emphasis",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-c-lyk.academic-collocations": {
    "topicAnchorEl": "Ακαδημαϊκές συνάψεις",
    "topicAnchorEn": "Academic collocations",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-c-lyk.passive-voice-formal": {
    "topicAnchorEl": "Παθητική φωνή σε επίσημο ύφος",
    "topicAnchorEn": "Passive voice in formal register",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-c-lyk.phrasal-vs-formal-verb": {
    "topicAnchorEl": "Phrasal vs επίσημο ρήμα",
    "topicAnchorEn": "Phrasal vs formal verb",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-g-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-st.past-simple-irregular": {
    "topicAnchorEl": "Ανώμαλα ρήματα (Past Simple)",
    "topicAnchorEn": "Irregular verbs (Past Simple)",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F101"
  },
  "efl-st.comparative-superlative": {
    "topicAnchorEl": "Συγκριτικός vs Υπερθετικός",
    "topicAnchorEn": "Comparative vs Superlative",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F101"
  },
  "efl-st.going-to-will": {
    "topicAnchorEl": "Going to vs Will",
    "topicAnchorEn": "Going to vs Will",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F101"
  },
  "efl-st.countable-uncountable": {
    "topicAnchorEl": "Countable vs Uncountable",
    "topicAnchorEn": "Countable vs Uncountable",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F101"
  },
  "efl-st.preposition-time": {
    "topicAnchorEl": "Προθέσεις χρόνου",
    "topicAnchorEn": "Prepositions of time",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F101"
  },
  "math-a-dim.number-order": {
    "topicAnchorEl": "Σύγκριση αριθμών έως 20",
    "topicAnchorEn": "Comparing numbers to 20",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "math-a-dim.addition-crossing-ten": {
    "topicAnchorEl": "Πρόσθεση με πέρασμα δεκάδας",
    "topicAnchorEn": "Addition crossing ten",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "math-a-dim.subtraction-borrowing-concept": {
    "topicAnchorEl": "Έννοια της αφαίρεσης",
    "topicAnchorEn": "Meaning of subtraction",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "math-a-dim.shape-recognition": {
    "topicAnchorEl": "Βασικά γεωμετρικά σχήματα",
    "topicAnchorEn": "Basic geometric shapes",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "glossa-a-dim.letter-sound-confusion": {
    "topicAnchorEl": "Παρόμοια γράμματα",
    "topicAnchorEn": "Similar-looking letters",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "glossa-a-dim.sentence-boundary": {
    "topicAnchorEl": "Όρια πρότασης",
    "topicAnchorEn": "Sentence boundaries",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "glossa-a-dim.plural-formation": {
    "topicAnchorEl": "Σχηματισμός πληθυντικού",
    "topicAnchorEn": "Forming plurals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "glossa-a-dim.word-order-basic": {
    "topicAnchorEl": "Σειρά λέξεων σε πρόταση",
    "topicAnchorEn": "Basic word order",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "math-b-dim.place-value-tens-ones": {
    "topicAnchorEl": "Δεκάδες & μονάδες",
    "topicAnchorEn": "Tens & ones",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "math-b-dim.multiplication-as-repeated-addition": {
    "topicAnchorEl": "Πολλαπλασιασμός ως πρόσθεση",
    "topicAnchorEn": "Multiplication as repeated addition",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "math-b-dim.telling-time-half-hour": {
    "topicAnchorEl": "Ώρα στη μισή",
    "topicAnchorEn": "Telling time to the half hour",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "math-b-dim.money-counting": {
    "topicAnchorEl": "Μέτρημα χρημάτων",
    "topicAnchorEn": "Counting money",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "glossa-b-dim.punctuation-question-exclamation": {
    "topicAnchorEl": "Ερωτηματικό vs Θαυμαστικό",
    "topicAnchorEn": "Question mark vs Exclamation mark",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "glossa-b-dim.story-sequence": {
    "topicAnchorEl": "Σειρά γεγονότων ιστορίας",
    "topicAnchorEn": "Story sequence",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "glossa-b-dim.simple-past-tense": {
    "topicAnchorEl": "Αόριστος χρόνος ρημάτων",
    "topicAnchorEn": "Simple past tense",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "glossa-b-dim.synonym-basic": {
    "topicAnchorEl": "Συνώνυμα & αντίθετα",
    "topicAnchorEn": "Synonyms & antonyms",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "math-c-dim.times-table-recall": {
    "topicAnchorEl": "Προπαίδεια",
    "topicAnchorEn": "Times tables",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "math-c-dim.division-as-sharing": {
    "topicAnchorEl": "Έννοια της διαίρεσης",
    "topicAnchorEn": "Meaning of division",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "math-c-dim.measurement-units": {
    "topicAnchorEl": "Μονάδες μέτρησης μήκους",
    "topicAnchorEn": "Units of length",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "math-c-dim.quarter-hour": {
    "topicAnchorEl": "Ώρα σε τέταρτα",
    "topicAnchorEn": "Telling time in quarters",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "glossa-c-dim.description-paragraph": {
    "topicAnchorEl": "Περιγραφική παράγραφος",
    "topicAnchorEn": "Descriptive paragraph",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "glossa-c-dim.iota-vowel-spelling": {
    "topicAnchorEl": "Ορθογραφία ι/η/υ",
    "topicAnchorEn": "Spelling ι/η/υ",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "glossa-c-dim.pronoun-reference": {
    "topicAnchorEl": "Αντωνυμίες αναφοράς",
    "topicAnchorEn": "Referential pronouns",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "glossa-c-dim.adjective-agreement": {
    "topicAnchorEl": "Συμφωνία επιθέτου-ουσιαστικού",
    "topicAnchorEn": "Adjective-noun agreement",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "efl-c-dim.colors-vocab": {
    "topicAnchorEl": "Λεξιλόγιο χρωμάτων",
    "topicAnchorEn": "Colors vocabulary",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C107"
  },
  "efl-c-dim.this-that": {
    "topicAnchorEl": "This vs That",
    "topicAnchorEn": "This vs That",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C107"
  },
  "efl-c-dim.numbers-1-20": {
    "topicAnchorEl": "Αριθμοί 1-20",
    "topicAnchorEn": "Numbers 1-20",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C107"
  },
  "efl-c-dim.family-vocab": {
    "topicAnchorEl": "Λεξιλόγιο οικογένειας",
    "topicAnchorEn": "Family vocabulary",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C107"
  },
  "istoria-c-dim.myth-vs-history": {
    "topicAnchorEl": "Μύθος vs Ιστορία",
    "topicAnchorEn": "Myth vs History",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C103"
  },
  "istoria-c-dim.heracles-labors-order": {
    "topicAnchorEl": "Άθλοι του Ηρακλή",
    "topicAnchorEn": "Labors of Heracles",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C103"
  },
  "istoria-c-dim.trojan-war-cause": {
    "topicAnchorEl": "Αιτία Τρωικού Πολέμου",
    "topicAnchorEn": "Cause of the Trojan War",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C103"
  },
  "istoria-c-dim.odysseus-journey": {
    "topicAnchorEl": "Το ταξίδι του Οδυσσέα",
    "topicAnchorEn": "Odysseus's journey",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C103"
  },
  "math-d-dim.fraction-same-denominator": {
    "topicAnchorEl": "Πρόσθεση κλασμάτων ίδιου παρονομαστή",
    "topicAnchorEn": "Adding fractions with the same denominator",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "math-d-dim.decimal-intro-place-value": {
    "topicAnchorEl": "Εισαγωγή στους δεκαδικούς",
    "topicAnchorEn": "Introduction to decimals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "math-d-dim.perimeter-concept": {
    "topicAnchorEl": "Έννοια της περιμέτρου",
    "topicAnchorEn": "Meaning of perimeter",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "math-d-dim.roman-numerals": {
    "topicAnchorEl": "Ρωμαϊκοί αριθμοί",
    "topicAnchorEn": "Roman numerals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "glossa-d-dim.paragraph-structure": {
    "topicAnchorEl": "Δομή αφηγηματικής παραγράφου",
    "topicAnchorEn": "Narrative paragraph structure",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "glossa-d-dim.future-tense": {
    "topicAnchorEl": "Μέλλοντας χρόνος",
    "topicAnchorEn": "Future tense",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "glossa-d-dim.adjective-vs-adverb": {
    "topicAnchorEl": "Επίθετο vs Επίρρημα",
    "topicAnchorEn": "Adjective vs Adverb",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "glossa-d-dim.direct-indirect-speech": {
    "topicAnchorEl": "Ευθύς vs Πλάγιος λόγος",
    "topicAnchorEn": "Direct vs Indirect speech",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "efl-d-dim.present-simple-routines": {
    "topicAnchorEl": "Present Simple καθημερινότητας",
    "topicAnchorEn": "Present Simple for routines",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D101"
  },
  "efl-d-dim.house-rooms-vocab": {
    "topicAnchorEl": "Λεξιλόγιο σπιτιού",
    "topicAnchorEn": "House vocabulary",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D101"
  },
  "efl-d-dim.can-ability": {
    "topicAnchorEl": "Can για ικανότητα",
    "topicAnchorEn": "Can for ability",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D101"
  },
  "efl-d-dim.days-months": {
    "topicAnchorEl": "Μέρες & μήνες",
    "topicAnchorEn": "Days & months",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "english-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D101"
  },
  "istoria-d-dim.minoan-crete": {
    "topicAnchorEl": "Μινωικός πολιτισμός",
    "topicAnchorEn": "Minoan civilization",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D103"
  },
  "istoria-d-dim.mycenaean-achaeans": {
    "topicAnchorEl": "Μυκηναίοι/Αχαιοί",
    "topicAnchorEn": "Mycenaeans/Achaeans",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D103"
  },
  "istoria-d-dim.polis-emergence": {
    "topicAnchorEl": "Γέννηση της πόλης-κράτους",
    "topicAnchorEn": "Rise of the city-state",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D103"
  },
  "istoria-d-dim.olympic-games-origin": {
    "topicAnchorEl": "Καταγωγή Ολυμπιακών Αγώνων",
    "topicAnchorEn": "Origin of the Olympic Games",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D103"
  },
  "math-st-dim.decimal-multiplication": {
    "topicAnchorEl": "Πολλαπλασιασμός δεκαδικών",
    "topicAnchorEn": "Multiplying decimals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "math-st-dim.discount-percent": {
    "topicAnchorEl": "Ποσοστά έκπτωσης",
    "topicAnchorEn": "Discount percentages",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "math-st-dim.ratio-proportion": {
    "topicAnchorEl": "Αναλογίες",
    "topicAnchorEn": "Ratios and proportions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "math-st-dim.volume-units": {
    "topicAnchorEl": "Μονάδες όγκου/χωρητικότητας",
    "topicAnchorEn": "Units of volume/capacity",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "glossa-st-dim.participle-recognition": {
    "topicAnchorEl": "Αναγνώριση μετοχής",
    "topicAnchorEn": "Recognizing participles",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "glossa-st-dim.compound-sentence-connectors": {
    "topicAnchorEl": "Σύνδεσμοι σύνθετης πρότασης",
    "topicAnchorEn": "Compound sentence connectors",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "glossa-st-dim.literary-text-interpretation": {
    "topicAnchorEl": "Ερμηνεία λογοτεχνικού κειμένου",
    "topicAnchorEn": "Interpreting a literary text",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "glossa-st-dim.paragraph-connectors-flow": {
    "topicAnchorEl": "Συνοχή ανάμεσα σε παραγράφους",
    "topicAnchorEn": "Flow between paragraphs",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "science-st-dim.circuit-open-closed": {
    "topicAnchorEl": "Ανοιχτό vs κλειστό κύκλωμα",
    "topicAnchorEn": "Open vs closed circuit",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL101"
  },
  "science-st-dim.digestive-system-order": {
    "topicAnchorEl": "Σειρά πεπτικού συστήματος",
    "topicAnchorEn": "Order of the digestive system",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL101"
  },
  "science-st-dim.renewable-vs-nonrenewable": {
    "topicAnchorEl": "Ανανεώσιμες vs μη ανανεώσιμες πηγές",
    "topicAnchorEn": "Renewable vs non-renewable energy",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL101"
  },
  "science-st-dim.states-of-matter-change": {
    "topicAnchorEl": "Αλλαγές κατάστασης ύλης",
    "topicAnchorEn": "Changes of state",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "science-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL101"
  },
  "istoria-st-dim.revolution-year": {
    "topicAnchorEl": "Έτος Ελληνικής Επανάστασης",
    "topicAnchorEn": "Year of the Greek Revolution",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F114"
  },
  "istoria-st-dim.kapodistrias-role": {
    "topicAnchorEl": "Ο Καποδίστριας",
    "topicAnchorEn": "Kapodistrias",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F114"
  },
  "istoria-st-dim.navarino-battle": {
    "topicAnchorEl": "Ναυμαχία του Ναβαρίνου",
    "topicAnchorEn": "Battle of Navarino",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F114"
  },
  "istoria-st-dim.modern-state-formation": {
    "topicAnchorEl": "Δημιουργία ελληνικού κράτους",
    "topicAnchorEn": "Formation of the Greek state",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F114"
  },
  "glossa-a-gym.parts-of-speech": {
    "topicAnchorEl": "Μέρη του λόγου",
    "topicAnchorEn": "Parts of speech",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A112/337/"
  },
  "glossa-a-gym.opinion-vs-fact": {
    "topicAnchorEl": "Άποψη vs Γεγονός",
    "topicAnchorEn": "Opinion vs Fact",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-A112/337/"
  },
  "efl-a-gym.present-cont-vs-simple": {
    "topicAnchorEl": "Present Continuous vs Simple",
    "topicAnchorEn": "Present Continuous vs Simple",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-a-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-a-gym.past-cont-vs-simple": {
    "topicAnchorEl": "Past Continuous vs Simple",
    "topicAnchorEn": "Past Continuous vs Simple",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-a-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-a-gym.comparative-forms": {
    "topicAnchorEl": "Σχηματισμός συγκριτικού",
    "topicAnchorEn": "Forming comparatives",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-a-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-a-gym.question-word-order": {
    "topicAnchorEl": "Σειρά λέξεων σε ερώτηση",
    "topicAnchorEn": "Question word order",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-a-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "math-b-gym.monomial-like-terms": {
    "topicAnchorEl": "Όμοιοι μονόμιοι όροι",
    "topicAnchorEn": "Like monomial terms",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B105/372/"
  },
  "math-b-gym.identity-square-sum": {
    "topicAnchorEl": "Ταυτότητα (α+β)²",
    "topicAnchorEn": "Identity (a+b)²",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B105/372/"
  },
  "math-b-gym.pythagorean-application": {
    "topicAnchorEl": "Εφαρμογή Πυθαγορείου θεωρήματος",
    "topicAnchorEn": "Applying the Pythagorean theorem",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B105/372/"
  },
  "math-b-gym.linear-equation-basic": {
    "topicAnchorEl": "Βασική εξίσωση 1ου βαθμού",
    "topicAnchorEn": "Basic first-degree equation",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "mathimatika-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B105/372/"
  },
  "glossa-b-gym.essay-intro-function": {
    "topicAnchorEl": "Λειτουργία πρόλογου δοκιμίου",
    "topicAnchorEn": "Function of an essay introduction",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B110/676/"
  },
  "glossa-b-gym.text-types-purpose": {
    "topicAnchorEl": "Σκοπός κειμενικού είδους",
    "topicAnchorEn": "Purpose of a text type",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B110/676/"
  },
  "glossa-b-gym.argument-vs-evidence": {
    "topicAnchorEl": "Επιχείρημα vs Τεκμήριο",
    "topicAnchorEn": "Argument vs Evidence",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B110/676/"
  },
  "glossa-b-gym.summary-vs-opinion": {
    "topicAnchorEl": "Περίληψη vs Σχόλιο",
    "topicAnchorEn": "Summary vs Commentary",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B110/676/"
  },
  "istoria-b-gym.hellenistic-fusion": {
    "topicAnchorEl": "Ελληνιστικός πολιτισμός",
    "topicAnchorEn": "Hellenistic culture",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B107/371/"
  },
  "istoria-b-gym.rome-greece-conquest": {
    "topicAnchorEl": "Ρωμαϊκή κατάκτηση της Ελλάδας",
    "topicAnchorEn": "Roman conquest of Greece",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B107/371/"
  },
  "istoria-b-gym.byzantium-transition": {
    "topicAnchorEl": "Μετάβαση στο Βυζάντιο",
    "topicAnchorEn": "Transition to Byzantium",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B107/371/"
  },
  "istoria-b-gym.medieval-feudalism": {
    "topicAnchorEl": "Φεουδαρχία στη Δυτική Ευρώπη",
    "topicAnchorEn": "Feudalism in Western Europe",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "istoria-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-B107/371/"
  },
  "efl-b-gym.present-perfect-intro": {
    "topicAnchorEl": "Εισαγωγή στον Present Perfect",
    "topicAnchorEn": "Introduction to Present Perfect",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-b-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-b-gym.modals-obligation": {
    "topicAnchorEl": "Must vs Should",
    "topicAnchorEn": "Must vs Should",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-b-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-b-gym.passive-voice-intro": {
    "topicAnchorEl": "Εισαγωγή στην παθητική φωνή",
    "topicAnchorEn": "Introduction to the passive voice",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-b-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "efl-b-gym.relative-clauses": {
    "topicAnchorEl": "Αναφορικές προτάσεις",
    "topicAnchorEn": "Relative clauses",
    "status": "catalog-topic-anchor",
    "sourceQuizId": "english-b-gymnasiou",
    "statusLabelEl": "Topic anchor με επιβεβαίωση μόνο μαθήματος/καταλόγου",
    "statusLabelEn": "Topic anchor with course/catalog verification only",
    "noteEl": "Έχει επιβεβαιωθεί το μάθημα/η τάξη στον επίσημο κατάλογο, όχι ακριβής ενότητα. Μην επινοείς επίσημο κεφάλαιο/σελίδα.",
    "sourceUrl": "https://www.ebooks.edu.gr/ebooks/v2/allcoursespdf.jsp"
  },
  "physics-g-gym.ohms-law": {
    "topicAnchorEl": "Νόμος του Ωμ",
    "topicAnchorEn": "Ohm's Law",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "fysiki-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C201/296/"
  },
  "physics-g-gym.speed-velocity-formula": {
    "topicAnchorEl": "Τύπος ταχύτητας",
    "topicAnchorEn": "Speed formula",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "fysiki-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C201/296/"
  },
  "physics-g-gym.energy-transformation": {
    "topicAnchorEl": "Μετατροπή ενέργειας",
    "topicAnchorEn": "Energy transformation",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "fysiki-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C201/296/"
  },
  "chimeia-g-gym.element-vs-compound": {
    "topicAnchorEl": "Στοιχείο vs Ένωση",
    "topicAnchorEn": "Element vs Compound",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "chimeia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C102/223/"
  },
  "chimeia-g-gym.mixture-vs-compound": {
    "topicAnchorEl": "Μείγμα vs Ένωση",
    "topicAnchorEn": "Mixture vs Compound",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "chimeia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C102/223/"
  },
  "chimeia-g-gym.atom-structure": {
    "topicAnchorEl": "Δομή του ατόμου",
    "topicAnchorEn": "Structure of the atom",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "chimeia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C102/223/"
  },
  "math-a-dim.ordinal-numbers": {
    "topicAnchorEl": "Τακτικοί αριθμοί",
    "topicAnchorEn": "Ordinal numbers",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "math-a-dim.length-comparison": {
    "topicAnchorEl": "Σύγκριση μήκους",
    "topicAnchorEn": "Comparing length",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "math-a-dim.subtraction-crossing-ten": {
    "topicAnchorEl": "Αφαίρεση με πέρασμα δεκάδας",
    "topicAnchorEn": "Subtraction crossing ten",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "math-a-dim.coin-recognition": {
    "topicAnchorEl": "Αναγνώριση νομισμάτων",
    "topicAnchorEn": "Coin recognition",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A102"
  },
  "glossa-a-dim.rhyming-words": {
    "topicAnchorEl": "Ομοιοκαταληξία",
    "topicAnchorEn": "Rhyming words",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "glossa-a-dim.singular-plural-verb": {
    "topicAnchorEl": "Συμφωνία ρήματος-υποκειμένου (ενικός/πληθυντικός)",
    "topicAnchorEn": "Verb-subject agreement (singular/plural)",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "glossa-a-dim.question-mark-usage": {
    "topicAnchorEl": "Χρήση ερωτηματικού",
    "topicAnchorEn": "Using the question mark",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "glossa-a-dim.opposite-words-basic": {
    "topicAnchorEl": "Απλά αντίθετα",
    "topicAnchorEn": "Basic opposites",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-a-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-A110"
  },
  "math-b-dim.addition-two-digit-no-carry": {
    "topicAnchorEl": "Πρόσθεση διψήφιων χωρίς κρατούμενο",
    "topicAnchorEn": "Two-digit addition without carrying",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "math-b-dim.subtraction-two-digit-borrow": {
    "topicAnchorEl": "Αφαίρεση διψήφιων με δανεικό",
    "topicAnchorEn": "Two-digit subtraction with borrowing",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "math-b-dim.even-odd-numbers": {
    "topicAnchorEl": "Ζυγοί και μονοί αριθμοί",
    "topicAnchorEn": "Even and odd numbers",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "math-b-dim.simple-graph-reading": {
    "topicAnchorEl": "Ανάγνωση απλού ραβδογράμματος",
    "topicAnchorEn": "Reading a simple bar graph",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B101"
  },
  "glossa-b-dim.capital-letter-proper-nouns": {
    "topicAnchorEl": "Κεφαλαίο σε κύρια ονόματα",
    "topicAnchorEn": "Capital letters for proper nouns",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "glossa-b-dim.compound-word-basic": {
    "topicAnchorEl": "Απλές σύνθετες λέξεις",
    "topicAnchorEn": "Basic compound words",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "glossa-b-dim.sentence-vs-fragment": {
    "topicAnchorEl": "Πλήρης πρόταση vs απόσπασμα",
    "topicAnchorEn": "Complete sentence vs fragment",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "glossa-b-dim.dialogue-punctuation": {
    "topicAnchorEl": "Στίξη διαλόγου",
    "topicAnchorEn": "Dialogue punctuation",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-b-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-B105"
  },
  "math-c-dim.multiplication-two-digit-by-one": {
    "topicAnchorEl": "Πολλαπλασιασμός διψήφιου επί μονοψήφιο",
    "topicAnchorEn": "Two-digit by one-digit multiplication",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "math-c-dim.weight-units": {
    "topicAnchorEl": "Μονάδες βάρους",
    "topicAnchorEn": "Units of weight",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "math-c-dim.remainder-meaning": {
    "topicAnchorEl": "Έννοια του υπολοίπου",
    "topicAnchorEn": "Meaning of the remainder",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "math-c-dim.simple-fraction-recognition": {
    "topicAnchorEl": "Αναγνώριση απλού κλάσματος",
    "topicAnchorEn": "Recognizing simple fractions",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C102"
  },
  "glossa-c-dim.verb-tense-consistency": {
    "topicAnchorEl": "Συνέπεια χρόνου στην αφήγηση",
    "topicAnchorEn": "Tense consistency in narration",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "glossa-c-dim.comma-in-list": {
    "topicAnchorEl": "Κόμμα σε απαρίθμηση",
    "topicAnchorEn": "Comma in a list",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "glossa-c-dim.homophone-basic": {
    "topicAnchorEl": "Απλές ομόηχες λέξεις",
    "topicAnchorEn": "Basic homophones",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "glossa-c-dim.title-choice": {
    "topicAnchorEl": "Επιλογή κατάλληλου τίτλου",
    "topicAnchorEn": "Choosing a fitting title",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-c-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-C105"
  },
  "math-d-dim.fraction-simplify-basic": {
    "topicAnchorEl": "Απλοποίηση κλάσματος",
    "topicAnchorEn": "Simplifying a fraction",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "math-d-dim.decimal-compare": {
    "topicAnchorEl": "Σύγκριση δεκαδικών",
    "topicAnchorEn": "Comparing decimals",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "math-d-dim.area-basic-concept": {
    "topicAnchorEl": "Βασική έννοια εμβαδού",
    "topicAnchorEn": "Basic concept of area",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "math-d-dim.multiplication-by-10-100": {
    "topicAnchorEl": "Πολλαπλασιασμός με 10 και 100",
    "topicAnchorEn": "Multiplying by 10 and 100",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D102"
  },
  "glossa-d-dim.subject-predicate": {
    "topicAnchorEl": "Υποκείμενο και κατηγόρημα",
    "topicAnchorEn": "Subject and predicate",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "glossa-d-dim.paragraph-main-idea": {
    "topicAnchorEl": "Κύρια ιδέα παραγράφου",
    "topicAnchorEn": "Main idea of a paragraph",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "glossa-d-dim.conjunction-choice": {
    "topicAnchorEl": "Επιλογή κατάλληλου συνδέσμου",
    "topicAnchorEn": "Choosing the right conjunction",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "glossa-d-dim.formal-informal-greeting": {
    "topicAnchorEl": "Επίσημος vs ανεπίσημος χαιρετισμός",
    "topicAnchorEn": "Formal vs informal greeting",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-d-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-D109"
  },
  "math-e-dim.multiply-fraction-whole": {
    "topicAnchorEl": "Πολλαπλασιασμός κλάσματος με ακέραιο",
    "topicAnchorEn": "Multiplying a fraction by a whole number",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "math-e-dim.average-concept": {
    "topicAnchorEl": "Έννοια του μέσου όρου",
    "topicAnchorEn": "Concept of average",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "math-e-dim.volume-cube-basic": {
    "topicAnchorEl": "Όγκος με κύβους μονάδας",
    "topicAnchorEn": "Volume using unit cubes",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "math-e-dim.negative-number-intro": {
    "topicAnchorEl": "Εισαγωγή σε αρνητικούς αριθμούς",
    "topicAnchorEn": "Introduction to negative numbers",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E113"
  },
  "glossa-e-dim.simile-metaphor-basic": {
    "topicAnchorEl": "Απλή παρομοίωση",
    "topicAnchorEn": "Basic simile",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "glossa-e-dim.cause-effect-connector": {
    "topicAnchorEl": "Σύνδεσμοι αιτίας-αποτελέσματος",
    "topicAnchorEn": "Cause-effect connectors",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "glossa-e-dim.formal-letter-structure": {
    "topicAnchorEl": "Δομή επίσημης επιστολής",
    "topicAnchorEn": "Formal letter structure",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "glossa-e-dim.adjective-degree": {
    "topicAnchorEl": "Βαθμοί επιθέτου",
    "topicAnchorEn": "Degrees of adjectives",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-e-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-E104"
  },
  "math-st-dim.percent-increase": {
    "topicAnchorEl": "Ποσοστό αύξησης",
    "topicAnchorEn": "Percent increase",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "math-st-dim.speed-distance-time-basic": {
    "topicAnchorEl": "Σχέση ταχύτητας-απόστασης-χρόνου",
    "topicAnchorEn": "Speed-distance-time relationship",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "math-st-dim.negative-number-operations": {
    "topicAnchorEl": "Βασικές πράξεις με αρνητικούς",
    "topicAnchorEn": "Basic operations with negative numbers",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "math-st-dim.probability-basic": {
    "topicAnchorEl": "Βασική πιθανότητα",
    "topicAnchorEn": "Basic probability",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "math-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM101"
  },
  "glossa-st-dim.passive-voice-basic": {
    "topicAnchorEl": "Βασική αναγνώριση παθητικής φωνής",
    "topicAnchorEn": "Basic passive voice recognition",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "glossa-st-dim.text-purpose-identification": {
    "topicAnchorEl": "Αναγνώριση σκοπού κειμένου",
    "topicAnchorEn": "Identifying a text's purpose",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "glossa-st-dim.register-formal-informal": {
    "topicAnchorEl": "Επίσημο vs ανεπίσημο ύφος",
    "topicAnchorEn": "Formal vs informal register",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "glossa-st-dim.conclusion-quality": {
    "topicAnchorEl": "Ποιότητα επιλόγου",
    "topicAnchorEn": "Quality of a conclusion",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "glossa-st-dimotikou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSDIM-F102"
  },
  "biologia-b-gym.gas-exchange-organ": {
    "topicAnchorEl": "Ανταλλαγή αερίων",
    "topicAnchorEn": "Gas exchange organ",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-b-gym.heart-role": {
    "topicAnchorEl": "Ρόλος της καρδιάς",
    "topicAnchorEn": "Role of the heart",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-b-gym.blood-function": {
    "topicAnchorEl": "Λειτουργία του αίματος",
    "topicAnchorEn": "Function of blood",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-b-gym.producer-definition": {
    "topicAnchorEl": "Παραγωγοί σε οικοσύστημα",
    "topicAnchorEn": "Producers in an ecosystem",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-b-gym.nervous-system-role": {
    "topicAnchorEl": "Ρόλος νευρικού συστήματος",
    "topicAnchorEn": "Role of the nervous system",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-b-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-g-gym.dna-location": {
    "topicAnchorEl": "Θέση του DNA",
    "topicAnchorEn": "Location of DNA",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-g-gym.inheritance-both-parents": {
    "topicAnchorEl": "Κληρονομικότητα από τους γονείς",
    "topicAnchorEn": "Inheritance from parents",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-g-gym.biodiversity-definition": {
    "topicAnchorEl": "Έννοια βιοποικιλότητας",
    "topicAnchorEn": "Concept of biodiversity",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-g-gym.extinction-cause": {
    "topicAnchorEl": "Αιτία εξαφάνισης είδους",
    "topicAnchorEn": "Cause of species extinction",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-g-gym.reproduction-purpose": {
    "topicAnchorEl": "Σκοπός αναπαραγωγής",
    "topicAnchorEn": "Purpose of reproduction",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-g-gymnasiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/modules/ebook/show.php/DSGYM-C103/478/"
  },
  "biologia-a-lyk.homeostasis": {
    "topicAnchorEl": "Ομοιόσταση",
    "topicAnchorEn": "Homeostasis",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-a-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A105"
  },
  "biologia-a-lyk.red-blood-cell-role": {
    "topicAnchorEl": "Ρόλος ερυθρών αιμοσφαιρίων",
    "topicAnchorEn": "Role of red blood cells",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-a-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A105"
  },
  "biologia-a-lyk.immune-system": {
    "topicAnchorEl": "Ανοσοποιητικό σύστημα",
    "topicAnchorEn": "Immune system",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-a-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A105"
  },
  "biologia-a-lyk.mitochondria-function": {
    "topicAnchorEl": "Λειτουργία μιτοχονδρίου",
    "topicAnchorEn": "Function of the mitochondrion",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-a-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A105"
  },
  "biologia-a-lyk.digestion-purpose": {
    "topicAnchorEl": "Σκοπός πέψης",
    "topicAnchorEn": "Purpose of digestion",
    "status": "official-course-topic-anchor",
    "sourceQuizId": "biologia-a-lykeiou",
    "statusLabelEl": "Topic anchor μέσα σε επιβεβαιωμένο επίσημο μάθημα/βιβλίο",
    "statusLabelEn": "Topic anchor within a verified official course/book",
    "noteEl": "Το topic anchor είναι το όνομα του υπάρχοντος μαθησιακού κενού του site. Δεν αποτελεί από μόνο του επίσημο τίτλο κεφαλαίου ή απόδειξη ότι το συγκεκριμένο σημείο είναι εξεταστέο το 2026–27.",
    "sourceUrl": "https://old.ebooks.edu.gr/new/ebooks.php?course=DSGL-A105"
  },
  "physics-b-lyk.circular-motion-tangent-velocity": {
    "topicAnchorEl": "Διεύθυνση ταχύτητας στην κυκλική κίνηση",
    "topicAnchorEn": "Velocity direction in circular motion",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-b-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Ομαλή κυκλική κίνηση: στιγμιαία ταχύτητα",
    "officialSectionEn": "Uniform circular motion: instantaneous velocity",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-b-lyk.centripetal-force-direction": {
    "topicAnchorEl": "Κατεύθυνση κεντρομόλου δύναμης",
    "topicAnchorEn": "Direction of centripetal force",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-b-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Κεντρομόλος δύναμη / κεντρομόλος επιτάχυνση",
    "officialSectionEn": "Centripetal force / acceleration",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-b-lyk.momentum-definition": {
    "topicAnchorEl": "Ορμή: μάζα, ταχύτητα και κατεύθυνση",
    "topicAnchorEn": "Momentum: mass, velocity and direction",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-b-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Κεφάλαιο 2: Ορμή",
    "officialSectionEn": "Chapter 2: Momentum",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-b-lyk.momentum-conservation": {
    "topicAnchorEl": "Διατήρηση ορμής στις κρούσεις",
    "topicAnchorEn": "Conservation of momentum in collisions",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-b-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Κεφάλαιο 2: Διατήρηση ορμής και κρούσεις",
    "officialSectionEn": "Chapter 2: Momentum conservation and collisions",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "physics-b-lyk.first-law-energy-balance": {
    "topicAnchorEl": "1ος θερμοδυναμικός νόμος: ενεργειακό ισοζύγιο",
    "topicAnchorEn": "First law of thermodynamics: energy balance",
    "status": "exact-section-verified",
    "sourceQuizId": "fysiki-b-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Κεφάλαιο 4: Θερμοδυναμική / 1ος θερμοδυναμικός νόμος",
    "officialSectionEn": "Chapter 4: Thermodynamics / first law",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "biologia-b-lyk.specific-nonspecific-immunity": {
    "topicAnchorEl": "Ειδική και μη ειδική άμυνα",
    "topicAnchorEn": "Specific and non-specific immunity",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-b-lykeiou",
    "statusLabelEl": "Σχετική επίσημη ενότητα επαληθευμένη",
    "statusLabelEn": "Related official section verified",
    "officialSectionEl": "Κεφάλαιο 1: Άνθρωπος και Υγεία",
    "officialSectionEn": "Chapter 1: Human and Health",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "biologia-b-lyk.vaccine-immune-memory": {
    "topicAnchorEl": "Εμβολιασμός και ανοσολογική μνήμη",
    "topicAnchorEn": "Vaccination and immune memory",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-b-lykeiou",
    "statusLabelEl": "Σχετική επίσημη ενότητα επαληθευμένη",
    "statusLabelEn": "Related official section verified",
    "officialSectionEl": "Κεφάλαιο 1: Άνθρωπος και Υγεία",
    "officialSectionEn": "Chapter 1: Human and Health",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "biologia-b-lyk.producers-ecosystem": {
    "topicAnchorEl": "Παραγωγοί σε ένα οικοσύστημα",
    "topicAnchorEn": "Producers in an ecosystem",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-b-lykeiou",
    "statusLabelEl": "Σχετική επίσημη ενότητα επαληθευμένη",
    "statusLabelEn": "Related official section verified",
    "officialSectionEl": "Κεφάλαιο 2: Άνθρωπος και Περιβάλλον",
    "officialSectionEn": "Chapter 2: Humans and Environment",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "biologia-b-lyk.energy-trophic-levels": {
    "topicAnchorEl": "Ροή ενέργειας στα τροφικά επίπεδα",
    "topicAnchorEn": "Energy flow across trophic levels",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-b-lykeiou",
    "statusLabelEl": "Σχετική επίσημη ενότητα επαληθευμένη",
    "statusLabelEn": "Related official section verified",
    "officialSectionEl": "Κεφάλαιο 2: Άνθρωπος και Περιβάλλον",
    "officialSectionEn": "Chapter 2: Humans and Environment",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "biologia-b-lyk.natural-selection-population": {
    "topicAnchorEl": "Φυσική επιλογή σε πληθυσμούς",
    "topicAnchorEn": "Natural selection in populations",
    "status": "related-section-verified",
    "sourceQuizId": "biologia-b-lykeiou",
    "statusLabelEl": "Σχετική επίσημη ενότητα επαληθευμένη",
    "statusLabelEn": "Related official section verified",
    "officialSectionEl": "Κεφάλαιο 3: Εξέλιξη (επιλεγμένες ενότητες)",
    "officialSectionEn": "Chapter 3: Evolution (selected sections)",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70567-29-07-26-kathorismos-exetasteas-yles-gia-ta-mathemata-ton-a-b-kai-g-taxeon-genikou-lykeiou-pou-exetazontai-graptos-stis-proagogikes-kai-apolyteries-exetaseis-gia-to-sch-etos-2026-2027"
  },
  "biologia-g-lyk.genetic-material-dna": {
    "topicAnchorEl": "DNA ως γενετικό υλικό",
    "topicAnchorEn": "DNA as genetic material",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-g-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Τεύχος Β, Κεφάλαιο 1: Το γενετικό υλικό",
    "officialSectionEn": "Book B, Chapter 1: Genetic material",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2"
  },
  "biologia-g-lyk.transcription-translation": {
    "topicAnchorEl": "Μεταγραφή και μετάφραση",
    "topicAnchorEn": "Transcription and translation",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-g-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Τεύχος Β, Κεφάλαιο 2: Έκφραση της γενετικής πληροφορίας",
    "officialSectionEn": "Book B, Chapter 2: Expression of genetic information",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2"
  },
  "biologia-g-lyk.heterozygous-genotype": {
    "topicAnchorEl": "Ομόζυγος και ετερόζυγος γονότυπος",
    "topicAnchorEn": "Homozygous and heterozygous genotype",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-g-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Τεύχος Β, Κεφάλαιο 5: Μενδελική κληρονομικότητα",
    "officialSectionEn": "Book B, Chapter 5: Mendelian inheritance",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2"
  },
  "biologia-g-lyk.mutation-definition": {
    "topicAnchorEl": "Μεταλλάξεις και γενετικό υλικό",
    "topicAnchorEn": "Mutations and genetic material",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-g-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Τεύχος Β, Κεφάλαιο 6: Μεταλλάξεις",
    "officialSectionEn": "Book B, Chapter 6: Mutations",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2"
  },
  "biologia-g-lyk.recombinant-dna": {
    "topicAnchorEl": "Ανασυνδυασμένο DNA",
    "topicAnchorEn": "Recombinant DNA",
    "status": "exact-section-verified",
    "sourceQuizId": "biologia-g-lykeiou",
    "statusLabelEl": "Ακριβής επίσημη αντιστοίχιση ενότητας",
    "statusLabelEn": "Exact official section mapping",
    "officialSectionEl": "Τεύχος Β, Κεφάλαιο 4: Τεχνολογία ανασυνδυασμένου DNA",
    "officialSectionEn": "Book B, Chapter 4: Recombinant-DNA technology",
    "noteEl": "Η αντιστοίχιση βασίζεται στην επίσημη ετήσια/εξεταστέα ύλη. Ο Tutor πρέπει να μένει στο επίπεδο και στην ορολογία της τάξης.",
    "noteEn": "Mapping is based on the official annual/examinable syllabus. The Tutor should stay at the grade’s level and terminology.",
    "sourceUrl": "https://www.minedu.gov.gr/site/70350-07-07-26-kathorismos-exetasteas-yles-gia-to-etos-2027-gia-ta-mathemata-pou-exetazontai-panelladika-gia-ten-eisagoge-sten-tritobathmia-ekpaideuse-apophoiton-g-taxes-emeresiou-genikou-lykeiou-kai-g-taxes-esperinou-genikou-lykeiou-2"
  }
};
  function getByQuizId(id){ return BY_QUIZ[id] || null; }
  function getGapAlignment(id){ return GAP_ALIGNMENT[id] || null; }
  window.AITOOLSKIDS_OFFICIAL_CURRICULUM = Object.freeze({ meta:Object.freeze(META), byQuiz:Object.freeze(BY_QUIZ), gapAlignment:Object.freeze(GAP_ALIGNMENT), getByQuizId, getGapAlignment });
})();
