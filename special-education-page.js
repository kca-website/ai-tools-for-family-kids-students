(function(){
  ...
  function gradeLabelFromEntry(entry){
    return entry?.gradeLabel || `${String(entry?.grade||"").toUpperCase()}΄ τάξη`;
  }

  function aiHref(schoolType,gradeId,subjectId,role="student"){
    const zone=schoolType==="eneegyl"?"high":"middle";
    const params=new URLSearchParams({schoolTrack:schoolType,grade:gradeId,subject:subjectId});
  ...
  function renderSpecialGymProfile(){
    ...
    const cards=(grade?.subjects||[]).map((row)=>{
      ...
      const helper=hasLearning
        ? "Έχει έτοιμη βήμα-βήμα μελέτη και μικρό διαγνωστικό."
    ...
    sgProfile.innerHTML=`
      <div class="sp-choice-block">
        <h3>2. Διάλεξε τάξη</h3>
        ${gradeTabs(grades,selectedSpecialGymGrade,"data-sg-grade")}
      </div>
      <div class="sp-choice-block">
        <h3>3. Διάλεξε μάθημα</h3>
        <div class="sp-subject-grid">${cards}</div>
      </div>`;
  }
  ...
  function renderEneegylProfile(){
    ...
    const cards=current.map((entry)=>{
      const q=Q?.[entry.id];
      const helper=q?.questions?.length
        ? "Έτοιμη απλή παρουσίαση, εξάσκηση και μικρό διαγνωστικό."
        : "Έτοιμη απλή παρουσίαση και εξάσκηση.";
      return `<article class="sp-subject-card">
    ...
    enProfile.innerHTML=`
      <div class="sp-choice-block">
        <h3>2. Διάλεξε τάξη</h3>
        ${gradeTabs(grades,selectedEneegylGrade,"data-en-grade")}
      </div>
      <div class="sp-choice-block">
        <h3>3. Διάλεξε μάθημα</h3>
        <div class="sp-subject-grid">${cards||'<div class="sp-empty">Δεν υπάρχει ακόμη έτοιμη μαθησιακή διαδρομή για αυτή την τάξη.</div>'}</div>
      </div>`;
  }
  ...
  function renderUnit(id,target,focus="learn"){
    ...
    target.innerHTML=`<article class="sp-unit">
      <header class="sp-unit__header">
        <div><span class="sp-kicker">${esc(gradeLabelFromEntry(c))}${c.sector?` · ${esc(c.sector)}`:""}</span><h3>${esc(c.subject)}</h3></div>
      </header>
      ${unitAiActions(c)}
  ...
  sgProfile?.addEventListener("click",(event)=>{
    const grade=event.target.closest("[data-sg-grade]");
    if(grade){selectedSpecialGymGrade=grade.dataset.sgGrade;sgUnitMount.innerHTML="";renderSpecialGymProfile();return;}
    const open=event.target.closest("[data-open-sg-unit]");
    if(open) renderUnit(open.dataset.openSgUnit,sgUnitMount,open.dataset.focus||"learn");
  });

  enProfile?.addEventListener("click",(event)=>{
    const grade=event.target.closest("[data-en-grade]");
    if(grade){selectedEneegylGrade=grade.dataset.enGrade;unitMount.innerHTML="";renderEneegylProfile();return;}
    const open=event.target.closest("[data-open-unit]");
    if(open) renderUnit(open.dataset.openUnit,unitMount,open.dataset.focus||"learn");
  });
