(function(){
  ...
  function renderUnit(id){
    ...
    if(!c || c.status!=="verified" || !l){
      unitMount.innerHTML='<div class="sp-pending-box"><strong>Η ενότητα δεν είναι ακόμη έτοιμη.</strong></div>';
      return;
    }
    const primarySourceLabel=c.verificationBasis==="current-exam-syllabus" ? "Επίσημη εξεταστέα ύλη ↗" : "Επίσημη πηγή ↗";
    const instructionLink=c.instructionSourceUrl
      ? `<a href="${esc(c.instructionSourceUrl)}" target="_blank" rel="noopener">Οδηγίες διδασκαλίας 2026-27 ↗</a>`
      : "";
    ...
    unitMount.innerHTML=`<article class="sp-unit">
      ...
      <div class="sp-source-strip"><span>Έλεγχος πηγής: ${esc(c.verificationDate)}</span><span>${esc(c.protocol)}</span><a href="${esc(c.sourceUrl)}" target="_blank" rel="noopener">${primarySourceLabel}</a>${c.sourcePdfUrl?`<a href="${esc(c.sourcePdfUrl)}" target="_blank" rel="noopener">Εγκύκλιος PDF ↗</a>`:""}${instructionLink}</div>
      <details class="sp-step" open><summary><span class="sp-num">1</span>Επίσημη βάση</summary><div class="sp-step__body"><div class="sp-info"><strong>Τι σημαίνει «επαληθευμένο» εδώ</strong><p>${c.verificationBasis==="current-exam-syllabus"?"Τα παρακάτω σημεία ελέγχθηκαν στην επίσημη εξεταστέα ύλη 2026-2027. Η μαθησιακή παρουσίαση περιορίζεται σκόπιμα μόνο σε αυτό το επαληθευμένο τμήμα.":"Ελέγχθηκε η επίσημη εγκύκλιος του Υπουργείου. Δεν θεωρούμε ένα topic anchor από μόνο του απόδειξη εξεταστέας ύλης."}</p></div><h4>Σημεία αναφοράς από την επίσημη πηγή</h4><ul>${c.officialAnchors.map(x=>`<li>${esc(x)}</li>`).join("")}</ul>${verificationNote}</div></details>
  ...
  function renderQuiz(quiz){
    ...
    const renderQ=()=>{
      ...
      mount.innerHTML=`<div style="font-size:.76rem;font-weight:800;color:#64748B;margin-bottom:7px">Ερώτηση ${idx+1} από ${quiz.questions.length}</div><h4>${esc(x.q)}</h4><div class="sp-quiz-options">${x.options.map((t,i)=>`<button class="sp-option" type="button" data-i="${i}">${esc(t)}</button>`).join("")}</div><div class="sp-feedback" id="spFeedback" aria-live="polite"></div>`;
    ...
    const answer=(i)=>{
      ...
      const f=document.getElementById("spFeedback");
      const last=idx===quiz.questions.length-1;
      f.innerHTML=`<p style="color:${i===x.correctIndex?"#176B43":"#8B4A4A"}">${i===x.correctIndex?"Σωστά.":"Όχι αυτή τη φορά."}</p><button class="sp-btn" type="button" id="spNext">${last?"Δες αποτέλεσμα":"Επόμενη ερώτηση"}</button>`;
      document.getElementById("spNext").onclick=()=>{if(last)result();else{idx++;locked=false;renderQ();}};
    };
    const result=()=>{
      ...
      mount.innerHTML=`<div class="sp-score">${score}/${quiz.questions.length}</div><p>${esc(message)}</p><button class="sp-btn" type="button" id="spRetry">Ξαναδοκίμασε</button>`;
      document.getElementById("spRetry").onclick=start;
    };
  ...
  sourceIndex.addEventListener("click",e=>{
    ...
    if(b) renderUnit(b.dataset.openUnit);
  });
  renderSourceIndex();
  renderProgress();
})();