(function(){
  "use strict";

  const C=window.SPECIAL_EDUCATION_CURRICULUM;
  if(!C?.entries) throw new Error("SPECIAL_EDUCATION_CURRICULUM must load before annual Special Gymnasium mappings");

  const VERIFIED="2026-09-20";
  const HUB="https://www.minedu.gov.gr/site/70835-15-09-26-enkyklioi-yle-odegies-didaskalias-mathematon-gymnasiou-e-a-e";
  const SOURCES={
    ancientGreek:"https://www.minedu.gov.gr/publications/docs2026/%CE%91%CE%A1%CE%A7%CE%91%CE%99%CE%91_%CE%95%CE%9B%CE%9B_%CE%93%CE%9B%CE%A9%CE%A3%CE%A3%CE%91_%CE%93%CE%A3%CE%99%CE%9F%CE%A5_%CE%95%CE%91%CE%95_2026_27.pdf",
    biologyA:"https://www.minedu.gov.gr/publications/docs2026/%CE%92%CE%99%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91_%CE%91_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95AE_26-27.docx",
    biologyB:"https://www.minedu.gov.gr/publications/docs2026/%CE%92%CE%99%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91_B_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95AE_26-27.docx",
    biologyC:"https://www.minedu.gov.gr/publications/docs2026/%CE%92%CE%99%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91_%CE%93_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95AE_26-27.docx",
    geographyAB:"https://www.minedu.gov.gr/publications/docs2026/%CE%93%CE%95%CE%A9%CE%9B%CE%9F%CE%93%CE%99%CE%91-%CE%93%CE%95%CE%A9%CE%93%CE%A1%CE%91%CE%A6%CE%99%CE%91_%CE%91-%CE%92_%CE%93%CE%A5%CE%9C%CE%9D_EAE_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_26-27.docx",
    religion:"https://www.minedu.gov.gr/publications/docs2026/%CE%98%CE%A1%CE%97%CE%A3%CE%9A%CE%95%CE%A5%CE%A4%CE%99%CE%9A%CE%91_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%95%CE%91%CE%95_26-27.docx",
    ethics:"https://www.minedu.gov.gr/publications/docs2026/%CE%9F%CE%B4%CE%B7%CE%B3%CE%AF%CE%B5%CF%82_%CE%94%CE%B9%CE%B4%CE%B1%CF%83%CE%BA%CE%B1%CE%BB%CE%AF%CE%B1%CF%82_%CE%97%CE%B8%CE%B9%CE%BA%CE%AE%CF%82_%CE%93%CF%85%CE%BC%CE%BD%CE%B1%CF%83%CE%AF%CF%89%CE%BD_%CE%95%CE%91%CE%95_%CE%A0%CF%81%CF%8E%CF%84%CE%BF_%CE%9C%CE%AD%CF%81%CE%BF%CF%82.docx",
    history:"https://www.minedu.gov.gr/publications/docs2026/%CE%99%CE%A3%CE%A4%CE%9F%CE%A1%CE%99%CE%91_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%95.%CE%91.%CE%95._2026-2027.docx",
    informatics:"https://www.minedu.gov.gr/publications/docs2026/%CE%A0%CE%9B%CE%97%CE%A1%CE%9F%CE%A6%CE%9F%CE%A1%CE%99%CE%9A%CE%97_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95%CE%91%CE%95_26-27.docx",
    culture:"https://www.minedu.gov.gr/publications/docs2026/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3_%CE%93%CE%A5%CE%9C%CE%9D_EAE_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3_26-27.docx",
    technology:"https://www.minedu.gov.gr/publications/docs2026/%CE%A4%CE%95%CE%A7%CE%9D%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3_26-27_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95%CE%91%CE%95.docx",
    mathA:"https://www.minedu.gov.gr/publications/docs2026/%CE%9C%CE%91%CE%98%CE%97%CE%9C%CE%91%CE%A4%CE%99%CE%9A%CE%91_%CE%91_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_EAE_%CE%A5%CE%9B%CE%97-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_26-27_%CE%BD%CE%AD%CE%BF_v1.docx",
    mathB:"https://www.minedu.gov.gr/publications/docs2026/%CE%9C%CE%91%CE%98%CE%97%CE%9C%CE%91%CE%A4%CE%99%CE%9A%CE%91_%CE%92_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%95%CE%91%CE%95_%CE%A5%CE%9B%CE%97-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_26-27_%CE%BD%CE%AD%CE%BF_v1.docx",
    mathC:"https://www.minedu.gov.gr/publications/docs2026/%CE%9C%CE%91%CE%98%CE%97%CE%9C%CE%91%CE%A4%CE%99%CE%9A%CE%91_%CE%93_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%95%CE%91%CE%95_%CE%A5%CE%9B%CE%97-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_26-27_%CE%BD%CE%AD%CE%BF_v1.docx",
    physics:"https://www.minedu.gov.gr/publications/docs2026/%CE%A6%CE%A5%CE%A3%CE%99%CE%9A%CE%97_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F_%CE%95%CE%91%CE%95_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_26-27.docx",
    pe:"https://www.minedu.gov.gr/publications/docs2026/%CE%A6%CE%A5%CE%A3%CE%99%CE%9A%CE%97_%CE%91%CE%93%CE%A9%CE%93%CE%97_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95.%CE%91.%CE%95._2026-2027.docx",
    chemistryB:"https://www.minedu.gov.gr/publications/docs2026/%CE%A7%CE%97%CE%9C%CE%95%CE%99%CE%91_%CE%92_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%95.%CE%91.%CE%95.__26-27_.docx",
    chemistryC:"https://www.minedu.gov.gr/publications/docs2026/%CE%A7%CE%97%CE%9C%CE%95%CE%99%CE%91_%CE%93_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%95.%CE%91.%CE%95._26-27_.docx",
    civics:"https://www.minedu.gov.gr/publications/docs2026/%CE%9A%CE%A0%CE%91_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%95.%CE%91.%CE%95._26-27.docx",
    english:"https://www.minedu.gov.gr/publications/docs2026/%CE%9E%CE%95%CE%9D%CE%95%CE%A3_%CE%93%CE%9B%CE%A9%CE%A3%CE%A3%CE%95%CE%A3_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%A9%CE%9D_EAE_%CE%91%CE%93%CE%93%CE%9B%CE%99%CE%9A%CE%91.docx",
    greekLanguageLiterature:"https://www.minedu.gov.gr/publications/docs2026/%CE%9D.%CE%95._%CE%9B%CE%9F%CE%93%CE%9F%CE%A4%CE%95%CE%A7%CE%9D%CE%99%CE%91_%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3_%CE%93%CE%A5%CE%9C%CE%9D_%CE%95.%CE%91.%CE%95._26-27.docx",
    economics:"https://www.minedu.gov.gr/publications/docs2026/%CE%9F%CE%99%CE%9A%CE%9F%CE%9D%CE%9F%CE%9C%CE%99%CE%9A%CE%91_%CE%93_%CE%93%CE%A5%CE%9C%CE%9D%CE%91%CE%A3%CE%99%CE%9F%CE%A5_%CE%95%CE%91%CE%95_2026-2027.pdf"
  };


  const PUBLISHED_PENDING=Object.freeze([
    {grade:"A",subjectId:"religion",label:"Θρησκευτικά / Ηθική",sourceTitle:"ΘΡΗΣΚΕΥΤΙΚΑ ΓΥΜΝΑΣΙΟΥ ΟΔΗΓΙΕΣ ΕΑΕ 26-27 + Οδηγίες Ηθικής",sourceUrl:SOURCES.religion,alternateSourceUrl:SOURCES.ethics},
    {grade:"B",subjectId:"religion",label:"Θρησκευτικά / Ηθική",sourceTitle:"ΘΡΗΣΚΕΥΤΙΚΑ ΓΥΜΝΑΣΙΟΥ ΟΔΗΓΙΕΣ ΕΑΕ 26-27 + Οδηγίες Ηθικής",sourceUrl:SOURCES.religion,alternateSourceUrl:SOURCES.ethics},
    {grade:"C",subjectId:"religion",label:"Θρησκευτικά / Ηθική",sourceTitle:"ΘΡΗΣΚΕΥΤΙΚΑ ΓΥΜΝΑΣΙΟΥ ΟΔΗΓΙΕΣ ΕΑΕ 26-27 + Οδηγίες Ηθικής",sourceUrl:SOURCES.religion,alternateSourceUrl:SOURCES.ethics},
  ]);

  const GRADE_LABEL={A:"Α΄ Γυμνασίου",B:"Β΄ Γυμνασίου",C:"Γ΄ Γυμνασίου"};
  const pref=(label,arr)=>(arr||[]).map(x=>`${label} — ${x}`);

  function upsert({
    id,grade,subject,subjectId,sourceTitle,sourceUrl,
    required=[],modified=[],optional=[],official=[],excluded=[],note="",meta={}
  }){
    const selectable=official.length
      ? official
      : [...required,...pref("Με προσαρμογή",modified),...pref("Προαιρετικό",optional)];

    C.entries[id]={
      id,
      schoolType:"special-gymnasium",
      grade,
      gradeLabel:GRADE_LABEL[grade],
      subject,
      subjectId,
      subjectType:"Επίσημη ετήσια χαρτογράφηση Ε.Α.Ε. 2026-2027 για επιλογή ενότητας",
      status:"verified",
      coverageStatus:"annual-instructions-verified",
      annualInstructionsStatus:"2026-27-verified",
      verificationBasis:"annual-eae-instructions-2026-27",
      verificationDate:VERIFIED,
      sourceTitle,
      sourceUrl,
      instructionSourceUrl:sourceUrl,
      officialHubUrl:HUB,
      officialAnchors:selectable,
      requiredAnchors:[...required],
      modifiedAnchors:[...modified],
      optionalAnchors:[...optional],
      excludedAnchors:[...excluded],
      verificationNote:note || "Οι επιλογές προέρχονται από τις επίσημες οδηγίες Ε.Α.Ε. 2026-2027. Οι εξαιρούμενες ενότητες δεν εκτίθενται ως επιλογές στον AI Βοηθό Εκπαιδευτικού.",
      ...meta
    };
  }





  upsert({
    id:"teacher-annual-special-gym-a-ancient-language",grade:"A",subject:"Αρχαία Ελληνική Γλώσσα",subjectId:"ancient-language",
    sourceTitle:"ΑΡΧΑΙΑ ΕΛΛ ΓΛΩΣΣΑ ΓΣΙΟΥ ΕΑΕ 2026-27",sourceUrl:SOURCES.ancientGreek,
    official:[
      "Ενότητα 1 — Εισαγωγικά στην αρχαία ελληνική γλώσσα",
      "Ενότητα 2 — Κείμενο και φθόγγοι/γράμματα",
      "Ενότητα 3 — Τονισμός, πνεύματα και βασικά μορφολογικά στοιχεία",
      "Ενότητα 4 + Ενότητα 12 (μόνο Γ2) — άρθρο, β΄ κλίση και είδη προτάσεων",
      "Ενότητα 5 + στοιχεία Ενοτήτων 3 και 4 — ρήμα, χρόνοι και προσωπικές αντωνυμίες",
      "Ενότητα 6 — α΄ κλίση ουσιαστικών",
      "Ενότητα 8 — δευτερόκλιτα επίθετα και δεικτική αντωνυμία",
      "Ενότητα 7 — αύξηση και ιστορικοί χρόνοι ενεργητικής φωνής",
      "Ενότητα 9 — παρακείμενος και υπερσυντέλικος ενεργητικής φωνής",
      "Ενότητα 11 — απαρέμφατο και μετοχή",
      "Ενότητα 13 — συντακτική λειτουργία απαρεμφάτου και μετοχής",
      "Προαιρετικό — Ενότητα 10"
    ],
    optional:["Ενότητα 10"],
    excluded:[
      "Ενότητα 12 — το κείμενο της ενότητας (διδάσκεται μόνο το Γ2 μαζί με την Ενότητα 4)",
      "Ενότητες 14-18 — να μη διδαχθούν"
    ],
    note:"Η χαρτογράφηση ακολουθεί το επίσημο πρόγραμμα Αρχαίας Ελληνικής Γλώσσας Α΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Η Ενότητα 10 αναφέρεται ρητά ως προαιρετική, το κείμενο της Ενότητας 12 δεν διδάσκεται και οι Ενότητες 14-18 εξαιρούνται. Η σειρά 8→7 διατηρείται όπως προτείνεται στην επίσημη οδηγία.",
    meta:{weeklyHours:2,plannedHours:43,coverageCompleteness:"section-level-verified",anchorPolicy:"official-annual-sequence-with-explicit-exclusions"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-ancient-language",grade:"B",subject:"Αρχαία Ελληνική Γλώσσα",subjectId:"ancient-language",
    sourceTitle:"ΑΡΧΑΙΑ ΕΛΛ ΓΛΩΣΣΑ ΓΣΙΟΥ ΕΑΕ 2026-27",sourceUrl:SOURCES.ancientGreek,
    official:[
      "Ενότητα 2 — γ΄ κλίση ουσιαστικών και μέση φωνή",
      "Ενότητα 3 — γ΄ κλίση, μετοχές και χρόνοι μέσης φωνής",
      "Ενότητα 4 — τριτόκλιτα επίθετα",
      "Ενότητα 5 + υποενότητα Γ2 της Ενότητας 4 — υποτακτική ενεργητικής φωνής",
      "Ενότητα 6 — τριτόκλιτα επίθετα και ερωτηματική/αόριστη αντωνυμία",
      "Ενότητα 7 — αναφορικές αντωνυμίες (χωρίς ευκτική)",
      "Ενότητα 8 — χρήση παράλληλου κειμένου· το κύριο κείμενο της ενότητας δεν διδάσκεται",
      "Ενότητα 9 — μετοχή και είδη μετοχής",
      "Ενότητα 11 — σύνθεση και προστακτική ενεργητικής φωνής",
      "Ενότητα 12 — προστακτική και γραμματικά φαινόμενα",
      "Ενότητα 13 — υποτακτική μέσης φωνής",
      "Ενότητα 16 — προστακτική μέσης φωνής"
    ],
    excluded:[
      "Ενότητα 1 — να μη διδαχθεί",
      "Ενότητα 8 — το κύριο κείμενο της ενότητας (αξιοποιείται το παράλληλο κείμενο)",
      "Ενότητα 10 — να μη διδαχθεί",
      "Ενότητες 14-15 — να μη διδαχθούν",
      "Ενότητες 17-18 — να μη διδαχθούν",
      "Ενότητα 7 — ευκτική ενεστώτα, μέλλοντα και αορίστου ενεργητικής φωνής",
      "Ενότητα 13 — παρασύνθετα",
      "Ενότητα 16 — ετυμολογικά, κτητικές αντωνυμίες και ετερόπτωτοι ονοματικοί προσδιορισμοί"
    ],
    note:"Η εγγραφή αποτυπώνει τις πραγματικές ενότητες του επίσημου προγράμματος Β΄ Γυμνασίου Ε.Α.Ε. 2026-2027 και κρατά χωριστά τις ρητές περικοπές. Η Ενότητα 8 παραμένει επιλογή μόνο επειδή η οδηγία αξιοποιεί το παράλληλο κείμενό της, ενώ το κύριο κείμενο δεν διδάσκεται.",
    meta:{weeklyHours:2,plannedHours:44,coverageCompleteness:"section-level-verified",anchorPolicy:"official-annual-sequence-with-explicit-exclusions"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-ancient-language",grade:"C",subject:"Αρχαία Ελληνική Γλώσσα",subjectId:"ancient-language",
    sourceTitle:"ΑΡΧΑΙΑ ΕΛΛ ΓΛΩΣΣΑ ΓΣΙΟΥ ΕΑΕ 2026-27",sourceUrl:SOURCES.ancientGreek,
    official:[
      "Προαιρετικό — Ενότητα 1 (επανάληψη/εμπέδωση ανάλογα με τα μαθησιακά κενά)",
      "Ενότητα 2 — λεξιλόγιο, παραθετικά και σύνταξη",
      "Ενότητα 4 — ευκτική και επιρρηματικοί προσδιορισμοί",
      "Ενότητα 9 — μόριο ἄν, σύνδεση προτάσεων και δευτερεύουσες ονοματικές προτάσεις",
      "Ενότητα 10 — δευτερεύουσες επιρρηματικές προτάσεις (α΄ μέρος)",
      "Ενότητα 11 — δευτερεύουσες επιρρηματικές προτάσεις (β΄ μέρος)",
      "Ενότητα 6 — αόριστος β΄ και επιλεγμένα μορφολογικά φαινόμενα",
      "Ενότητα 8 — παθητικοί χρόνοι, ποιητικό αίτιο και παθητική σύνταξη",
      "Αρριανού Αλεξάνδρου Ανάβαση — Εισαγωγή I, II, III, VIII και V στη γενική θεώρηση",
      "Αρριανού Αλεξάνδρου Ανάβαση — Α΄ Βιβλίο, Κεφ. 13, 3-7: Η μάχη του Γρανικού, διάσταση απόψεων Παρμενίωνα και Αλεξάνδρου",
      "Αρριανού Αλεξάνδρου Ανάβαση — Α΄ Βιβλίο, Κεφ. 15, 3-8: δεύτερη φάση της μάχης - γενναιότητα Αλεξάνδρου",
      "Αρριανού Αλεξάνδρου Ανάβαση — Α΄ Βιβλίο, Κεφ. 16, 4-7: τα μετά τη μάχη",
      "Αρριανού Αλεξάνδρου Ανάβαση — Ζ΄ Βιβλίο, Κεφ. 28-30: χαρακτηρισμός του Αλεξάνδρου",
      "Αρριανού Αλεξάνδρου Ανάβαση — Γενική θεώρηση"
    ],
    optional:["Ενότητα 1"],
    excluded:[
      "Ενότητα 3 — να μη διδαχθεί",
      "Ενότητα 5 — να μη διδαχθεί",
      "Ενότητα 7 — να μη διδαχθεί",
      "Ενότητα 12 — να μη διδαχθεί",
      "Ενότητα 4 — το Β΄ μέρος ως έχει και η β΄ τάξη συνηρημένων ρημάτων",
      "Ενότητες 9-11 — το Β΄ μέρος ως έχει και η εκφορά των δευτερευουσών προτάσεων",
      "Ενότητα 8 — το Β΄ μέρος ως έχει και οι αυτοπαθητικές αντωνυμίες",
      "Αρριανού Ανάβαση — Εισαγωγή IV, VI και VII",
      "Αρριανού Ανάβαση — Β΄ Βιβλίο, Κεφ. 4, 9-11 και Κεφ. 12, 3-8",
      "Αρριανού Ανάβαση — Ε΄ Βιβλίο, Κεφ. 18, 4-6 και Κεφ. 19, 1-3",
      "Αρριανού Ανάβαση — Ζ΄ Βιβλίο, Κεφ. 26, 1-3"
    ],
    note:"Η Γ΄ τάξη χωρίζεται σε Αρχαία Ελληνική Γλώσσα (Σεπτέμβριος-Μάρτιος) και Αρριανού Αλεξάνδρου Ανάβαση (Μάρτιος-Μάιος). Η Ενότητα 1 είναι ρητά προαιρετική και οι Ενότητες 3, 5, 7 και 12 δεν διδάσκονται. Τα επιλεγμένα και εξαιρούμενα κεφάλαια του Αρριανού καταγράφονται χωριστά για να μην προκύπτουν ψευδείς επιλογές.",
    meta:{weeklyHours:2,languagePlannedHours:30,arrianPlannedHours:14,coverageCompleteness:"section-level-verified",anchorPolicy:"official-annual-sequence-plus-arrian"}
  });



  upsert({
    id:"teacher-annual-special-gym-a-ancient-translation",grade:"A",subject:"Αρχαία Ελληνικά από Μετάφραση",subjectId:"ancient-translation",
    sourceTitle:"ΑΡΧΑΙΑ ΕΛΛ ΓΛΩΣΣΑ ΓΣΙΟΥ ΕΑΕ 2026-27",sourceUrl:SOURCES.ancientGreek,
    official:[
      "Οδύσσεια — Εισαγωγή και Προοίμιο: 1–4 και 6–8 (τα 9–10 κατά κρίση εκπαιδευτικού)",
      "Οδύσσεια — α 1–25",
      "Οδύσσεια — α 26–108",
      "Οδύσσεια — α 109–360",
      "Οδύσσεια — α 361–497",
      "Οδύσσεια — β, γ, δ: περιληπτική αναδιήγηση",
      "Οδύσσεια — ε 1–164",
      "Οδύσσεια — ε 165–310 (αναλυτικά 165–251, ανάγνωση 252–310)",
      "Οδύσσεια — ε 311–420",
      "Οδύσσεια — ε 421–552",
      "Οδύσσεια — ζ 139–259",
      "Οδύσσεια — η, θ, ι: περιληπτική αναδιήγηση",
      "Οδύσσεια — θ 550–688",
      "Οδύσσεια — ι 240–512",
      "Οδύσσεια — ι 513–630",
      "Οδύσσεια — κ, λ: περίληψη και λ 376–433, 522–604",
      "Οδύσσεια — μ, ν, ξ, ο, π: περίληψη και π 1–172",
      "Οδύσσεια — π 185–336",
      "Οδύσσεια — ρ, σ, τ, υ: περίληψη και ρ 331–376, τ 112–198",
      "Οδύσσεια — φ, χ: περίληψη και χ 1–446",
      "Οδύσσεια — ψ 89–380",
      "Οδύσσεια — ω: περιληπτική αναδιήγηση",
      "Οδύσσεια — Γενική ανακεφαλαίωση",
      "Ηρόδοτος — Εισαγωγή: ζωή και έργο, εποχή, λογογράφοι, μέθοδος και χρονολογικό σύστημα",
      "Ηρόδοτος, Βιβλίο 1 — Ενότητα 1: Προοίμιο",
      "Ηρόδοτος, Βιβλίο 1 — Ενότητα 2: Κροίσος και Σόλων",
      "Ηρόδοτος, Βιβλίο 1 — Ενότητα 3: Κροίσος και Κύρος",
      "Ηρόδοτος, Βιβλίο 3 — Ενότητα 8: Τα πολιτεύματα",
      "Ηρόδοτος, Βιβλίο 6 — Ενότητα 11: Η μάχη του Μαραθώνα",
      "Ηρόδοτος, Βιβλίο 7 — Ενότητα 12: Διάλογος Ξέρξη και Δημάρατου",
      "Ηρόδοτος, Βιβλίο 7 — Ενότητα 13: Η μάχη των Θερμοπυλών",
      "Ηρόδοτος, Βιβλίο 8 — Ενότητα 15: Η ναυμαχία της Σαλαμίνας",
      "Ηρόδοτος — Γενική ανακεφαλαίωση"
    ],
    note:"Η Α΄ Γυμνασίου Ε.Α.Ε. διδάσκει Οδύσσεια από Σεπτέμβριο έως Φεβρουάριο και Ηρόδοτο από Μάρτιο έως Μάιο. Οι επιλογές διατηρούν τις επίσημες διακρίσεις αναλυτικής διδασκαλίας, απλής ανάγνωσης και περιληπτικής αναδιήγησης χωρίς να μετατρέπονται σε τεχνητά κεφάλαια.",
    meta:{weeklyHours:2,odysseyPlannedHours:36,herodotusPlannedHours:13,coverageCompleteness:"section-level-verified",anchorPolicy:"official-annual-text-selections"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-ancient-translation",grade:"B",subject:"Αρχαία Ελληνικά από Μετάφραση",subjectId:"ancient-translation",
    sourceTitle:"ΑΡΧΑΙΑ ΕΛΛ ΓΛΩΣΣΑ ΓΣΙΟΥ ΕΑΕ 2026-27",sourceUrl:SOURCES.ancientGreek,
    official:[
      "Ιλιάδα — Εισαγωγή (σ. 8–11)",
      "Ιλιάδα — Α 1–53",
      "Ιλιάδα — Α 54–306",
      "Ιλιάδα — Α 307–431α",
      "Ιλιάδα — Α 431β–612 (ανάγνωση 431β–493, αναλυτικά 494–612)",
      "Ιλιάδα — Β, Γ: περίληψη και Γ 121–244",
      "Ιλιάδα — Δ, Ε, Ζ: περίληψη και Ζ 369–529",
      "Ιλιάδα — Η, Θ, Ι: περίληψη και Ι 225–431",
      "Ιλιάδα — Κ, Λ, Μ, Ν, Ξ, Ο: περιληπτική αναδιήγηση",
      "Ιλιάδα — Π: περίληψη, Π 1–100 ανάγνωση και Π 684–867 αναλυτικά",
      "Ιλιάδα — Ρ, Σ: περίληψη και Σ 478–616",
      "Ιλιάδα — Τ 1–152: περίληψη/ανάγνωση",
      "Ιλιάδα — Υ, Φ, Χ: περίληψη και Χ 247–394 αναλυτικά",
      "Ιλιάδα — Ψ, Ω: περίληψη και Ω 468–677 αναλυτικά",
      "Ιλιάδα — Γενική ανακεφαλαίωση",
      "Αρχαία Ελλάδα, ο τόπος και οι άνθρωποι — 1. Αθήνα (επιλογή τουλάχιστον δύο κειμένων)",
      "Αρχαία Ελλάδα, ο τόπος και οι άνθρωποι — 2. Σπάρτη",
      "Αρχαία Ελλάδα, ο τόπος και οι άνθρωποι — 3. Μακεδονία",
      "Αρχαία Ελλάδα, ο τόπος και οι άνθρωποι — 4. Αλεξάνδρεια",
      "Αρχαία Ελλάδα, ο τόπος και οι άνθρωποι — 5. Αθλητισμός και αθλήματα"
    ],
    note:"Η Β΄ Γυμνασίου Ε.Α.Ε. διδάσκει Ιλιάδα από Σεπτέμβριο έως Φεβρουάριο και την ενότητα «Αρχαία Ελλάδα, ο τόπος και οι άνθρωποι» από Μάρτιο έως Μάιο. Στην ενότητα «Αθήνα» οι οδηγίες ζητούν επιλογή τουλάχιστον δύο κειμένων κατά την κρίση του/της εκπαιδευτικού.",
    meta:{weeklyHours:2,iliadPlannedHours:35,ancientGreecePlannedHours:13,coverageCompleteness:"section-level-verified",anchorPolicy:"official-annual-text-selections"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-ancient-translation",grade:"C",subject:"Αρχαία Ελληνικά από Μετάφραση",subjectId:"ancient-translation",
    sourceTitle:"ΑΡΧΑΙΑ ΕΛΛ ΓΛΩΣΣΑ ΓΣΙΟΥ ΕΑΕ 2026-27",sourceUrl:SOURCES.ancientGreek,
    official:[
      "Ευριπίδη, Ελένη — Εισαγωγή στη δραματική ποίηση και την τραγωδία",
      "Ευριπίδη, Ελένη — Πρόλογος 1–191 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Πάροδος 192–436 (περιληπτικά)",
      "Ευριπίδη, Ελένη — Α΄ Επεισόδιο 437–575 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Επιπάροδος / Β΄ Επεισόδιο 576–730 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Β΄ Επεισόδιο 731–941 (περιληπτικά)",
      "Ευριπίδη, Ελένη — Β΄ Επεισόδιο 942–1139 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Β΄ Επεισόδιο 1140–1219 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Α΄ Στάσιμο 1220–1285 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Γ΄ Επεισόδιο 1286–1424 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Β΄ Στάσιμο 1425–1499 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Δ΄ Επεισόδιο 1500–1592 (περιληπτικά)",
      "Ευριπίδη, Ελένη — Γ΄ Στάσιμο 1593–1652 (περιληπτικά)",
      "Ευριπίδη, Ελένη — Έξοδος 1653–1778 (περιληπτικά)",
      "Ευριπίδη, Ελένη — Έξοδος 1779–1870 (αναλυτικά)",
      "Ευριπίδη, Ελένη — Γενική ανακεφαλαίωση / δραστηριότητες",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Γενική εισαγωγή: Φιλοσοφία και Ιστορία της Αρχαίας Ελληνικής Φιλοσοφίας",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 1: Προσωκρατικοί, εισαγωγή + Ενότητα 3 «Αρετές και ευδαιμονία»",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 2: Σοφιστές, εισαγωγή",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 2, Ενότητα 4: Δικαιοσύνη, φύση-νόμος και πολιτική οργάνωση",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 3: Σωκράτης και Πλάτων, εισαγωγή",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 3, Ενότητα 4: Πολιτική και πολιτικοί",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 4: Αριστοτέλης, εισαγωγή",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 4, Ενότητα 3: Η φιλία είναι απολύτως αναγκαία για τη ζωή",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 4, Ενότητα 5: Ο άνθρωπος είναι «φύσει πολιτικόν ζῷον»",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 5: Κυνική φιλοσοφία και ελληνιστικές/ρωμαϊκές φιλοσοφίες, εισαγωγή",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Κεφάλαιο 5, Ενότητα 3: Στωική ηθική - απάθεια και αταραξία",
      "Ανθολόγιο Φιλοσοφικών Κειμένων — Γενική ανακεφαλαίωση"
    ],
    note:"Η Γ΄ Γυμνασίου Ε.Α.Ε. διδάσκει την «Ελένη» του Ευριπίδη από Σεπτέμβριο έως το τέλος Φεβρουαρίου και το Ανθολόγιο Φιλοσοφικών Κειμένων από Μάρτιο έως Μάιο. Οι χαρακτηρισμοί «αναλυτικά» και «περιληπτικά» διατηρούνται όπως στην επίσημη οδηγία.",
    meta:{weeklyHours:2,helenPlannedHours:35,philosophyPlannedHours:13,coverageCompleteness:"section-level-verified",anchorPolicy:"official-annual-text-selections"}
  });







  upsert({
    id:"teacher-annual-special-gym-a-music",grade:"A",subject:"Μουσική",subjectId:"music",
    sourceTitle:"ΠΟΛΙΤΙΣΜΟΣ ΓΥΜΝ EAE ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27",sourceUrl:SOURCES.culture,
    required:[
      "Α1. Διάλογοι στην ήπειρο του ρυθμού",
      "Α2. Μουσικοί διάλογοι και Αντίφωνα",
      "Α3. Ανακαλύπτω τη φωνή",
      "Α4. Τα χρώματα των ήχων",
      "Β1β. Επιμένω μουσικά — μόνο η έννοια και το μουσικό φαινόμενο του κανόνα",
      "Β2α+Β2β. Μουσικές σε πέντε τόνους",
      "Β3. Ανεβαίνω τη σκάλα — μόνο η έννοια της σκάλας",
      "Δ4. Μουσική σε κύκλο — φόρμα ροντό",
      "ΣΤ1α+ΣΤ1β+ΣΤ2. Καλαματιανός, Τσάμικος, Βαλς — περιληπτικά",
      "Η1α+Η1γ. Το νερό"
    ],
    excluded:[
      "Β1α. Επιμένω μουσικά",
      "Γ΄ Ενότητα — Ας κάνουμε μια επανάληψη (1)",
      "Δ1. Μουσική σε δύο χρόνους","Δ2. Α, Β στη σειρά","Δ3. Τί-κε-τα-κε",
      "Ε1α. Ταξίδι στο παρελθόν 1","Ε1β. Ταξίδι στο παρελθόν 2","Ε2. Στη χώρα του παραμυθιού",
      "Ζ΄ Ενότητα — Ας κάνουμε μια επανάληψη (2)",
      "Η1β. Το νερό - η θάλασσα","Η1δ. Το νερό - Τελική Παρουσίαση"
    ],
    note:"Ακριβής χαρτογράφηση Μουσικής Α΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Οι ρητά εξαιρούμενες υποενότητες δεν εμφανίζονται ως επιλογές. Στις συμπτυγμένες ενότητες διατηρείται ο περιορισμός της επίσημης οδηγίας.",
    meta:{weeklyHours:1,coverageCompleteness:"section-level-verified",classificationStatus:"official-required-excluded-preserved",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-music",grade:"B",subject:"Μουσική",subjectId:"music",
    sourceTitle:"ΠΟΛΙΤΙΣΜΟΣ ΓΥΜΝ EAE ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27",sourceUrl:SOURCES.culture,
    required:[
      "1. Ένας κόσμος μετά μουσικής",
      "2. Σε ρυθμούς… αφρικάνικους",
      "3. Μελωδικοί εξωτικοί αυτοσχεδιασμοί — βασικός κορμός",
      "4. Μουσική για… χορό — με τις επίσημες περικοπές",
      "5. Ευρωπαϊκές προδιαγραφές — βασικός κορμός",
      "6. Ο γύρος της Ελλάδας"
    ],
    optional:[
      "3. Μπαλινέζικο γκαμελάν","3. Η πολυσύνθετη μουσική της Κίνας","3. Η χώρα του Ανατέλλοντος Ηλίου",
      "5. Μανουέλ ντε Φάλλα, Ισαάκ Αλμπένιθ, Τραγούδι (σ. 59) και Μουσικά όργανα (σ. 60)",
      "5. Η γέννηση της όπερας στην Ιταλία — περιληπτικά, χωρίς έμφαση στα ονόματα"
    ],
    excluded:[
      "4. Η μουσική στη Λατινική Αμερική — σελ. 34",
      "4. Η μουσική κατά την προκολομβιανή περίοδο",
      "4. Τα μουσικά όργανα των ιθαγενών",
      "4. Τα μουσικά όργανα μας μπατερία"
    ],
    note:"Η Β΄ Γυμνασίου Ε.Α.Ε. διατηρεί έξι βασικά κεφάλαια, με ρητές προαιρετικές και εξαιρούμενες υποενότητες. Το κεφάλαιο «Ευρωπαϊκές προδιαγραφές» προβλέπεται να διδαχθεί τελευταίο.",
    meta:{weeklyHours:1,plannedHours:25,coverageCompleteness:"section-level-verified",classificationStatus:"official-required-optional-excluded-preserved",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-music",grade:"C",subject:"Μουσική",subjectId:"music",
    sourceTitle:"ΠΟΛΙΤΙΣΜΟΣ ΓΥΜΝ EAE ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27",sourceUrl:SOURCES.culture,
    required:[
      "1. Το ταξίδι της μουσικής στον 20ό αιώνα",
      "2. Εικόνα και Ήχος",
      "3. Από το γκόσπελ στο ροκ",
      "4. Ελληνικές μουσικές ιστορίες — βασικός κορμός",
      "5. Στις γειτονιές του ελληνικού τραγουδιού — βασικός κορμός"
    ],
    optional:[
      "4. Ελληνική Τζαζ",
      "5. Η επιστροφή του Οδυσσέα του Γιάννη Μαρκόπουλου",
      "5. Η γειτονιά του Σταύρου Ξαρχάκου",
      "5. Ο δρόμος του Μάνου Λοΐζου",
      "5. Για τη μικρή Ελλάδα του Διονύση Σαββόπουλου",
      "5. Η επιστροφή στις ρίζες του Γιάννη Μαρκόπουλου",
      "6. Είμαστε στον αέρα!",
      "6. Πολύτεχνο υπερθέαμα"
    ],
    note:"Η Γ΄ Γυμνασίου Ε.Α.Ε. κρατά τον βασικό κορμό των πέντε κεφαλαίων και χωριστά τις ρητά προαιρετικές υποενότητες/σχέδια εργασίας. Οι ίδιες οι αναλυτικές προτάσεις τρόπου διδασκαλίας επισημαίνονται ως προαιρετικές.",
    meta:{weeklyHours:1,coverageCompleteness:"section-level-verified",classificationStatus:"official-core-optional-preserved",teachingGuidanceOptional:true,mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-a-art",grade:"A",subject:"Καλλιτεχνικά",subjectId:"art",
    sourceTitle:"ΠΟΛΙΤΙΣΜΟΣ ΓΥΜΝ EAE ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27",sourceUrl:SOURCES.culture,
    official:[
      "1ο Κεφάλαιο — Από την κίνηση στην έκφραση",
      "2ο Κεφάλαιο — Όλα είναι χρώμα",
      "3ο Κεφάλαιο — Οργανώνω, σύνθεση και αισθητική",
      "4ο Κεφάλαιο — Σχέδιο για όλους, μορφικά στοιχεία, μορφοποίηση",
      "5ο Κεφάλαιο — Η δική μου πραγματικότητα, σχέδιο από τη φύση, κοινωνικές επιδράσεις στην Τέχνη",
      "6ο Κεφάλαιο — Κινούμενο σχέδιο, εξέλιξη και λειτουργία",
      "7ο Κεφάλαιο — Πηλός, πλαστική, Αρχαίοι Πολιτισμοί",
      "8ο Κεφάλαιο — Χαρακτική, μορφή και Περιεχόμενο",
      "9ο Κεφάλαιο — Η ζωή και οι τέχνες",
      "10ο Κεφάλαιο — Ένα πρωινό στον Παρθενώνα",
      "11ο Κεφάλαιο — Στους θησαυρούς του Μουσείου"
    ],
    note:"Οι οδηγίες Καλλιτεχνικών είναι ανοικτού τύπου και ο/η εικαστικός επιλέγει και διαφοροποιεί ενότητες, δραστηριότητες και βάθος με βάση την τάξη. Το μενού λειτουργεί ως επαληθευμένο πλαίσιο πλοήγησης, όχι ως δεσμευτική εξεταστέα σειρά. Στη χαρακτική, η χάραξη φύλλων χαλκού αναφέρεται ως προαιρετική και απαιτεί ιδιαίτερη προσοχή.",
    meta:{weeklyHours:1,selectionFramework:true,selectionStatus:"open-teacher-designed-art-framework",anchorPolicy:"official-open-art-guidance",coverageCompleteness:"chapter-framework-verified",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-art",grade:"B",subject:"Καλλιτεχνικά",subjectId:"art",
    sourceTitle:"ΠΟΛΙΤΙΣΜΟΣ ΓΥΜΝ EAE ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27",sourceUrl:SOURCES.culture,
    official:[
      "1η Διδακτική Ενότητα — Υλικά, Τεχνικές, Πειραματισμοί",
      "2η Διδακτική Ενότητα — Εφαρμοσμένες τέχνες",
      "3η Διδακτική Ενότητα — Μορφικά Στοιχεία",
      "4η Διδακτική Ενότητα — Ανάλυση έργου – Ιστορία Τέχνης"
    ],
    note:"Ανοικτό, μη δεσμευτικό πλαίσιο Καλλιτεχνικών Β΄ Ε.Α.Ε. Ο/Η εκπαιδευτικός επιλέγει επίπεδο εμβάθυνσης και εφαρμογές με βάση υλικά, ασφάλεια και ανάγκες της τάξης· η ξυλογλυπτική/σκάλισμα προτείνεται μόνο εφόσον οι συνθήκες το επιτρέπουν.",
    meta:{weeklyHours:1,selectionFramework:true,selectionStatus:"open-teacher-designed-art-framework",anchorPolicy:"official-open-art-guidance",coverageCompleteness:"unit-framework-verified",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-art",grade:"C",subject:"Καλλιτεχνικά",subjectId:"art",
    sourceTitle:"ΠΟΛΙΤΙΣΜΟΣ ΓΥΜΝ EAE ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27",sourceUrl:SOURCES.culture,
    official:[
      "1η Διδακτική Ενότητα — Μορφικά στοιχεία",
      "2η Διδακτική Ενότητα — Εφαρμοσμένες Τέχνες",
      "3η Διδακτική Ενότητα — Νέες τεχνολογίες",
      "4η Διδακτική Ενότητα — Σύγχρονες μορφές εικαστικών τεχνών"
    ],
    note:"Ανοικτό, μη δεσμευτικό πλαίσιο Καλλιτεχνικών Γ΄ Ε.Α.Ε. Η 2η ενότητα προτείνεται να ενσωματώνεται σε άλλες ενότητες, ενώ στη 3η το βίντεο/ΗΥ μπορεί επίσης να ενσωματώνεται σε άλλες δραστηριότητες. Οι επιλογές είναι πλαίσιο σχεδιασμού και όχι σταθερή εξεταστέα ύλη.",
    meta:{weeklyHours:1,selectionFramework:true,selectionStatus:"open-teacher-designed-art-framework",anchorPolicy:"official-open-art-guidance",coverageCompleteness:"unit-framework-verified",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9F%CE%9B%CE%99%CE%A4%CE%99%CE%A3%CE%9C%CE%9F%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%94%CE%99%CE%94%CE%91%CE%A3%CE%9A%CE%91%CE%9B%CE%99%CE%91%CE%A3-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-a-informatics",grade:"A",subject:"Πληροφορική",subjectId:"informatics",
    sourceTitle:"ΠΛΗΡΟΦΟΡΙΚΗ ΟΔΗΓΙΕΣ ΓΥΜΝ ΕΑΕ 26-27",sourceUrl:SOURCES.informatics,
    required:["1. Ψηφιακός Κόσμος","3. Το εσωτερικό του υπολογιστή","4. Λογισμικό","5. Γνωρίζω το Διαδίκτυο και Επικοινωνώ","6. Κυβερνοασφάλεια","8. Αλγοριθμική","10. Πληροφορική και Κοινωνία"],
    optional:["2. Το υλικό του υπολογιστή","7. Δημιουργώ με τον Κειμενογράφο","9. Προγραμματισμός Υπολογιστικών Συστημάτων"],
    note:"Η σειρά και οι ώρες είναι ενδεικτικές και προσαρμόζονται στις ανάγκες της τάξης. Οι ενότητες 2, 7 και 9 είναι προαιρετικές. Οι επίσημες οδηγίες προτείνουν στην Α΄ Γυμνασίου να αποφεύγεται η χρήση εργαλείων Τεχνητής Νοημοσύνης από τους μαθητές.",
    meta:{plannedHours:52,sequenceFlexible:true,spiralApproachRecommended:true,projectAndProblemSolvingRecommended:true,aiClassroomPolicy:"avoid-direct-student-ai-use-in-a-grade",noStudentPersonalDataInPrompts:true,personalStudentAccountsAtSchool:false,coverageCompleteness:"section-level-verified",classificationStatus:"official-required-optional-preserved",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9B%CE%97%CE%A1%CE%9F%CE%A6%CE%9F%CE%A1%CE%99%CE%9A%CE%97-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-%CE%95%CE%91%CE%95-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-informatics",grade:"B",subject:"Πληροφορική",subjectId:"informatics",
    sourceTitle:"ΠΛΗΡΟΦΟΡΙΚΗ ΟΔΗΓΙΕΣ ΓΥΜΝ ΕΑΕ 26-27",sourceUrl:SOURCES.informatics,
    required:["1. Ψηφιακός Κόσμος","2. Δίκτυα Υπολογιστών","3. Ανάλυση Δεδομένων με Υπολογιστικά Φύλλα","6. Δομές Δεδομένων","7. Τεχνητή Νοημοσύνη","8. Πληροφορική και Κοινωνία"],
    optional:["4. Παρουσιάσεις","5. Τμηματικός Προγραμματισμός"],
    note:"Η Β΄ Γυμνασίου Ε.Α.Ε. περιλαμβάνει εισαγωγική ενότητα Τεχνητής Νοημοσύνης. Οι ενότητες 4 και 5 είναι προαιρετικές. Η σειρά/χρονική κατανομή είναι ενδεικτική και επιτρέπεται συνδυασμός ενοτήτων.",
    meta:{plannedHours:25,sequenceFlexible:true,spiralApproachRecommended:true,aiIntroductionUnit:"7. Τεχνητή Νοημοσύνη",aiClassroomPolicy:"introductory-supervised-critical-ai-use",noStudentPersonalDataInPrompts:true,personalStudentAccountsAtSchool:false,studentFirstThenAiCompare:true,coverageCompleteness:"section-level-verified",classificationStatus:"official-required-optional-preserved",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9B%CE%97%CE%A1%CE%9F%CE%A6%CE%9F%CE%A1%CE%99%CE%9A%CE%97-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-%CE%95%CE%91%CE%95-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-informatics",grade:"C",subject:"Πληροφορική",subjectId:"informatics",
    sourceTitle:"ΠΛΗΡΟΦΟΡΙΚΗ ΟΔΗΓΙΕΣ ΓΥΜΝ ΕΑΕ 26-27",sourceUrl:SOURCES.informatics,
    required:["1. Αλγοριθμική","3. Φυσική Υπολογιστική / Ρομποτικές Διατάξεις","5. Συνεργάζομαι σε διαδικτυακά περιβάλλοντα","7. Επιστημονικός Προγραμματισμός"],
    modified:["2. Προγραμματισμός με τη γλώσσα Python — το EduBlocks είναι προαιρετικό· έμφαση σε πραγματικό Python IDE"],
    optional:["4. Τεχνητή Νοημοσύνη","6. Πληροφορική και Κοινωνία — δεν απαιτείται πλήρης κάλυψη"],
    note:"Στη Γ΄ Γυμνασίου Ε.Α.Ε. δίνεται προτεραιότητα στην αλγοριθμική/υπολογιστική σκέψη, στη συστηματική επαφή με Python και στη φυσική υπολογιστική. Το EduBlocks μπορεί να παραλειφθεί υπέρ πραγματικού Python IDE.",
    meta:{plannedHours:25,sequenceFlexible:true,spiralApproachRecommended:true,programmingPriority:"Python",aiClassroomPolicy:"supervised-critical-ai-use-when-selected",noStudentPersonalDataInPrompts:true,personalStudentAccountsAtSchool:false,studentFirstThenAiCompare:true,coverageCompleteness:"section-level-verified",classificationStatus:"official-required-modified-optional-preserved",mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%A0%CE%9B%CE%97%CE%A1%CE%9F%CE%A6%CE%9F%CE%A1%CE%99%CE%9A%CE%97-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-%CE%93%CE%A5%CE%9C%CE%9D-%CE%95%CE%91%CE%95-26-27.pdf"}
  });

  upsert({
    id:"teacher-annual-special-gym-a-history",grade:"A",subject:"Ιστορία",subjectId:"history",
    sourceTitle:"ΙΣΤΟΡΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝΑΣΙΟΥ Ε.Α.Ε. 2026-2027",sourceUrl:SOURCES.history,
    official:[
      "Αναλυτικά — Κεφ. Β΄ 2. Ο Κυκλαδικός πολιτισμός",
      "Αναλυτικά — Κεφ. Β΄ 3-4. Ο Μινωικός πολιτισμός · Θρησκεία και Τέχνη",
      "Αναλυτικά — Κεφ. Β΄ 5-6. Ο Μυκηναϊκός κόσμος · Θρησκεία και Τέχνη",
      "Προαιρετικά — Κεφ. Γ΄ 1. Οι μεταβατικοί χρόνοι",
      "Προαιρετικά — Κεφ. Γ΄ 2. Η πολιτισμική αναγέννηση",
      "Αναλυτικά — Κεφ. Δ΄ 1. Αποικιακή εξάπλωση",
      "Αναλυτικά — Κεφ. Δ΄ 2. Η πόλη-κράτος και η εξέλιξη του πολιτεύματος",
      "Αναλυτικά — Κεφ. Δ΄ 3. Η Σπάρτη",
      "Αναλυτικά — Κεφ. Δ΄ 4-5. Αθήνα: από τη βασιλεία στην αριστοκρατία · πορεία προς τη δημοκρατία",
      "Αναλυτικά — Κεφ. Δ΄ 7-8. Πέρσες και Έλληνες · οριστική απομάκρυνση της περσικής επίθεσης",
      "Αναλυτικά — Κεφ. Δ΄ 10. Η Τέχνη",
      "Αναλυτικά — Κεφ. Ε΄ 1. Η Συμμαχία της Δήλου και η αθηναϊκή ηγεμονία",
      "Αναλυτικά — Κεφ. Ε΄ 2-3. Η δημοκρατία εδραιώνεται · το πολίτευμα και οι λειτουργίες",
      "Αναλυτικά — Κεφ. Ε΄ 4-5. Η αθηναϊκή κοινωνία, καθημερινή ζωή, μόρφωση και εργασία",
      "Αναλυτικά — Κεφ. ΣΤ΄ 1-2. Πελοποννησιακός πόλεμος: αίτια, φάσεις και εξέλιξη",
      "Προαιρετικά/Συνοπτικά — Κεφ. ΣΤ΄ 3-4. Σπαρτιατική ηγεμονία · Θηβαϊκή κυριαρχία",
      "Αναλυτικά — Κεφ. Ζ΄ 2. Το κράτος της Μακεδονίας επεκτείνεται",
      "Συνοπτικά — Κεφ. Ζ΄ 3. Ο Αλέξανδρος κατακτά την Ανατολή",
      "Αναλυτικά — Κεφ. Ζ΄ 4. Το έργο του Αλεξάνδρου",
      "Συνοπτικά — Κεφ. Η΄ 1-3. Ελληνιστικά βασίλεια, ελληνικός κόσμος και ρωμαϊκή ισχύς",
      "Αναλυτικά — Κεφ. Η΄ 5-6. Γράμματα και Τέχνες των Ελληνιστικών χρόνων · Ελληνορωμαϊκός πολιτισμός",
      "Αναλυτικά — Κεφ. Ι΄ 1. Η Ρωμαϊκή Ειρήνη",
      "Συνοπτικά — Κεφ. Ι΄ 3. Η διάδοση του Χριστιανισμού",
      "Αναλυτικά — Κεφ. Ι΄ 4-5. Κρίση του 3ου αιώνα · ο Κωνσταντίνος και η μεγάλη στροφή"
    ],
    note:"Η Α΄ Γυμνασίου Ε.Α.Ε. έχει 37 ώρες. Οι οδηγίες ξεχωρίζουν αναλυτική, συνοπτική και προαιρετική διδασκαλία. Τα labels του μενού διατηρούν αυτό το καθεστώς ώστε η AI να μην αντιμετωπίζει συνοπτικές ή προαιρετικές ενότητες σαν ισότιμα εξεταστέο υλικό.",
    meta:{
      plannedHours:37,
      selectionFramework:true,
      selectionStatus:"analytic-summary-optional-status-preserved",
      anchorPolicy:"official-history-teaching-status",
      coverageCompleteness:"section-status-verified",
      analyticExamPolicy:"Οι ενότητες που διδάσκονται αναλυτικά αποτελούν τον βασικό κορμό· οι συνοπτικές λειτουργούν για ιστορική συνέχεια και οι προαιρετικές παραμένουν διακριτές."
    }
  });

  upsert({
    id:"teacher-annual-special-gym-b-history",grade:"B",subject:"Ιστορία",subjectId:"history",
    sourceTitle:"ΙΣΤΟΡΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝΑΣΙΟΥ Ε.Α.Ε. 2026-2027",sourceUrl:SOURCES.history,
    official:[
      "Θεματική Ι — Πολιτικές εξελίξεις στη Βυζαντινή Αυτοκρατορία",
      "Ι · Από τη Ρώμη στη Νέα Ρώμη",
      "Ι · Ο Ιουστινιανός και το έργο του",
      "Ι · Ο Ηράκλειος και η δυναστεία του",
      "Ι · Η επέκταση των Αράβων",
      "Ι · Συνοπτικά: Η Βυζαντινή Εποποιία — αγώνες με Άραβες και Βουλγάρους",
      "Ι · Ο Καρλομάγνος και η εποχή του",
      "Ι · Συνοπτικά: Η κρίση και οι απώλειες του Βυζαντίου τον 11ο αιώνα",
      "Ι · Συνοπτικά: Οι Κομνηνοί και η μερική αναδιοργάνωση της αυτοκρατορίας",
      "Ι · Οι Σταυροφορίες και η πρώτη άλωση της Πόλης",
      "Ι · Η εξάπλωση των Οθωμανών και η Άλωση της Κωνσταντινούπολης",
      "Θεματική ΙΙ — Οικονομία και Κοινωνία στον Μεσαιωνικό Κόσμο",
      "ΙΙ · Ιουστινιανός: εσωτερική πολιτική, κτίσματα και Αγία Σοφία",
      "ΙΙ · Οικονομία, κοινωνία και πολιτισμός του αραβικού κόσμου",
      "ΙΙ · Οικονομικές συνέπειες των βυζαντινών πολέμων",
      "ΙΙ · Η φεουδαρχία στη Δυτική Ευρώπη",
      "ΙΙ · Συνοπτικά: οικονομία και κοινωνία στην εποχή των Κομνηνών",
      "Θεματική ΙΙΙ — Το Βυζάντιο και η Ανατολική Ευρώπη",
      "ΙΙΙ · Σλάβοι και Βυζάντιο",
      "ΙΙΙ · Συνοπτικά: Βούλγαροι και Βυζάντιο",
      "ΙΙΙ · Εκχριστιανισμός Μοραβών και Βουλγάρων",
      "ΙΙΙ · Ίδρυση, εξέλιξη και εκχριστιανισμός του Ρωσικού κράτους",
      "Θεματική IV — Ανατολική Μεσόγειος και Δυτική Ευρώπη στους Μεσαιωνικούς και Νεότερους Χρόνους",
      "IV · Βυζάντιο και Δύση — Σχίσμα και οικονομική διείσδυση των Βενετών",
      "IV · Η περίοδος της Λατινοκρατίας και τα ελληνικά κράτη",
      "IV · Γεωγραφικές ανακαλύψεις και συνέπειες",
      "IV · Αναγέννηση και Ανθρωπισμός",
      "IV · Συνοπτικά: Η θρησκευτική Μεταρρύθμιση",
      "IV · Ο Ελληνισμός υπό βενετική και οθωμανική κυριαρχία"
    ],
    note:"Οι επίσημες οδηγίες Β΄ Γυμνασίου Ε.Α.Ε. αναδιαρθρώνουν ρητά το βιβλίο σε τέσσερις ευρύτερες θεματικές και προτείνουν συνδιδασκαλία συναφών ενοτήτων. Για αυτό το site δεν επιβάλλει την παλιά γραμμική σειρά του βιβλίου αλλά κρατά τις τέσσερις επίσημες θεματικές και τις επαληθευμένες υποενότητες μέσα σε αυτές.",
    meta:{
      plannedHours:40,
      selectionFramework:true,
      selectionStatus:"official-four-theme-restructured-course",
      anchorPolicy:"official-four-theme-history-framework",
      coverageCompleteness:"thematic-and-core-section-verified",
      officialThemes:[
        "Ι. Πολιτικές εξελίξεις στη Βυζαντινή Αυτοκρατορία",
        "ΙΙ. Οικονομία και Κοινωνία στον Μεσαιωνικό Κόσμο",
        "ΙΙΙ. Το Βυζάντιο και η Ανατολική Ευρώπη",
        "IV. Η Ανατολική Μεσόγειος και η Δυτική Ευρώπη στους Μεσαιωνικούς και Νεότερους Χρόνους"
      ]
    }
  });

  upsert({
    id:"teacher-annual-special-gym-c-history",grade:"C",subject:"Ιστορία",subjectId:"history",
    sourceTitle:"ΙΣΤΟΡΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝΑΣΙΟΥ Ε.Α.Ε. 2026-2027",sourceUrl:SOURCES.history,
    official:[
      "Αναλυτικά — Κεφ. 1, Εν. 1. Η εποχή του Διαφωτισμού",
      "Προαιρετικά — Κεφ. 1, Εν. 2. Η Αμερικανική Επανάσταση",
      "Αναλυτικά — Κεφ. 1, Εν. 3 + Εν. 4 (τρίτη φάση). Η Γαλλική Επανάσταση 1789-1799",
      "Αναλυτικά — Συνέδριο της Βιέννης και μεταναπολεόντεια Ευρώπη",
      "Προαιρετικά — Κεφ. 2, Εν. 6. Τα επαναστατικά κινήματα 1820-1821 στην Ευρώπη",
      "Συνοπτικά — Κεφ. 2, Εν. 5. Ο Ελληνισμός από τα μέσα του 18ου έως τις αρχές του 19ου αιώνα",
      "Αναλυτικά — Κεφ. 2, Εν. 7. Η Φιλική Εταιρεία και η κήρυξη της Επανάστασης στις Παραδουνάβιες Ηγεμονίες",
      "Αναλυτικά — Κεφ. 2, Εν. 8. Η εξέλιξη της Ελληνικής Επανάστασης 1821-1827",
      "Αναλυτικά — Κεφ. 2, Εν. 9. Πρώτες προσπάθειες συγκρότησης ελληνικού κράτους",
      "Συνοπτικά — Κεφ. 2, Εν. 10. Ελληνική Επανάσταση και Ευρώπη",
      "Συνοπτικά — Κεφ. 1 Εν. 1 (Ευρώπη 17ου-18ου αι.) + Κεφ. 3 Εν. 12. Βιομηχανική Επανάσταση",
      "Αναλυτικά — Κεφ. 3, Εν. 13. Κοινωνικές και πολιτικές διαστάσεις της Βιομηχανικής Επανάστασης",
      "Αναλυτικά — Κεφ. 3, Εν. 15. Αποικιοκρατία και αποικιακοί ανταγωνισμοί",
      "Αναλυτικά — Κεφ. 4, Εν. 17. Ο Ι. Καποδίστριας ως κυβερνήτης της Ελλάδας",
      "Προαιρετικά — Κεφ. 4, Εν. 18. Εκλογή/άφιξη Όθωνα και Αντιβασιλεία",
      "Αναλυτικά — Κεφ. 4, Εν. 18-19. Απόλυτη και Συνταγματική Μοναρχία · 3η Σεπτεμβρίου 1843 · έξωση Όθωνα",
      "Αναλυτικά — Κεφ. 4, Εν. 20. Από την έξωση του Όθωνα έως το κίνημα στο Γουδί",
      "Προαιρετικά — Κεφ. 4, Εν. 23. Ελληνική οικονομία και κοινωνία τον 19ο αιώνα",
      "Αναλυτικά — Κεφ. 6, Εν. 27-28. Κίνημα στο Γουδί και βενιζελική πολιτική 1910-1912",
      "Αναλυτικά — Κεφ. 4 Εν. 21-22 + Κεφ. 6 Εν. 29-30. Κρητικό ζήτημα, Βαλκάνια και Βαλκανικοί Πόλεμοι",
      "Αναλυτικά — Κεφ. 7, Εν. 31. Αίτια, έκρηξη και μέτωπα του Α΄ Παγκοσμίου Πολέμου",
      "Αναλυτικά — Κεφ. 7, Εν. 32. Η Ελλάδα στον Α΄ Παγκόσμιο Πόλεμο — Εθνικός Διχασμός",
      "Αναλυτικά — Κεφ. 7, Εν. 33. Η Ρωσική Επανάσταση",
      "Αναλυτικά — Κεφ. 7, Εν. 34. Λήξη Α΄ Παγκοσμίου Πολέμου και μεταπολεμικές ρυθμίσεις",
      "Προαιρετικά — Κεφ. 8, Εν. 35. Διεκδικήσεις της Αντάντ και της Ελλάδας στην Οθωμανική Αυτοκρατορία",
      "Αναλυτικά — Κεφ. 8, Εν. 36. Ελληνισμός δυτικής Μικράς Ασίας και Πόντου",
      "Αναλυτικά — Κεφ. 8, Εν. 38. Ο Μικρασιατικός Πόλεμος 1919-1922",
      "Αναλυτικά — Κεφ. 8 Εν. 39 + Κεφ. 9 Εν. 44. Μετά τον Μικρασιατικό Πόλεμο και προσφυγικό ζήτημα",
      "Αναλυτικά — Κεφ. 9, Εν. 40-41. Παγκόσμια οικονομική κρίση 1929 και κοινωνικές διαστάσεις",
      "Αναλυτικά — Κεφ. 9, Εν. 42 + Εν. 43 (4η Αυγούστου). Πολιτικές διαστάσεις της κρίσης",
      "Αναλυτικά — Κεφ. 10, Εν. 45. Προμηνύματα και αίτια του Β΄ Παγκοσμίου Πολέμου",
      "Συνοπτικά — Κεφ. 10, Εν. 46. Ο Β΄ Παγκόσμιος Πόλεμος",
      "Αναλυτικά — Κεφ. 10, Εν. 47. Η συμμετοχή της Ελλάδας στον Β΄ Παγκόσμιο Πόλεμο",
      "Αναλυτικά — Κεφ. 10, Εν. 48. Κατοχή, Αντίσταση και Απελευθέρωση",
      "Αναλυτικά — Κεφ. 10, Εν. 49. Αποτελέσματα Β΄ Παγκοσμίου Πολέμου και ίδρυση ΟΗΕ",
      "Συνοπτικά — Κεφ. 11, Εν. 50-51. Μεταπολεμική διαίρεση της Ευρώπης, Διπολισμός και Ψυχρός Πόλεμος",
      "Συνοπτικά — Κεφ. 11, Εν. 52-53. Αποαποικιοποίηση, Τρίτος Κόσμος και μεταψυχροπολεμική Ευρώπη",
      "Συνοπτικά — Κεφ. 12, Εν. 54-55. Εμφύλιος, μετεμφυλιακή Ελλάδα, Κυπριακός αγώνας και δικτατορία 1967-1974"
    ],
    note:"Η Γ΄ Γυμνασίου Ε.Α.Ε. έχει 42 ώρες και οι οδηγίες διακρίνουν αναλυτική, συνοπτική και προαιρετική διδασκαλία. Το site διατηρεί αυτές τις διακρίσεις μέσα στα ίδια τα labels, αντί να παρουσιάζει όλες τις ενότητες σαν ίσης βαρύτητας ύλη.",
    meta:{
      plannedHours:42,
      selectionFramework:true,
      selectionStatus:"analytic-summary-optional-status-preserved",
      anchorPolicy:"official-history-teaching-status",
      coverageCompleteness:"section-status-verified"
    }
  });

  upsert({
    id:"teacher-annual-special-gym-a-technology",grade:"A",subject:"Τεχνολογία",subjectId:"technology",
    sourceTitle:"ΤΕΧΝΟΛΟΓΙΑ ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27 ΓΥΜΝ ΕΑΕ",sourceUrl:SOURCES.technology,
    official:[
      "Μέθοδος ατομικής εργασίας — αναγνώριση και μελέτη του τεχνολογικού περιβάλλοντος",
      "Ατομική επιλογή τεχνολογικού έργου",
      "Έρευνα βιβλιογραφίας και πηγών πληροφόρησης για το επιλεγμένο έργο",
      "Συλλογή στοιχείων από το τεχνολογικό περιβάλλον",
      "Ταξινόμηση πληροφόρησης και καθορισμός εργαλείων/υλικών",
      "Προγραμματισμός εργασίας και παρουσίαση σε σεμινάριο"
    ],
    note:"Οι επίσημες οδηγίες Τεχνολογίας Γυμνασίου Ε.Α.Ε. 2026-2027 δηλώνουν ρητά ότι δεν διδάσκεται συγκεκριμένη κοινή ύλη ανά τάξη. Στην Α΄ εφαρμόζεται η μέθοδος της ατομικής εργασίας. Οι επιλογές του site αποτυπώνουν στάδια/δραστηριότητες της μεθόδου και δεν παρουσιάζονται ως κεφάλαια ή εξεταστέα ύλη.",
    meta:{weeklyHours:1,selectionFramework:true,selectionStatus:"activity-method-framework-not-fixed-syllabus",anchorPolicy:"official-individual-project-method",coverageCompleteness:"method-framework-verified",explicitNoFixedSyllabus:true}
  });

  upsert({
    id:"teacher-annual-special-gym-b-technology",grade:"B",subject:"Τεχνολογία",subjectId:"technology",
    sourceTitle:"ΤΕΧΝΟΛΟΓΙΑ ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27 ΓΥΜΝ ΕΑΕ",sourceUrl:SOURCES.technology,
    official:[
      "Ομαδική εργασία — μελέτη βιομηχανίας και βιομηχανικής παραγωγικής μονάδας",
      "Ανάλυση οργάνωσης βιομηχανικής μονάδας και αρμοδιοτήτων τμημάτων",
      "Ανάλυση συνεργασίας τμημάτων/εργαζομένων και τεχνολογιών κάθε τμήματος",
      "Συγκρότηση ομάδων — επιλογή βιομηχανίας, μοντέλου και ρόλων",
      "Συλλογή και παρουσίαση πληροφοριών για τον ρόλο κάθε μαθητή/τριας",
      "Γραμμή παραγωγής — μελέτη οργάνωσης παραγωγικής γραμμής",
      "Επιλογή προϊόντος για παραγωγή σε μεγάλη ποσότητα και κατανομή ρόλων"
    ],
    note:"Στη Β΄ Γυμνασίου Ε.Α.Ε. εφαρμόζεται, ανάλογα με την υλικοτεχνική υποδομή του σχολείου, η μέθοδος της ομαδικής εργασίας ή της γραμμής παραγωγής για τη μελέτη της βιομηχανίας. Δεν μετατρέπουμε τη μέθοδο σε τεχνητή λίστα κεφαλαίων.",
    meta:{weeklyHours:1,selectionFramework:true,selectionStatus:"activity-method-framework-not-fixed-syllabus",anchorPolicy:"official-industry-group-work-or-production-line-method",coverageCompleteness:"method-framework-verified",explicitNoFixedSyllabus:true}
  });

  upsert({
    id:"teacher-annual-special-gym-c-technology",grade:"C",subject:"Τεχνολογία",subjectId:"technology",
    sourceTitle:"ΤΕΧΝΟΛΟΓΙΑ ΟΔΗΓΙΕΣ ΔΙΔΑΣΚΑΛΙΑΣ 26-27 ΓΥΜΝ ΕΑΕ",sourceUrl:SOURCES.technology,
    official:[
      "Μέθοδος «Έρευνα και Πειραματισμός» σε τεχνολογικό θέμα",
      "Ενημέρωση για τη φύση και τα στάδια της ερευνητικής διαδικασίας",
      "Σύνδεση της έρευνας με πραγματικές τεχνολογικές καταστάσεις",
      "Σχεδίαση ερευνητικής διαδικασίας",
      "Επιλογή ερευνητικού θέματος από την ομάδα",
      "Αναζήτηση και αξιολόγηση πηγών πληροφόρησης",
      "Κατασκευές/δοκίμια και πειραματισμός",
      "Συγγραφή ομαδικής ερευνητικής εργασίας",
      "Σεμινάρια προόδου, παρουσίαση και σημεία ελέγχου/αξιολόγησης"
    ],
    note:"Στη Γ΄ Γυμνασίου Ε.Α.Ε. η επίσημη μέθοδος είναι «Έρευνα και Πειραματισμός». Το θέμα επιλέγεται από την ομάδα με έγκριση του/της εκπαιδευτικού. Το site εμφανίζει τα επαληθευμένα στάδια της ερευνητικής διαδικασίας και όχι επινοημένη κοινή ύλη.",
    meta:{weeklyHours:1,selectionFramework:true,selectionStatus:"activity-method-framework-not-fixed-syllabus",anchorPolicy:"official-research-and-experimentation-method",coverageCompleteness:"method-framework-verified",explicitNoFixedSyllabus:true}
  });

  upsert({
    id:"teacher-annual-special-gym-a-geography",grade:"A",subject:"Γεωλογία-Γεωγραφία",subjectId:"geography",
    sourceTitle:"ΓΕΩΛΟΓΙΑ-ΓΕΩΓΡΑΦΙΑ Α-Β ΓΥΜΝ ΕΑΕ ΟΔΗΓΙΕΣ 26-27",sourceUrl:SOURCES.geographyAB,
    required:[
      "Α1.4 Ποιον χάρτη να διαλέξω; + Α1.5 «Ανακρίνοντας» τους χάρτες",
      "Α1.3 Η χρήση των χαρτών στην καθημερινή ζωή",
      "Α1.1 Γεωγραφικές συντεταγμένες + Α1.2 Παιχνίδια με τις γεωγραφικές συντεταγμένες (σε σύμπτυξη)",
      "Β1.1 Ο πλανήτης Γη",
      "Β1.2 Χωρίζοντας το περιβάλλον σε ενότητες",
      "Β2.1 Σύνθεση της Ατμόσφαιρας, θερμοκρασία, άνεμοι",
      "Β2.2 Οι βροχές, το κλίμα",
      "Β3.1 Το νερό στη φύση",
      "Β3.2 Ωκεανοί και θάλασσες",
      "Β3.4 Τα ποτάμια του κόσμου",
      "Β4.1 Μιλώντας για την ηλικία της Γης",
      "Β4.2 Το εσωτερικό της Γης",
      "Β4.3 Δυνάμεις που διαμορφώνουν την επιφάνεια της Γης: Ενδογενείς και εξωγενείς",
      "Β5.1 Η γεωγραφική κατανομή των οργανισμών",
      "Γ1.1 Ο πληθυσμός της Γης",
      "Γ1.3 Παιχνίδια με τις ηλικιακές πυραμίδες",
      "Γ1.5 Πού είναι χτισμένες οι μεγάλες πόλεις του πλανήτη",
      "Γ2.1 Φυσικοί πόροι + Γ2.2 Ανθρώπινοι πόροι",
      "Γ1.6 Τόσο διαφορετικοί, τόσο ίδιοι + Γ2.3 Προβλήματα που ζητούν απαντήσεις"
    ],
    modified:[
      "Β4.4 Μορφές του αναγλύφου της Γης — χωρίς τις υποενότητες που εξαιρούνται ως επανάληψη της ΣΤ΄ Δημοτικού",
      "Δ.7 Ευρώπη — μόνο ως σύνδεση/αναφορά στο project ηπείρων και στη Β΄ Γυμνασίου"
    ],
    optional:[
      "Δ.1 Αφρική — Φυσικό Περιβάλλον … και άνθρωποι",
      "Δ.2 Ασία — Φυσικό Περιβάλλον … και άνθρωποι",
      "Δ.3 Βόρεια και Κεντρική Αμερική — Φυσικό Περιβάλλον … και άνθρωποι",
      "Δ.4 Νότια Αμερική — Φυσικό Περιβάλλον … και άνθρωποι",
      "Δ.5 Ωκεανία — Φυσικό Περιβάλλον … και άνθρωποι",
      "Δ.6 Ανταρκτική"
    ],
    excluded:[
      "Α1.4 Έγχρωμο ένθετο «Συνταγές για νεαρούς χαρτογράφους», σ. 22-23",
      "Α1.1-Α1.2 «Παίζοντας με το γεωγραφικό μήκος», «Από την υδρόγειο σφαίρα στους χάρτες», «Κάθε χαρτογραφική προβολή έχει και… συνέπειες!!!», «Ας κάνουμε τους χαρτογράφους…»",
      "Β3.1 Υποενότητα «Οι μεγάλες λίμνες του κόσμου»",
      "Β3.3 Άνθρωποι και θάλασσα – Τα νησιωτικά κράτη",
      "Β3.5 Τα ποτάμια της Ασίας","Β3.6 Τα ποτάμια της Αμερικής","Β3.7 Τα ποτάμια της Αφρικής – Τα ποτάμια της Αυστραλίας",
      "Β4.4 Υποενότητα «Μεγάλες Οροσειρές του πλανήτη…»","Β4.4 Υποενότητα «Πεδιάδες, βοσκοτόπια και έρημοι του πλανήτη»",
      "Γ1.2 Η κατανομή των ανθρώπων στη Γη",
      "Γ1.4 Οι μεγάλες πόλεις του πλανήτη"
    ],
    note:"Ακριβής χαρτογράφηση από τις επίσημες οδηγίες Γεωλογίας-Γεωγραφίας Α΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Οι έξι ηπειρωτικές ενότητες Δ.1-Δ.6 διατηρούνται ως ρητά προαιρετικές. Όσα κεφάλαια ή υποενότητες σημειώνονται «ΟΧΙ» δεν εκτίθενται ως επιλογές.",
    meta:{
      plannedHours:30,
      coverageCompleteness:"section-level-verified",
      classificationStatus:"official-required-optional-excluded-preserved",
      mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%93%CE%95%CE%A9%CE%9B%CE%9F%CE%93%CE%99%CE%91-%CE%93%CE%95%CE%A9%CE%93%CE%A1%CE%91%CE%A6%CE%99%CE%91-%CE%91-%CE%92-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-26-27.pdf"
    }
  });

  upsert({
    id:"teacher-annual-special-gym-b-geography",grade:"B",subject:"Γεωλογία-Γεωγραφία",subjectId:"geography",
    sourceTitle:"ΓΕΩΛΟΓΙΑ-ΓΕΩΓΡΑΦΙΑ Α-Β ΓΥΜΝ ΕΑΕ ΟΔΗΓΙΕΣ 26-27",sourceUrl:SOURCES.geographyAB,
    required:[
      "Μάθημα 1 — Οι έννοιες «Γεωγραφική» και «σχετική» θέση",
      "Μάθημα 2 — Η σημασία της σχετικής θέσης για τους ανθρώπους",
      "Μάθημα 3 — Μελετώντας με χάρτες τη θέση της Ευρώπης στον κόσμο",
      "Μάθημα 6 — Η γεωλογική ιστορία της Ευρώπης και η ορογένεση",
      "Μάθημα 7 — Η διαμόρφωση του ανάγλυφου στην Ευρώπη",
      "Μάθημα 8 — Η γεωλογική ιστορία της Ελλάδας",
      "Μάθημα 9 — Σεισμική και ηφαιστειακή δράση στην Ευρώπη και στην Ελλάδα",
      "Μάθημα 10 — Η επίδραση των σεισμών και των ηφαιστείων στη ζωή μας",
      "Μάθημα 11 — Οι φυσιογραφικές περιοχές της Ευρώπης",
      "Μάθημα 16 — Τα βουνά και οι πεδιάδες της Ευρώπης",
      "Μάθημα 18 — Τα βουνά και οι πεδιάδες της Ελλάδας",
      "Μάθημα 12 — Οι θάλασσες της Ευρώπης",
      "Μάθημα 13 — Βαλτική και Βόρεια θάλασσα: Δύο θάλασσες του Ευρωπαϊκού Βορρά",
      "Μάθημα 14 — Η Μεσόγειος Θάλασσα",
      "Μάθημα 15 — Οι άνθρωποι στη Μεσόγειο",
      "Μάθημα 19 — Το κλίμα της Ευρώπης",
      "Μάθημα 20 — Το κλίμα της Ελλάδας",
      "Μάθημα 21 — Τα ποτάμια και οι λίμνες της Ευρώπης",
      "Μάθημα 22 — Τα ποτάμια και οι λίμνες στη ζωή των Ευρωπαίων",
      "Μάθημα 23 — Τα ποτάμια και οι λίμνες της Ελλάδας",
      "Μάθημα 25 — Η πολιτική διαίρεση της Ευρώπης",
      "Μάθημα 26 — Η Ευρωπαϊκή Ένωση",
      "Μάθημα 27 — Η σημασία της Ευρωπαϊκής Ένωσης",
      "Μάθημα 28 — Η διοικητική διαίρεση της Ελλάδας",
      "Μάθημα 29 — Ο πληθυσμός της Ευρώπης",
      "Μάθημα 30 — Τα χαρακτηριστικά του πληθυσμού της Ευρώπης",
      "Μάθημα 31 — Ο πληθυσμός της Ελλάδας",
      "Μάθημα 32 — Τα πολιτισμικά χαρακτηριστικά των Ευρωπαίων",
      "Μάθημα 34 — Οι μεγάλες πόλεις της Ελλάδας",
      "Μάθημα 35 — Οι Βαλκανικές χώρες",
      "Μάθημα 36 — Οι γείτονές μας στα Βαλκάνια",
      "Μάθημα 43 — Η εξόρυξη και οι κατασκευές στην Ευρώπη",
      "Μάθημα 44 — Ο δευτερογενής τομέας στην Ελλάδα"
    ],
    optional:[
      "Μάθημα 17 — Τα βουνά και οι πεδιάδες στη ζωή των Ευρωπαίων",
      "Μάθημα 24 — Η βλάστηση της Ευρώπης",
      "Μάθημα 33 — Οι μεγάλες πόλεις της Ευρώπης",
      "Μάθημα 37 — Οι τομείς παραγωγής της Ευρωπαϊκής Οικονομίας",
      "Μάθημα 38 — Η γεωργία και η δασοκομία στην Ευρώπη",
      "Μάθημα 39 — Η κτηνοτροφία, η αλιεία και οι υδατοκαλλιέργειες στην Ευρώπη",
      "Μάθημα 40 — Ο πρωτογενής τομέας στην Ελλάδα",
      "Μάθημα 41 — Η βιομηχανία και η βιοτεχνία στην Ευρώπη",
      "Μάθημα 42 — Η παραγωγή και η κατανάλωση ενέργειας στην Ευρώπη",
      "Μάθημα 45 — Το εμπόριο στην Ευρώπη",
      "Μάθημα 46 — Ο τουρισμός στην Ευρώπη",
      "Μάθημα 47 — Οι μεταφορές, οι επικοινωνίες και οι άλλες υπηρεσίες στην Ευρώπη",
      "Μάθημα 48 — Ο τριτογενής τομέας στην Ελλάδα"
    ],
    excluded:[
      "Μάθημα 4 — Μελετώντας με χάρτες το φυσικό περιβάλλον της Ευρώπης",
      "Μάθημα 5 — Μελετώντας με χάρτες τους κατοίκους της Ευρώπης"
    ],
    note:"Ακριβής χαρτογράφηση από τις επίσημες οδηγίες Γεωλογίας-Γεωγραφίας Β΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Τα μαθήματα 4 και 5 εξαιρούνται ρητά, ενώ τα 17, 24, 33, 37-42, 45-48 διατηρούνται ως προαιρετικά και δεν συγχέονται με τον βασικό κορμό.",
    meta:{
      plannedHours:67,
      coverageCompleteness:"section-level-verified",
      classificationStatus:"official-required-optional-excluded-preserved",
      mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%93%CE%95%CE%A9%CE%9B%CE%9F%CE%93%CE%99%CE%91-%CE%93%CE%95%CE%A9%CE%93%CE%A1%CE%91%CE%A6%CE%99%CE%91-%CE%91-%CE%92-%CE%93%CE%A5%CE%9C%CE%9D-EAE-%CE%9F%CE%94%CE%97%CE%93%CE%99%CE%95%CE%A3-26-27.pdf"
    }
  });

  upsert({
    id:"teacher-annual-special-gym-a-biology",grade:"A",subject:"Βιολογία",subjectId:"biology",
    sourceTitle:"ΒΙΟΛΟΓΙΑ Α ΓΥΜΝ ΕΑΕ 26-27",sourceUrl:SOURCES.biologyA,
    required:[
      "1.1 Τα χαρακτηριστικά των οργανισμών",
      "1.2 Κύτταρο: Η μονάδα της ζωής",
      "1.3 Η οργάνωση των πολυκύτταρων οργανισμών",
      "2.1 Η παραγωγή θρεπτικών ουσιών στα φυτά – φωτοσύνθεση",
      "2.2 Η πρόσληψη ουσιών και πέψη στους μονοκύτταρους οργανισμούς",
      "2.4 Η πρόσληψη ουσιών και πέψη στον άνθρωπο",
      "3.1 Η μεταφορά και η αποβολή ουσιών στους μονοκύτταρους οργανισμούς",
      "3.2 Η μεταφορά και αποβολή ουσιών στα φυτά",
      "3.4 Η μεταφορά και αποβολή ουσιών στον άνθρωπο",
      "4.1 Η αναπνοή στους μονοκύτταρους οργανισμούς",
      "4.2 Η αναπνοή στα φυτά",
      "4.4 Η αναπνοή στον άνθρωπο"
    ],
    optional:[
      "1.4 Αλληλεπιδράσεις και προσαρμογές",
      "2.3 Η πρόσληψη ουσιών και πέψη στους ζωικούς οργανισμούς"
    ],
    note:"Ακριβής χαρτογράφηση από την επίσημη ύλη Βιολογίας Α΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Οι 1.4 και 2.3 αναφέρονται ρητά ως προαιρετικές. Δεν εμφανίζονται στο μενού ενότητες που δεν περιλαμβάνονται στον επίσημο φετινό κατάλογο ύλης.",
    meta:{
      plannedHours:25,
      coverageCompleteness:"section-level-verified",
      classificationStatus:"official-required-optional-preserved",
      mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%92%CE%99%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91-%CE%91-%CE%93%CE%A5%CE%9C%CE%9D-%CE%95AE-26-27.pdf"
    }
  });

  upsert({
    id:"teacher-annual-special-gym-b-biology",grade:"B",subject:"Βιολογία",subjectId:"biology",
    sourceTitle:"ΒΙΟΛΟΓΙΑ Β ΓΥΜΝ ΕΑΕ 26-27",sourceUrl:SOURCES.biologyB,
    required:[
      "Βιολογία Α΄ — 5.1 Στήριξη και κίνηση στους μονοκύτταρους οργανισμούς",
      "Βιολογία Α΄ — 5.2 Η στήριξη στα φυτά",
      "Βιολογία Α΄ — 5.3 Η στήριξη και κίνηση στους ζωικούς οργανισμούς",
      "Βιολογία Α΄ — 5.4 Το μυοσκελετικό σύστημα του ανθρώπου",
      "Βιολογία Α΄ — Αναπαραγωγή (εισαγωγικό κείμενο σελ. 114)",
      "Βιολογία Α΄ — 6.1 Η αναπαραγωγή στους μονοκύτταρους οργανισμούς",
      "Βιολογία Α΄ — 6.2 Η αναπαραγωγή στα φυτά",
      "Βιολογία Α΄ — 6.3 Η αναπαραγωγή στους ζωικούς οργανισμούς",
      "Βιολογία Α΄ — 6.4 Η αναπαραγωγή στον άνθρωπο",
      "Βιολογία Β΄-Γ΄ — 4.1 Ομοιόσταση",
      "Βιολογία Β΄-Γ΄ — 4.2 Ασθένειες",
      "Βιολογία Β΄-Γ΄ — 4.3 Αμυντικοί μηχανισμοί του ανθρώπου",
      "Βιολογία Β΄-Γ΄ — 4.4 Τρόπος ζωής και ασθένειες"
    ],
    optional:[
      "Βιολογία Β΄-Γ΄ — 1.2 Κύτταρο: η μονάδα της ζωής"
    ],
    note:"Η Β΄ Γυμνασίου Ε.Α.Ε. 2026-2027 χρησιμοποιεί δύο σχολικά βιβλία. Οι ενότητες Στήριξης-Κίνησης και Αναπαραγωγής που δεν διδάχθηκαν στην Α΄ μεταφέρονται στη Β΄, ενώ από το βιβλίο Β΄-Γ΄ διδάσκεται το Κεφάλαιο 4. Η 1.2 «Κύτταρο» αναφέρεται ρητά ως προαιρετική.",
    meta:{
      plannedHours:25,
      coverageCompleteness:"section-level-verified",
      classificationStatus:"official-required-optional-preserved",
      sourceBooks:["Βιολογία Α΄ Γυμνασίου","Βιολογία Β΄ & Γ΄ Γυμνασίου"],
      mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%92%CE%99%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91-B-%CE%93%CE%A5%CE%9C%CE%9D-%CE%95AE-26-27.pdf"
    }
  });

  upsert({
    id:"teacher-annual-special-gym-c-biology",grade:"C",subject:"Βιολογία",subjectId:"biology",
    sourceTitle:"ΒΙΟΛΟΓΙΑ Γ ΓΥΜΝ ΕΑΕ 26-27",sourceUrl:SOURCES.biologyC,
    required:[
      "1.1 Τα μόρια της ζωής",
      "1.2 Κύτταρο: η μονάδα της ζωής",
      "2.1 Ισορροπία στα βιολογικά οικοσυστήματα",
      "2.2 Οργάνωση και λειτουργίες οικοσυστήματος – Ο ρόλος της ενέργειας",
      "5.1 Το γενετικό υλικό οργανώνεται σε χρωμοσώματα",
      "5.2 Η ροή της γενετικής πληροφορίας",
      "5.3 Αλληλόμορφα",
      "5.4 Κυτταρική διαίρεση",
      "5.5 Κληρονομικότητα",
      "5.6 Μεταλλάξεις",
      "7.1 Η εξέλιξη και οι μαρτυρίες της – Βιοχημικές αποδείξεις",
      "7.2 Η εξέλιξη του ανθρώπου"
    ],
    note:"Ακριβής χαρτογράφηση του επίσημου καταλόγου ύλης Βιολογίας Γ΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Το site εκθέτει μόνο τις ενότητες που περιλαμβάνονται στον φετινό κατάλογο και δεν συμπληρώνει τα κενά κεφαλαίων από μνήμη ή από παλαιότερη ύλη.",
    meta:{
      plannedHours:25,
      coverageCompleteness:"section-level-verified",
      classificationStatus:"official-syllabus-index-verified",
      mirrorPdfUrl:"https://dide.ira.sch.gr/wp-content/uploads/2026/09/%CE%92%CE%99%CE%9F%CE%9B%CE%9F%CE%93%CE%99%CE%91-%CE%93-%CE%93%CE%A5%CE%9C%CE%9D-%CE%95AE-26-27.pdf"
    }
  });

  upsert({
    id:"teacher-annual-special-gym-a-math",grade:"A",subject:"Μαθηματικά",subjectId:"math",
    sourceTitle:"ΜΑΘΗΜΑΤΙΚΑ Α ΓΥΜΝΑΣΙΟΥ EAE ΥΛΗ-ΟΔΗΓΙΕΣ 26-27 νέο_v1",sourceUrl:SOURCES.mathA,
    required:[
      "1.3 Δυνάμεις φυσικών αριθμών","1.4 Ευκλείδεια διαίρεση - Διαιρετότητα","1.5 Χαρακτήρες διαιρετότητας - Μ.Κ.Δ. - Ε.Κ.Π. - Ανάλυση αριθμού σε γινόμενο πρώτων παραγόντων",
      "2.1 Η έννοια του κλάσματος","2.2 Ισοδύναμα κλάσματα","2.3 Σύγκριση κλασμάτων","2.4 Πρόσθεση και αφαίρεση κλασμάτων","2.6 Διαίρεση κλασμάτων",
      "3.1 Δεκαδικά κλάσματα, δεκαδικοί αριθμοί, διάταξη και στρογγυλοποίηση","3.5 Μονάδες μέτρησης",
      "5.1 Ποσοστά","5.2 Προβλήματα με ποσοστά",
      "7.1 Θετικοί και αρνητικοί αριθμοί - Η ευθεία των ρητών - Τετμημένη σημείου","7.2 Απόλυτη τιμή ρητού - Αντίθετοι ρητοί - Σύγκριση ρητών","7.3 Πρόσθεση ρητών αριθμών","7.4 Αφαίρεση ρητών αριθμών","7.5 Πολλαπλασιασμός ρητών αριθμών","7.6 Διαίρεση ρητών αριθμών",
      "Γεωμετρία 1.3 Μέτρηση, σύγκριση και ισότητα ευθυγράμμων τμημάτων - Απόσταση σημείων - Μέσο ευθύγραμμου τμήματος",
      "Γεωμετρία 1.7 Εφεξής και διαδοχικές γωνίες - Άθροισμα γωνιών",
      "Γεωμετρία 1.10 Απόσταση σημείου από ευθεία - Απόσταση παραλλήλων","Γεωμετρία 1.11 Κύκλος και στοιχεία του κύκλου",
      "Γεωμετρία 2.1 Συμμετρία ως προς άξονα","Γεωμετρία 2.3 Μεσοκάθετος ευθύγραμμου τμήματος","Γεωμετρία 2.4 Συμμετρία ως προς σημείο","Γεωμετρία 2.5 Κέντρο συμμετρίας","Γεωμετρία 2.6 Παράλληλες ευθείες που τέμνονται από μία άλλη ευθεία"
    ],
    modified:[
      "2.5 Πολλαπλασιασμός κλασμάτων",
      "4.1 Η έννοια της εξίσωσης - απλές εξισώσεις","4.2 Επίλυση προβλημάτων","4.3 Παραδείγματα επίλυσης προβλημάτων",
      "Γεωμετρία 1.1 Σημείο - Ευθύγραμμο τμήμα - Ευθεία - Ημιευθεία - Επίπεδο - Ημιεπίπεδο",
      "Γεωμετρία 1.2 Γωνία - Γραμμή - Επίπεδα και ευθύγραμμα σχήματα - Ίσα σχήματα",
      "Γεωμετρία 1.4 Πρόσθεση και αφαίρεση ευθυγράμμων τμημάτων",
      "Γεωμετρία 1.5 Μέτρηση, σύγκριση και ισότητα γωνιών - Διχοτόμος γωνίας",
      "Γεωμετρία 1.6 Είδη γωνιών - Κάθετες ευθείες",
      "Γεωμετρία 2.2 Άξονας συμμετρίας",
      "Γεωμετρία 3.1 Στοιχεία τριγώνου - Είδη τριγώνων","Γεωμετρία 3.2 Άθροισμα γωνιών τριγώνου - Ιδιότητες"
    ],
    optional:[
      "1.1 Φυσικοί αριθμοί - Διάταξη φυσικών - Στρογγυλοποίηση","1.2 Πρόσθεση - αφαίρεση και πολλαπλασιασμός φυσικών αριθμών",
      "4. Προβλήματα που απαιτούν σύνθετη σκέψη ή επίλυση σύνθετων εξισώσεων",
      "Γεωμετρία 1.12 Επίκεντρη γωνία - Μέτρηση τόξου","Γεωμετρία 1.13 Θέσεις ευθείας και κύκλου"
    ],
    excluded:["4.1 Η έννοια «αδύνατη εξίσωση»","4.1 Η έννοια «ταυτότητα»"],
    note:"Section-level χαρτογράφηση από τις επίσημες οδηγίες Μαθηματικών Α΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Τα Βασικό / Βασικό με τροποποίηση / Πρόσθετο διατηρούνται ως ξεχωριστές κατηγορίες και οι ρητές εξαιρέσεις δεν εμφανίζονται ως επιτρεπόμενες ενότητες.",
    meta:{coverageCompleteness:"section-level-verified-partial",classificationStatus:"official-symbol-status-preserved"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-math",grade:"B",subject:"Μαθηματικά",subjectId:"math",
    sourceTitle:"ΜΑΘΗΜΑΤΙΚΑ Β ΓΥΜΝΑΣΙΟΥ ΕΑΕ ΥΛΗ-ΟΔΗΓΙΕΣ 26-27 νέο_v1",sourceUrl:SOURCES.mathB,
    required:[
      "Επανάληψη Α΄: 7.8 Δυνάμεις ρητών αριθμών με εκθέτη φυσικό","Επανάληψη Α΄: 7.9 Δυνάμεις ρητών αριθμών με εκθέτη ακέραιο",
      "1.2 Εξισώσεις α΄ βαθμού","1.4 Επίλυση προβλημάτων με τη χρήση εξισώσεων",
      "2.1 Τετραγωνική ρίζα θετικού αριθμού","2.2 Άρρητοι αριθμοί - Πραγματικοί αριθμοί",
      "3.1 Η έννοια της συνάρτησης","3.3 Η συνάρτηση y = α·x","3.4 Η συνάρτηση y = α·x + β","3.5 Η συνάρτηση y = α/x - Η υπερβολή",
      "Γεωμετρία 1.1 Εμβαδόν επίπεδης επιφάνειας","Γεωμετρία 1.2 Μονάδες μέτρησης επιφανειών","Γεωμετρία 1.3 Εμβαδά επίπεδων σχημάτων","Γεωμετρία 1.4 Πυθαγόρειο θεώρημα",
      "Γεωμετρία 3.2 Κανονικά πολύγωνα","Γεωμετρία 3.3 Μήκος κύκλου","Γεωμετρία 3.5 Εμβαδόν κυκλικού δίσκου"
    ],
    modified:[
      "Επανάληψη Α΄: 7.7 Δεκαδική μορφή ρητών αριθμών",
      "1.1 Η έννοια της μεταβλητής - Αλγεβρικές παραστάσεις",
      "3.2 Καρτεσιανές συντεταγμένες - Γραφική παράσταση συνάρτησης",
      "4.1 Βασικές έννοιες της Στατιστικής: Πληθυσμός - Δείγμα","4.2 Γραφικές Παραστάσεις","4.5 Μέση τιμή - Διάμεσος",
      "Γεωμετρία 4.2 Στοιχεία και εμβαδόν πρίσματος και κυλίνδρου","Γεωμετρία 4.3 Όγκος πρίσματος και κυλίνδρου"
    ],
    optional:["2.3 Προβλήματα","Γεωμετρία 3.2 Κατασκευή κανονικών πολυγώνων"],
    excluded:[
      "1.1 Ο τυπικός ορισμός της «μεταβλητής» στη σ. 11",
      "3.2 Εφαρμογή 2 στη σ. 62","3.2 Τύπος απόστασης δύο σημείων στη σ. 63",
      "3.4 Η εξίσωση της μορφής α·x + β·y = γ","3.4 Σημεία τομής της ευθείας α·x + β·y = γ με τους άξονες",
      "4.5 Μέση τιμή ομαδοποιημένης κατανομής"
    ],
    note:"Οι παράγραφοι 7.7-7.9 προέρχονται από το βιβλίο της Α΄ Γυμνασίου ως επανάληψη και οι ίδιες οι οδηγίες διευκρινίζουν ότι δεν αποτελούν εξεταστέα ύλη. Η υπόλοιπη εγγραφή ακολουθεί τις section-level οδηγίες της Β΄ Ε.Α.Ε.",
    meta:{
      coverageCompleteness:"section-level-verified-partial",
      classificationStatus:"official-symbol-status-preserved",
      nonExamAnchors:["Επανάληψη Α΄: 7.7 Δεκαδική μορφή ρητών αριθμών","Επανάληψη Α΄: 7.8 Δυνάμεις ρητών αριθμών με εκθέτη φυσικό","Επανάληψη Α΄: 7.9 Δυνάμεις ρητών αριθμών με εκθέτη ακέραιο"]
    }
  });

  upsert({
    id:"teacher-annual-special-gym-c-math",grade:"C",subject:"Μαθηματικά",subjectId:"math",
    sourceTitle:"ΜΑΘΗΜΑΤΙΚΑ Γ ΓΥΜΝΑΣΙΟΥ ΕΑΕ ΥΛΗ-ΟΔΗΓΙΕΣ 26-27 νέο_v1",sourceUrl:SOURCES.mathC,
    required:[
      "1.1 Πράξεις με πραγματικούς αριθμούς - Δυνάμεις - Τετραγωνική ρίζα","1.2 Μονώνυμα - Πράξεις με μονώνυμα","1.3 Πολυώνυμα - Πρόσθεση και αφαίρεση πολυωνύμων",
      "1.4 Πολλαπλασιασμός πολυωνύμων","1.5 Αξιοσημείωτες ταυτότητες","1.6 Παραγοντοποίηση αλγεβρικών παραστάσεων","1.9 Ρητές αλγεβρικές παραστάσεις","1.10 Πράξεις ρητών παραστάσεων",
      "2.3 Προβλήματα εξισώσεων δευτέρου βαθμού",
      "3.1 Η έννοια της γραμμικής εξίσωσης","3.2 Η έννοια του γραμμικού συστήματος και η γραφική επίλυσή του","3.3 Αλγεβρική επίλυση γραμμικού συστήματος",
      "5.1 Σύνολα","5.2 Δειγματικός χώρος - Ενδεχόμενα",
      "Γεωμετρία 1.1 Ισότητα τριγώνων","Γεωμετρία 1.2 Λόγος ευθυγράμμων τμημάτων","Γεωμετρία 1.3 Θεώρημα του Θαλή","Γεωμετρία 1.5 Ομοιότητα - Όμοια πολύγωνα",
      "Τριγωνομετρία 2.1 Τριγωνομετρικοί αριθμοί γωνίας 0°≤ω≤180°","Τριγωνομετρία 2.2 Τριγωνομετρικοί αριθμοί παραπληρωματικών γωνιών","Τριγωνομετρία 2.3 Σχέσεις μεταξύ τριγωνομετρικών αριθμών μιας γωνίας"
    ],
    modified:[
      "1.8 Ε.Κ.Π. και Μ.Κ.Δ. ακεραίων αλγεβρικών παραστάσεων - βασικό μέρος μόνο για Ε.Κ.Π.",
      "2.2 Εξισώσεις δευτέρου βαθμού","2.5 Ανισότητες - Ανισώσεις μ' έναν άγνωστο","5.3 Έννοια της πιθανότητας"
    ],
    optional:[
      "1.3 Η έννοια της ισότητας πολυωνύμων","1.8 Μ.Κ.Δ. ακεραίων αλγεβρικών παραστάσεων",
      "2.1 Η εξίσωση αx + β = 0 ως επανάληψη","2.2 Απόδειξη του τύπου της δευτεροβάθμιας εξίσωσης","2.5 Αποδείξεις επιλεγμένων ιδιοτήτων της διάταξης",
      "Γεωμετρία 1.5 Όμοια τρίγωνα ως ειδική περίπτωση των όμοιων πολυγώνων"
    ],
    excluded:[
      "1.5 Διαφορά κύβων - Άθροισμα κύβων",
      "1.6 Διαφορά - άθροισμα κύβων","1.6 Παραγοντοποίηση τριωνύμου της μορφής x²+(α+β)x+αβ",
      "5.1 Πράξεις με σύνολα","5.2 Πράξεις με ενδεχόμενα","5.2 Ασυμβίβαστα ενδεχόμενα","5.3 Βασικοί κανόνες λογισμού των πιθανοτήτων"
    ],
    note:"Η εγγραφή ακολουθεί τις επίσημες οδηγίες Μαθηματικών Γ΄ Γυμνασίου Ε.Α.Ε. 2026-2027 και διατηρεί τις ρητές προαιρετικές και εξαιρούμενες υποπαραγράφους αντί να εμφανίζει ολόκληρα κεφάλαια ως ενιαία ύλη.",
    meta:{coverageCompleteness:"section-level-verified",classificationStatus:"official-symbol-status-preserved"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-physics",grade:"B",subject:"Φυσική",subjectId:"physics",
    sourceTitle:"ΦΥΣΙΚΗ ΓΥΜΝΑΣΙΟ ΕΑΕ ΟΔΗΓΙΕΣ 26-27",sourceUrl:SOURCES.physics,
    required:[
      "1.3 Τα φυσικά μεγέθη και οι μονάδες τους",
      "2.1 Περιγραφή της κίνησης",
      "3.1 Η έννοια «Δύναμη»","3.2 Δύο σημαντικές δυνάμεις στον κόσμο","3.4 Δύναμη και ισορροπία","3.6 Δύναμη και μεταβολή της ταχύτητας",
      "4.2 Υδροστατική πίεση","4.4 Μετάδοση των πιέσεων στα ρευστά - Αρχή του Πασκάλ","4.5 Άνωση - Αρχή του Αρχιμήδη",
      "5.2 Δυναμική - Κινητική ενέργεια","5.3 Η μηχανική ενέργεια και η διατήρησή της","5.5 Διατήρηση της ενέργειας","5.7 Απόδοση μιας μηχανής",
      "6.1 Θερμόμετρα και μέτρηση θερμοκρασίας","6.2 Θερμότητα: Μια μορφή ενέργειας","6.3 Πως μετράμε τη θερμότητα","6.4 Θερμοκρασία, θερμότητα και μικρόκοσμος"
    ],
    modified:[
      "2.2 Η έννοια της ταχύτητας",
      "3.3 Σύνθεση και ανάλυση δυνάμεων","3.5 Ισορροπία υλικού σημείου","3.7 Δύναμη και αλληλεπίδραση",
      "4.1 Πίεση","4.3 Ατμοσφαιρική πίεση",
      "5.1 Έργο και ενέργεια - εισαγωγικές εφαρμογές με δύναμη και μετατόπιση συγγραμμικές",
      "5.4 Μορφές και μετατροπές ενέργειας","5.8 Ισχύς",
      "6.5 Θερμική διαστολή και συστολή - ποιοτικά"
    ],
    excluded:[
      "2.2 Διανυσματική περιγραφή της ταχύτητας",
      "3.3 Δύναμη που ασκείται σε τραχιά επιφάνεια","3.3 Ανάλυση δύναμης","3.5 Ανάλυση δυνάμεων και ισορροπία","3.5 Παράδειγμα 3.2","3.7 Εφαρμογές",
      "4.1 Παράδειγμα εικόνας 4.4","4.3 Πως υπολογίζουμε την ατμοσφαιρική πίεση","4.6 Πλεύση",
      "5.4 Θεμελιώδεις μορφές ενέργειας","5.4 Μετατροπές ενέργειας","5.6 Πηγές ενέργειας","5.8 Ισχύς και κίνηση",
      "6.5 Μαθηματικές σχέσεις θερμικής διαστολής και συστολής"
    ],
    note:"Η επιλογή ενοτήτων ακολουθεί τον επίσημο κατάλογο ύλης και τις αναλυτικές οδηγίες της Φυσικής Β΄ Γυμνασίου Ε.Α.Ε. Οι ενότητες με ρητές περικοπές εμφανίζονται ως «Με προσαρμογή» και η Πλεύση / Πηγές ενέργειας παραμένουν εκτός.",
    meta:{coverageCompleteness:"section-level-verified",minimumPlannedHours:44}
  });

  upsert({
    id:"teacher-annual-special-gym-c-physics",grade:"C",subject:"Φυσική",subjectId:"physics",
    sourceTitle:"ΦΥΣΙΚΗ ΓΥΜΝΑΣΙΟ ΕΑΕ ΟΔΗΓΙΕΣ 26-27",sourceUrl:SOURCES.physics,
    required:[
      "1.1 Γνωριμία με την ηλεκτρική δύναμη","1.2 Το ηλεκτρικό φορτίο","1.3 Το ηλεκτρικό φορτίο στο εσωτερικό του ατόμου","1.4 Τρόποι ηλέκτρισης και η μικροσκοπική ερμηνεία","1.5 Νόμος του Κουλόμπ",
      "2.1 Το ηλεκτρικό ρεύμα","2.2 Ηλεκτρικό κύκλωμα",
      "3.3 Μαγνητικά αποτελέσματα του ηλεκτρικού ρεύματος","3.6 Ενέργεια και ισχύς του ηλεκτρικού ρεύματος",
      "4.2 Μεγέθη που χαρακτηρίζουν μια ταλάντωση",
      "5.1 Μηχανικά κύματα","5.2 Κύμα και ενέργεια","5.4 Ήχος","5.5 Υποκειμενικά χαρακτηριστικά του ήχου",
      "6.1 Φως: Όραση και ενέργεια","8.4 Το χρώμα"
    ],
    modified:[
      "1.6 Το ηλεκτρικό πεδίο - μόνο η υποενότητα «Ηλεκτρική δύναμη και πεδίο»",
      "2.3 Ηλεκτρικά δίπολα","2.5 Εφαρμογές αρχών διατήρησης στη μελέτη απλών ηλεκτρικών κυκλωμάτων - μόνο συνδέσεις αντιστατών",
      "3.1 Θερμικά αποτελέσματα του ηλεκτρικού ρεύματος",
      "4.1 Ταλαντώσεις - μόνο παραδείγματα για το τι είναι ταλάντωση",
      "5.3 Χαρακτηριστικά μεγέθη του κύματος - έως τη σχέση υ=λf χωρίς απόδειξη",
      "6.2 Διάδοση του φωτός","7.1 Ανάκλαση του φωτός","7.2 Εικόνες σε καθρέφτες: είδωλα",
      "8.1 Διάθλαση του φωτός","8.3 Ανάλυση του φωτός - μόνο η ανάλυση του λευκού φωτός"
    ],
    excluded:[
      "2.3 Εικόνες 2.30 και 2.31 με το αντίστοιχο κείμενο","2.3 Νόμος του Ωμ και μικρόκοσμος","2.3 Μικροσκοπική ερμηνεία της αντίστασης ενός μεταλλικού αγωγού",
      "3.1 Πειραματική μελέτη φαινομένου Joule","3.1 Νόμος του Joule","3.1 Ερμηνεία του φαινομένου Joule",
      "3.4 Ηλεκτρική και μηχανική ενέργεια","3.5 Βιολογικά αποτελέσματα του ηλεκτρικού ρεύματος",
      "5.3 Κυματικά φαινόμενα: ανάκλαση και διάθλαση των μηχανικών κυμάτων","5.3 Ανάκλαση μηχανικών κυμάτων","5.3 Διάθλαση μηχανικών κυμάτων",
      "6.2 Αρχή του ελαχίστου χρόνου","7.1 Ανάκλαση και αρχή του ελαχίστου χρόνου",
      "7.2 Καμπύλοι καθρέφτες","7.2 Σφαιρικοί καθρέφτες - εστία σφαιρικών καθρεφτών","7.2 Οπτικό πεδίο",
      "8.1 Διάθλαση και αρχή του ελάχιστου χρόνου","8.1 Νόμος της διάθλασης - Snell"
    ],
    note:"Η εγγραφή βασίζεται στον επίσημο κατάλογο ύλης και στις αναλυτικές οδηγίες της Φυσικής Γ΄ Γυμνασίου Ε.Α.Ε. 2026-2027. Διατηρούνται οι περιορισμοί σε ηλεκτρικά δίπολα/κυκλώματα, ταλαντώσεις, κύματα, ανάκλαση και διάθλαση ώστε να μη διδαχθεί υλικό που έχει αφαιρεθεί.",
    meta:{coverageCompleteness:"section-level-verified",classificationStatus:"official-teach-do-not-teach-preserved"}
  });

  upsert({
    id:"teacher-annual-special-gym-a-physics",
    grade:"A",subject:"Φυσική",subjectId:"physics",
    sourceTitle:"ΦΥΣΙΚΗ ΓΥΜΝΑΣΙΟ ΕΑΕ ΟΔΗΓΙΕΣ 26-27",
    sourceUrl:SOURCES.physics,
    official:[
      "1. Μετρήσεις μήκους - Η Μέση Τιμή",
      "2. Μετρήσεις Χρόνου – Η Ακρίβεια",
      "3. Μετρήσεις μάζας – Τα διαγράμματα",
      "4. Μέτρηση όγκου",
      "5. Μέτρηση Πυκνότητας",
      "6. Μετρήσεις Θερμοκρασίας – Η Βαθμονόμηση",
      "7. Από τη Θερμότητα στη Θερμοκρασία – Η Θερμική Ισορροπία",
      "8. Το Ηλεκτρικό βραχυκύκλωμα – Κίνδυνοι και Ασφάλεια",
      "9. Από τον Ηλεκτρισμό στον Μαγνητισμό – Ένας Ηλεκτρικός Κινητήρας",
      "10. Από το Μαγνητισμό στον Ηλεκτρισμό – Μια Ηλεκτρική Γεννήτρια"
    ],
    note:"Οι 10 τίτλοι αντιστοιχούν στον επίσημο ετήσιο πίνακα Φυσικής Α΄ Γυμνασίου Ε.Α.Ε. για το 2026-2027. Δεν γίνεται εδώ πρόσθετος χαρακτηρισμός υποχρεωτικού/προαιρετικού πέρα από ό,τι μπορεί να τεκμηριωθεί με ασφάλεια από την πηγή.",
    meta:{minimumPlannedHours:23}
  });

  upsert({
    id:"teacher-annual-special-gym-a-pe",grade:"A",subject:"Φυσική Αγωγή",subjectId:"pe",
    sourceTitle:"ΦΥΣΙΚΗ ΑΓΩΓΗ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 2026-2027",sourceUrl:SOURCES.pe,
    official:[
      "Εισαγωγικά μαθήματα (2 ώρες)","Ποδόσφαιρο (7 ώρες)","Πετοσφαίριση (7 ώρες)",
      "Κλασικός αθλητισμός: άλματα, δρόμοι, ρίψεις (7 ώρες)","Ενόργανη-Ρυθμική Γυμναστική (5 ώρες)",
      "Παραδοσιακοί χοροί (6 ώρες)","Καλαθοσφαίριση (7 ώρες)",
      "Φυσική κατάσταση - Καταγραφή δεικτών ΦΚ (5 ώρες)","Ενδοσχολικές διοργανώσεις προσαρμοσμένων αθλοπαιδιών (4 ώρες)"
    ],
    note:"Πρόκειται για την επίσημη προτεινόμενη κατανομή περίπου 50 ωρών Φυσικής Αγωγής και όχι για εξεταστέα ύλη.",
    meta:{weeklyHours:2,approxAnnualHours:50,anchorPolicy:"official-recommended-annual-allocation"}
  });
  upsert({
    id:"teacher-annual-special-gym-b-pe",grade:"B",subject:"Φυσική Αγωγή",subjectId:"pe",
    sourceTitle:"ΦΥΣΙΚΗ ΑΓΩΓΗ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 2026-2027",sourceUrl:SOURCES.pe,
    official:[
      "Καλαθοσφαίριση (5 ώρες)","Πετοσφαίριση (5 ώρες)","Παραδοσιακοί χοροί (6 ώρες)","Ποδόσφαιρο (6 ώρες)",
      "Κλασικός αθλητισμός: άλματα, δρόμοι, ρίψεις - Φυσική κατάσταση (10 ώρες)","Χειροσφαίριση (5 ώρες)",
      "Ενόργανη Γυμναστική (5 ώρες)","Ενδοσχολικές διοργανώσεις προσαρμοσμένων αθλοπαιδιών (4 ώρες)",
      "Σχέδιο εργασίας: μη διαδεδομένα αθλήματα, παραολυμπιακά αγωνίσματα, αθλήματα υγρού στίβου (έως 4 ώρες)"
    ],
    note:"Πρόκειται για την επίσημη προτεινόμενη κατανομή περίπου 50 ωρών Φυσικής Αγωγής και όχι για εξεταστέα ύλη.",
    meta:{weeklyHours:2,approxAnnualHours:50,anchorPolicy:"official-recommended-annual-allocation"}
  });
  upsert({
    id:"teacher-annual-special-gym-c-pe",grade:"C",subject:"Φυσική Αγωγή",subjectId:"pe",
    sourceTitle:"ΦΥΣΙΚΗ ΑΓΩΓΗ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 2026-2027",sourceUrl:SOURCES.pe,
    official:[
      "Πετοσφαίριση (6 ώρες)","Κλασικός αθλητισμός: άλματα, δρόμοι, ρίψεις - Φυσική κατάσταση - Καταγραφή δεικτών ΦΚ (10 ώρες)",
      "Παραδοσιακοί χοροί (5 ώρες)","Χειροσφαίριση (6 ώρες)","Ποδόσφαιρο (5 ώρες)","Γυμναστική (4 ώρες)",
      "Καλαθοσφαίριση (6 ώρες)","Σχέδιο εργασίας: μη διαδεδομένα αθλήματα, παραολυμπιακά αγωνίσματα, αθλήματα υγρού στίβου (έως 4 ώρες)",
      "Ενδοσχολικές διοργανώσεις προσαρμοσμένων αθλοπαιδιών (4 ώρες)"
    ],
    note:"Πρόκειται για την επίσημη προτεινόμενη κατανομή περίπου 50 ωρών Φυσικής Αγωγής και όχι για εξεταστέα ύλη.",
    meta:{weeklyHours:2,approxAnnualHours:50,anchorPolicy:"official-recommended-annual-allocation"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-chemistry",grade:"B",subject:"Χημεία",subjectId:"chemistry",
    sourceTitle:"ΧΗΜΕΙΑ Β ΓΥΜΝΑΣΙΟΥ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.chemistryB,
    official:[
      "1.1 Τι είναι η Χημεία και γιατί τη μελετάμε","1.2 Καταστάσεις των υλικών","1.3 Φυσικές ιδιότητες των υλικών",
      "2.1 Το νερό στη ζωή μας","2.2 Το νερό ως διαλύτης - Μείγματα","2.3 Περιεκτικότητα διαλύματος - Εκφράσεις περιεκτικότητας",
      "2.4 Ρύπανση του νερού","2.5 Διαχωρισμός μειγμάτων","2.6 Διάσπαση του νερού - Χημικές ενώσεις και χημικά στοιχεία",
      "2.7 Χημική αντίδραση","2.8 Άτομα και μόρια","2.9 Υποατομικά σωματίδια - Ιόντα",
      "2.10 Σύμβολα χημικών στοιχείων και χημικών ενώσεων (χωρίς χημικούς τύπους ιόντων και ιοντικών ενώσεων)",
      "2.11 Χημική εξίσωση","3.1 Σύσταση του ατμοσφαιρικού αέρα","3.2 Οξυγόνο","3.3 Διοξείδιο του άνθρακα",
      "3.4 Ατμοσφαιρική ρύπανση","4.2 Ρύπανση του εδάφους"
    ],
    excluded:["2.10 Χημικοί τύποι ιόντων και ιοντικών ενώσεων"],
    note:"Οι επιλογές ακολουθούν τον επίσημο κατάλογο «Ύλη» της Χημείας Β΄ Ε.Α.Ε. 2026-2027. Η ρητή εξαίρεση της 2.10 αποθηκεύεται αλλά δεν εμφανίζεται ως επιτρεπόμενη ενότητα.",
    meta:{minimumPlannedHours:26,classificationStatus:"official-syllabus-index-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-chemistry",grade:"C",subject:"Χημεία",subjectId:"chemistry",
    sourceTitle:"ΧΗΜΕΙΑ Γ ΓΥΜΝΑΣΙΟΥ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.chemistryC,
    official:[
      "1.1 Ιδιότητες οξέων","1.2 Οξέα κατά Arrhenius","1.3 Η κλίμακα pH ως μέτρο της οξύτητας","1.4 Το pH του καθαρού νερού",
      "1.5 Το pH των όξινων διαλυμάτων","1.6 Μέτρηση του pH ενός διαλύματος","2.1 Ιδιότητες βάσεων","2.2 Βάσεις κατά Arrhenius",
      "2.3 Κλίμακα pH ως μέτρο βασικότητας","3.1 Εξουδετέρωση","4.1 Σχηματισμός κρυστάλλων χλωριούχου νατρίου","4.3 Τα άλατα",
      "5.1 Ανθρώπινος οργανισμός","5.3 Αρκετή τροφή για να χορτάσει όλος ο κόσμος","5.4 Προστατεύοντας τον πλανήτη από την όξινη βροχή",
      "2η Ενότητα 1.1 Από το χθες…","2η Ενότητα 1.2 Ο σύγχρονος περιοδικός πίνακας","2η Ενότητα 1.3 Τα μέταλλα και τα αμέταλλα στον περιοδικό πίνακα",
      "2η Ενότητα 2.1 Αλκάλια - Γενικά","2η Ενότητα 2.2 Ιδιότητες των αλκαλίων","2η Ενότητα 3.1 Μέταλλα και αμέταλλα",
      "2η Ενότητα 3.3 Απλή αντικατάσταση","2η Ενότητα 3.4 Τα κράματα","2η Ενότητα 4.1 Ο άνθρακας - Γενικά",
      "2η Ενότητα 4.2 Φυσικοί άνθρακες","2η Ενότητα 4.3 Τεχνητοί άνθρακες",
      "3η Ενότητα 1.1 Υδρογονάνθρακες - Γενικά","3η Ενότητα 1.3 Καύση των υδρογονανθράκων","3η Ενότητα 1.4 Οι υδρογονάνθρακες ως καύσιμα",
      "3η Ενότητα 1.5 Η ρύπανση της ατμόσφαιρας","3η Ενότητα 2.7 Πολυμερισμός","3η Ενότητα 2.8 Τι είναι τα πλαστικά;",
      "3η Ενότητα 3.2 Αιθανόλη / αιθυλική αλκοόλη / οινόπνευμα","3η Ενότητα 3.5 Αλκοολούχα ποτά","3η Ενότητα 3.6 Φυσιολογική δράση της αιθανόλης"
    ],
    note:"Οι επιλογές είναι ο επίσημος κατάλογος «Ύλη» της Χημείας Γ΄ Ε.Α.Ε. 2026-2027. Επιμέρους στόχοι που οι αναλυτικές οδηγίες αφαιρούν δεν προστίθενται τεχνητά ως ξεχωριστές ενότητες.",
    meta:{minimumPlannedHours:25,classificationStatus:"official-syllabus-index-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-social-civic",grade:"B",subject:"Κοινωνική και Πολιτική Αγωγή",subjectId:"social-civic",
    sourceTitle:"ΚΠΑ ΓΥΜΝΑΣΙΟΥ ΟΔΗΓΙΕΣ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.civics,
    required:[
      "1.2 Τι είναι κοινωνία","1.3 Ατομική και κοινωνική συμπεριφορά",
      "2.1.1 Έννοια και χαρακτηριστικά των κοινωνικών ομάδων","2.1.2 Διακρίσεις των κοινωνικών ομάδων",
      "3.1 Η κοινωνική θέση","3.2 Ο κοινωνικός ρόλος","3.3 Οι κοινωνικοί κανόνες","3.4 Κοινωνική διαστρωμάτωση - κοινωνική κινητικότητα","3.5 Κοινωνική μεταβολή",
      "4.1 Τι είναι κοινωνικός θεσμός","4.2 Αναγκαιότητα των θεσμών",
      "5.2 Φορείς κοινωνικοποίησης (περιληπτικά)","5.3 Τρόποι κοινωνικοποίησης (περιληπτικά)","5.4 Ο κοινωνικός έλεγχος","5.5 Τα είδη του κοινωνικού ελέγχου",
      "6.1 Κοινωνικά προβλήματα: προσδιορισμός και συνέπειες","6.2 Αίτια και αντιμετώπιση","6.3 Φτώχεια, ανεργία, καταναλωτισμός, αγωγή καταναλωτή",
      "6.4 Τροχαία ατυχήματα - κυκλοφοριακή αγωγή","6.5 Αθλητισμός και βία"
    ],
    optional:["1.1 Άτομο: «φύσει κοινωνικό ον»","4.5 Ο θεσμός της οικογένειας","5.1 Έννοια και περιεχόμενο της κοινωνικοποίησης"],
    note:"Η Β΄ τάξη χρησιμοποιεί το Μέρος Α΄ «Το άτομο και η κοινωνία». Οι τρεις ρητά προαιρετικές υποενότητες επισημαίνονται ως προαιρετικές αντί να συγχέονται με τις βασικές επιλογές.",
    meta:{weeklyHours:1}
  });





  upsert({
    id:"teacher-annual-special-gym-a-language",grade:"A",subject:"Γλωσσική Διδασκαλία",subjectId:"language",
    sourceTitle:"Ν.Ε. ΛΟΓΟΤΕΧΝΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.greekLanguageLiterature,
    required:[
      "1η Ενότητα — Οι πρώτες μέρες σε ένα νέο σχολείο",
      "2η Ενότητα — Επικοινωνία στο σχολείο",
      "6η Ενότητα — Οι δημιουργικές δραστηριότητες στη ζωή μου"
    ],
    optional:[
      "8η Ενότητα — Αθλητισμός και Ολυμπιακοί αγώνες: Παρακολουθώ και συμμετέχω",
      "9η Ενότητα — Ανακαλύπτω τη μαγεία της γνώσης"
    ],
    note:"Οι οδηγίες της Α΄ Γυμνασίου Ε.Α.Ε. δεν προτείνουν γραμμική κάλυψη όλου του βιβλίου. Διακρίνουν τρεις βασικές και δύο συμπληρωματικές ενότητες, ενώ οι γλωσσικές δεξιότητες και τα γραμματικοσυντακτικά φαινόμενα ενσωματώνονται σε όλη τη διάρκεια του έτους.",
    meta:{
      weeklyHours:4,
      anchorPolicy:"official-basic-and-supplementary-units-flexible-order",
      coverageCompleteness:"section-level-verified",
      thematicAxes:["Σχολείο","Φύση, Υγεία και Διατροφή","Θέατρο/Κινηματογράφος","Δραστηριότητες","Ο κόσμος μέσα από την οθόνη/εικόνα","Αθλητισμός","Γνώση","Τόπος και Πολιτισμός"],
      textGenres:["Περιγραφή","Αφήγηση","Επιχειρηματολογία","Πολυτροπικά κείμενα"]
    }
  });

  upsert({
    id:"teacher-annual-special-gym-b-language",grade:"B",subject:"Γλωσσική Διδασκαλία",subjectId:"language",
    sourceTitle:"Ν.Ε. ΛΟΓΟΤΕΧΝΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.greekLanguageLiterature,
    official:[
      "Θεματικός άξονας — Ταξίδια",
      "Θεματικός άξονας — Οικογένεια",
      "Θεματικός άξονας — Φιλία",
      "Θεματικός άξονας — Σχολείο",
      "Θεματικός άξονας — Εργασία",
      "Θεματικός άξονας — Ενημέρωση",
      "Θεματικός άξονας — Σύγχρονα κοινωνικά προβλήματα"
    ],
    note:"Στη Β΄ Γυμνασίου το βιβλίο έχει εννέα διδακτικές ενότητες, αλλά οι επίσημες οδηγίες αφήνουν ρητά στον/στην εκπαιδευτικό την επιλογή θεματικών και τη συνεξέταση ενοτήτων με βάση τα ενδιαφέροντα των μαθητών/τριών. Για να μη δημιουργηθεί ψευδής υποχρεωτική σειρά κεφαλαίων, το site εκθέτει τους επαληθευμένους θεματικούς άξονες.",
    meta:{
      weeklyHours:4,
      anchorPolicy:"target-oriented-flexible-thematic-framework",
      coverageCompleteness:"thematic-framework-verified",
      textGenres:["Περιγραφή","Αφήγηση","Επιχειρηματολογία"],
      languageSkills:["Κατανόηση προφορικού λόγου","Κατανόηση γραπτού λόγου","Παραγωγή προφορικού λόγου","Παραγωγή γραπτού λόγου","Τρόποι ανάπτυξης παραγράφου","Περίληψη κειμένου - Πλαγιότιτλοι","Συνοχή ευρύτερου κειμένου","Αξιολόγηση και διατύπωση επιχειρημάτων"]
    }
  });

  upsert({
    id:"teacher-annual-special-gym-c-language",grade:"C",subject:"Γλωσσική Διδασκαλία",subjectId:"language",
    sourceTitle:"Ν.Ε. ΛΟΓΟΤΕΧΝΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.greekLanguageLiterature,
    official:[
      "3η Ενότητα — Είμαστε όλοι ίδιοι. Είμαστε όλοι διαφορετικοί",
      "2η Ενότητα — Γλώσσα - Γλώσσες και πολιτισμοί του κόσμου",
      "1η Ενότητα — Η Ελλάδα στον κόσμο",
      "4η Ενότητα — Ενωμένη Ευρώπη και Ευρωπαίοι πολίτες",
      "5η Ενότητα — Ειρήνη - Πόλεμος",
      "6η Ενότητα — Ενεργοί πολίτες για την υπεράσπιση οικουμενικών αξιών"
    ],
    note:"Η σειρά αυτή αποτυπώνει την ενδεικτική αναδιάταξη των επίσημων οδηγιών για πρόγραμμα περίπου 50 διδακτικών ωρών. Δεν χαρακτηρίζεται ως υποχρεωτική γραμμική σειρά· το site κρατά τους πραγματικούς τίτλους χωρίς να επινοεί πρόσθετες ενότητες.",
    meta:{
      weeklyHours:4,
      approximateProgramHours:50,
      anchorPolicy:"official-indicative-reordered-units-not-mandatory-sequence",
      coverageCompleteness:"section-level-verified"
    }
  });


  upsert({
    id:"teacher-annual-special-gym-a-literature",grade:"A",subject:"Νεοελληνική Λογοτεχνία",subjectId:"literature",
    sourceTitle:"Ν.Ε. ΛΟΓΟΤΕΧΝΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.greekLanguageLiterature,
    official:[
    "Ο άνθρωπος και η φύση • Πόλη – Ύπαιθρος",
    "Λαογραφικά",
    "Οικογενειακές σχέσεις",
    "Θρησκευτική ζωή",
    "Εθνική ζωή",
    "Παλαιότερες μορφές ζωής",
    "Ταξιδιωτικά κείμενα",
    "Η αποδημία • Ο καημός της ξενιτιάς • Ο ελληνισμός έξω από τα σύνορα • Τα μικρασιατικά • Οι πρόσφυγες",
    "Αθλητισμός",
    "Η αγάπη για τους συνανθρώπους μας • Οι φιλικοί δεσμοί • Η αγάπη",
    "Η βιοπάλη • Το αγωνιστικό πνεύμα του ανθρώπου",
    "Προβλήματα της σύγχρονης ζωής",
    "Οι φίλοι μας τα ζώα"
],
    note:"Οι επίσημες οδηγίες Ε.Α.Ε. 2026-2027 ορίζουν επιλογή κειμένων από τον/την διδάσκοντα/ουσα και, για την Α΄ και Β΄ τάξη, επιδίωξη ποικιλίας θεματικών ενοτήτων. Οι θεματικές εδώ λειτουργούν ως πλαίσιο επιλογής από το επίσημο σχολικό βιβλίο και όχι ως υποχρεωτική γραμμική ή εξεταστέα λίστα.",
    meta:{weeklyHours:2,continuousDoublePeriodRecommended:true,selectionFramework:true,selectionStatus:"teacher-selected-not-fixed-syllabus",anchorPolicy:"annual-selection-framework-plus-official-textbook-themes",textbookSourceUrl:"https://www.ebooks.edu.gr/ebooks/handle/8547/130",coverageCompleteness:"annual-selection-framework-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-literature",grade:"B",subject:"Νεοελληνική Λογοτεχνία",subjectId:"literature",
    sourceTitle:"Ν.Ε. ΛΟΓΟΤΕΧΝΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.greekLanguageLiterature,
    official:[
    "Ο άνθρωπος και η φύση • Πόλη – Ύπαιθρος",
    "Λαογραφικά",
    "Οικογενειακές σχέσεις",
    "Θρησκευτική ζωή",
    "Εθνική ζωή",
    "Παλαιότερες μορφές ζωής",
    "Ταξιδιωτικά κείμενα",
    "Η αποδημία • Ο καημός της ξενιτιάς • Ο ελληνισμός έξω από τα σύνορα • Τα μικρασιατικά • Οι πρόσφυγες",
    "Αθλητισμός",
    "Η αγάπη για τους συνανθρώπους μας • Οι φιλικοί δεσμοί • Η αγάπη",
    "Η βιοπάλη • Το αγωνιστικό πνεύμα του ανθρώπου",
    "Προβλήματα της σύγχρονης ζωής",
    "Οι φίλοι μας τα ζώα"
],
    note:"Οι επίσημες οδηγίες Ε.Α.Ε. 2026-2027 αφήνουν την επιλογή κειμένων στον/στην διδάσκοντα/ουσα και ζητούν, κατά το δυνατόν, ποικιλία θεματικών ενοτήτων στην Α΄ και Β΄ τάξη. Οι επιλογές του μενού είναι θεματικό πλαίσιο του επίσημου σχολικού βιβλίου, όχι υποχρεωτική σειρά διδασκαλίας.",
    meta:{weeklyHours:2,continuousDoublePeriodRecommended:true,selectionFramework:true,selectionStatus:"teacher-selected-not-fixed-syllabus",anchorPolicy:"annual-selection-framework-plus-official-textbook-themes",textbookSourceUrl:"https://www.ebooks.edu.gr/ebooks/handle/8547/131",coverageCompleteness:"annual-selection-framework-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-literature",grade:"C",subject:"Νεοελληνική Λογοτεχνία",subjectId:"literature",
    sourceTitle:"Ν.Ε. ΛΟΓΟΤΕΧΝΙΑ ΟΔΗΓΙΕΣ ΓΥΜΝ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.greekLanguageLiterature,
    official:[
    "Δημοτικά τραγούδια",
    "Η λόγια παραγωγή (1000–1600)",
    "Κρητική λογοτεχνία",
    "Νεοελληνικός Διαφωτισμός",
    "Ο 19ος αιώνας έως το 1880 — Απομνημονεύματα",
    "Ο 19ος αιώνας έως το 1880 — Επτανησιακή Σχολή",
    "Ο 19ος αιώνας έως το 1880 — Φαναριώτες και Ρομαντική Σχολή των Αθηνών",
    "Η Νέα Αθηναϊκή Σχολή (1880–1922)",
    "Η νεότερη λογοτεχνία — 1922–1945",
    "Η νεότερη λογοτεχνία — Πρώτη μεταπολεμική περίοδος",
    "Η νεότερη λογοτεχνία — Δεύτερη μεταπολεμική γενιά και γενιές του ’70 και ’80"
],
    note:"Στη Γ΄ Γυμνασίου Ε.Α.Ε. οι επίσημες οδηγίες χαρακτηρίζουν τη διάταξη ιστορικογραμματολογική: μπορεί να ακολουθείται η χρονολογική σειρά του εγχειριδίου, με παράλληλες θεματικές συνδέσεις. Οι περίοδοι του μενού είναι πλαίσιο πλοήγησης και επιλογής κειμένων, όχι δήλωση ότι κάθε κείμενο του βιβλίου είναι υποχρεωτικό.",
    meta:{weeklyHours:2,continuousDoublePeriodRecommended:true,selectionFramework:true,selectionStatus:"teacher-selected-historical-literary-framework",anchorPolicy:"historical-literary-chronological-with-thematic-links",textbookSourceUrl:"https://www.ebooks.edu.gr/ebooks/handle/8547/2217",coverageCompleteness:"annual-historical-framework-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-social-civic",grade:"C",subject:"Κοινωνική και Πολιτική Αγωγή",subjectId:"social-civic",
    sourceTitle:"ΚΠΑ ΓΥΜΝΑΣΙΟΥ ΟΔΗΓΙΕΣ Ε.Α.Ε. 26-27",sourceUrl:SOURCES.civics,
    required:[
      "8.1 Μορφές Πολιτευμάτων","8.2 Το Πολίτευμα της Ελλάδας","8.3 Τι είναι Σύνταγμα","8.4 Βασικές αρχές του Συντάγματος","8.4.1 Λαϊκή Κυριαρχία","8.4.2 Κράτος Δικαίου","8.4.3 Κοινωνικό Κράτος","8.4 Αρχή της Διάκρισης των Λειτουργιών",
      "9.1 Εκλογικό σώμα","9.2 Ψηφοφορία - Βασικές αρχές","9.3 Εκλογικά συστήματα","9.4 Τα πολιτικά κόμματα",
      "10.1 Νομοθετική Λειτουργία","10.1.1 Η σύνθεση της Βουλής","10.1.2 Αρμοδιότητες της Βουλής","10.2 Εκτελεστική Λειτουργία","10.2.1 Πρόεδρος της Δημοκρατίας","10.2.2 Κυβέρνηση","10.2.3 Η Δικαστική Λειτουργία",
      "12.1 Αντιστοιχία Δικαιωμάτων και Υποχρεώσεων","12.2 Προστασία των Δικαιωμάτων","12.3 Ατομικά Δικαιώματα","12.4 Τα πολιτικά δικαιώματα","12.5 Τα κοινωνικά δικαιώματα","12.5.1 Δικαίωμα της παιδείας","12.5.2 Δικαίωμα της εργασίας","12.5.3 Προστασία της υγείας - Αγωγή Υγείας","12.5.4 Προστασία του περιβάλλοντος - περιβαλλοντική αγωγή",
      "13.2 Βασικοί θεσμοί","13.3 Βασικές πολιτικές","13.4 Πολίτης και Ευρωπαϊκή Ένωση",
      "14.1 Η διεθνής κοινότητα και το διεθνές δίκαιο","14.2.1 Οι Διεθνείς Οργανισμοί","14.2.2 Οργανισμός Ηνωμένων Εθνών (περιληπτικά)","14.2.3 Οργανισμός Βορειοατλαντικού Συμφώνου (ΝΑΤΟ) (περιληπτικά)","14.2.4 Οργανισμός για την Ασφάλεια και τη Συνεργασία στην Ευρώπη (ΟΑΣΕ) (περιληπτικά)","14.3 Ανθρώπινα Δικαιώματα"
    ],
    optional:[
      "9.5 Μέσα Μαζικής Επικοινωνίας (ΜΜΕ)",
      "13.1 Ίδρυση και εξέλιξη της Ευρωπαϊκής Ένωσης",
      "14.2.5 Άλλοι Διεθνείς Οργανισμοί - Μη Κυβερνητικές Οργανώσεις (Μ.Κ.Ο.)"
    ],
    excluded:[
      "Κεφάλαιο 7 — δεν περιλαμβάνεται στον επίσημο πίνακα διδακτέας ύλης Γ΄ Ε.Α.Ε. 2026-27",
      "Κεφάλαιο 11 — δεν περιλαμβάνεται στον επίσημο πίνακα διδακτέας ύλης Γ΄ Ε.Α.Ε. 2026-27"
    ],
    note:"Η Γ΄ τάξη χαρτογραφείται μόνο από τον επίσημο πίνακα διδακτέας ύλης 2026-2027. Οι 9.5, 13.1 και 14.2.5 διατηρούνται ως ρητά προαιρετικές. Τα Κεφάλαια 7 και 11 υπάρχουν στο σχολικό βιβλίο αλλά δεν εμφανίζονται στον φετινό πίνακα διδακτέας ύλης, οπότε δεν εκτίθενται ως επιλογές.",
    meta:{weeklyHours:2,coverageCompleteness:"section-level-verified",classificationStatus:"official-required-optional-preserved"}
  });

  upsert({
    id:"teacher-annual-special-gym-a-english",grade:"A",subject:"Αγγλικά",subjectId:"english",
    sourceTitle:"ΞΕΝΕΣ ΓΛΩΣΣΕΣ ΟΔΗΓΙΕΣ ΓΥΜΝΑΣΙΩΝ EAE (ΑΓΓΛΙΚΑ)",sourceUrl:SOURCES.english,
    official:["Κατανόηση γραπτού λόγου","Παραγωγή γραπτού λόγου & γραπτή διάδραση","Γραπτή διαμεσολάβηση","Κατανόηση προφορικού λόγου","Παραγωγή προφορικού λόγου & προφορική διάδραση","Προφορική διαμεσολάβηση"],
    note:"Οι επίσημες οδηγίες Αγγλικής στα Γυμνάσια Ε.Α.Ε. οργανώνουν τη μάθηση με επικοινωνιακές δεξιότητες και επίπεδα γλωσσομάθειας, όχι με δεσμευτική σειρά κεφαλαίων σχολικού βιβλίου. Για αυτό το μενού εμφανίζει μόνο επαληθευμένους τομείς δεξιοτήτων και δεν κατασκευάζει τεχνητές ενότητες.",
    meta:{expectedCefrLevel:"A2-/B1-",anchorPolicy:"competency-framework-not-chapter-syllabus",coverageCompleteness:"competency-framework-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-b-english",grade:"B",subject:"Αγγλικά",subjectId:"english",
    sourceTitle:"ΞΕΝΕΣ ΓΛΩΣΣΕΣ ΟΔΗΓΙΕΣ ΓΥΜΝΑΣΙΩΝ EAE (ΑΓΓΛΙΚΑ)",sourceUrl:SOURCES.english,
    official:["Κατανόηση γραπτού λόγου","Παραγωγή γραπτού λόγου & γραπτή διάδραση","Γραπτή διαμεσολάβηση","Κατανόηση προφορικού λόγου","Παραγωγή προφορικού λόγου & προφορική διάδραση","Προφορική διαμεσολάβηση"],
    note:"Οι επίσημες οδηγίες Αγγλικής στα Γυμνάσια Ε.Α.Ε. οργανώνουν τη μάθηση με επικοινωνιακές δεξιότητες και επίπεδα γλωσσομάθειας, όχι με δεσμευτική σειρά κεφαλαίων σχολικού βιβλίου. Για αυτό το μενού εμφανίζει μόνο επαληθευμένους τομείς δεξιοτήτων και δεν κατασκευάζει τεχνητές ενότητες.",
    meta:{expectedCefrLevel:"B1-/B1+",anchorPolicy:"competency-framework-not-chapter-syllabus",coverageCompleteness:"competency-framework-verified"}
  });

  upsert({
    id:"teacher-annual-special-gym-c-english",grade:"C",subject:"Αγγλικά",subjectId:"english",
    sourceTitle:"ΞΕΝΕΣ ΓΛΩΣΣΕΣ ΟΔΗΓΙΕΣ ΓΥΜΝΑΣΙΩΝ EAE (ΑΓΓΛΙΚΑ)",sourceUrl:SOURCES.english,
    official:["Κατανόηση γραπτού λόγου","Παραγωγή γραπτού λόγου & γραπτή διάδραση","Γραπτή διαμεσολάβηση","Κατανόηση προφορικού λόγου","Παραγωγή προφορικού λόγου & προφορική διάδραση","Προφορική διαμεσολάβηση"],
    note:"Οι επίσημες οδηγίες Αγγλικής στα Γυμνάσια Ε.Α.Ε. οργανώνουν τη μάθηση με επικοινωνιακές δεξιότητες και επίπεδα γλωσσομάθειας, όχι με δεσμευτική σειρά κεφαλαίων σχολικού βιβλίου. Για αυτό το μενού εμφανίζει μόνο επαληθευμένους τομείς δεξιοτήτων και δεν κατασκευάζει τεχνητές ενότητες.",
    meta:{expectedCefrLevel:"B1+/B2-",anchorPolicy:"competency-framework-not-chapter-syllabus",coverageCompleteness:"competency-framework-verified"}
  });

  upsert({
    id:"teacher-extra-special-gym-c-economics",grade:"C",subject:"Οικονομικά",subjectId:"economics",
    sourceTitle:"ΟΙΚΟΝΟΜΙΚΑ Γ ΓΥΜΝΑΣΙΟΥ ΕΑΕ 2026-2027",sourceUrl:SOURCES.economics,
    required:[
      "1.1 Οικονομική Δραστηριότητα","1.2.1 Ανάγκες","1.2.2 Αγαθά","1.3 Στενότητα","1.4 Επιλογή και Κόστος Ευκαιρίας","1.5 Οικονομική Αρχή","1.6 Το Οικονομικό Κύκλωμα",
      "2.1 Έννοια της Παραγωγής","2.2 Διακρίσεις της Παραγωγής","2.3 Μετατόπιση Εργατικού Δυναμικού",
      "3.1 Συντελεστές Παραγωγής","3.2 Εργασία","3.2.1 Παραγωγικότητα της Εργασίας","3.3 Φυσικοί Πόροι","3.3.1 Επίδραση του Ανθρώπου στο Φυσικό Περιβάλλον",
      "3.4 Κεφάλαιο","3.5 Επιχειρηματικότητα","3.6 Αμοιβές Συντελεστών Παραγωγής","3.6.1 Μισθός","3.6.2 Γαιοπρόσοδος","3.6.3 Τόκος","3.6.4 Κέρδος",
      "4.1 Έννοια της Αγοράς","4.3 Διαμόρφωση της Τιμής","4.3.1 Ζήτηση","4.3.2 Προσφορά","4.3.3 Τιμή Ισορροπίας"
    ],
    modified:["4.2 Μορφές Αγοράς: μόνο Τέλειος Ανταγωνισμός"],
    optional:["1.6 Διαγραμματική αναπαράσταση του Οικονομικού Κυκλώματος και των ροών του","3.6 Υπολογισμός απλού τόκου ως επέκταση"],
    excluded:[
      "1.3 Το εξαιρούμενο κείμενο «Τόσο οι φτωχοί … και τα δύο ταυτόχρονα»",
      "3.2 Το εξαιρούμενο κείμενο για αύξηση κερδών / προσωπικό","3.5 Ερώτηση 19, σ. 50",
      "4.1 Το εξαιρούμενο κείμενο για Uber Taxi","4.2 Μονοπώλιο","4.2 Μονοπωλιακός Ανταγωνισμός","4.2 Ολιγοπώλιο",
      "Κεφάλαιο 5 — Βασικά Οικονομικά Μεγέθη (να μη διδαχθεί)"
    ],
    note:"Αντικαθιστά την παλιότερη υποστηρικτική αναφορά που εμφάνιζε ΑΕΠ, πληθωρισμό, αποταμίευση και χρηματοπιστωτικά εργαλεία. Η νέα εγγραφή ακολουθεί την επίσημη ετήσια ύλη Ε.Α.Ε. 2026-2027 και αποκλείει ολόκληρο το Κεφάλαιο 5.",
    meta:{sourceBookUrl:"https://ebooks.edu.gr/ebooks/handle/8547/5582"}
  });

  window.AITOOLSKIDS_SPECIAL_GYM_ANNUAL_2026_2027=Object.freeze({
    version:"1.12.0",
    verified:VERIFIED,
    officialHubUrl:HUB,
    sources:Object.freeze({...SOURCES}),
    publishedPending:PUBLISHED_PENDING,
    mappedEntries:Object.freeze([
      "teacher-annual-special-gym-a-ancient-language","teacher-annual-special-gym-b-ancient-language","teacher-annual-special-gym-c-ancient-language",
      "teacher-annual-special-gym-a-ancient-translation","teacher-annual-special-gym-b-ancient-translation","teacher-annual-special-gym-c-ancient-translation",
      "teacher-annual-special-gym-a-music","teacher-annual-special-gym-b-music","teacher-annual-special-gym-c-music",
      "teacher-annual-special-gym-a-art","teacher-annual-special-gym-b-art","teacher-annual-special-gym-c-art",
      "teacher-annual-special-gym-a-informatics","teacher-annual-special-gym-b-informatics","teacher-annual-special-gym-c-informatics",
      "teacher-annual-special-gym-a-history","teacher-annual-special-gym-b-history","teacher-annual-special-gym-c-history",
      "teacher-annual-special-gym-a-technology","teacher-annual-special-gym-b-technology","teacher-annual-special-gym-c-technology",
      "teacher-annual-special-gym-a-geography","teacher-annual-special-gym-b-geography",
      "teacher-annual-special-gym-a-biology","teacher-annual-special-gym-b-biology","teacher-annual-special-gym-c-biology",
      "teacher-annual-special-gym-a-math","teacher-annual-special-gym-b-math","teacher-annual-special-gym-c-math",
      "teacher-annual-special-gym-a-physics","teacher-annual-special-gym-b-physics","teacher-annual-special-gym-c-physics",
      "teacher-annual-special-gym-a-pe","teacher-annual-special-gym-b-pe","teacher-annual-special-gym-c-pe",
      "teacher-annual-special-gym-b-chemistry","teacher-annual-special-gym-c-chemistry",
      "teacher-annual-special-gym-a-language","teacher-annual-special-gym-b-language","teacher-annual-special-gym-c-language",
      "teacher-annual-special-gym-a-literature","teacher-annual-special-gym-b-literature","teacher-annual-special-gym-c-literature",
      "teacher-annual-special-gym-b-social-civic","teacher-annual-special-gym-c-social-civic",
      "teacher-annual-special-gym-a-english","teacher-annual-special-gym-b-english","teacher-annual-special-gym-c-english",
      "teacher-extra-special-gym-c-economics"
    ])
  });

  if(typeof window.refreshSubjects==="function" && ["specialGym","deafGym"].includes(document.getElementById("context")?.value)){
    window.refreshSubjects();
  }
})();
