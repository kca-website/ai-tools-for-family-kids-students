...
try {
  ...
  assert(methodEl.includes('35 canonical εργαλεία'), 'Missing 32-tool accessibility research scope');
  assert(methodEl.includes('12 τεκμηριωμένα θετικά'), 'Missing positive accessibility count');
  assert(methodEl.includes('13 μερικά/μεικτά'), 'Missing partial accessibility count');
  assert(methodEl.includes('1 τεκμηριωμένη ανησυχία'), 'Missing caution accessibility count');
  assert(methodEl.includes('9 χωρίς εντοπίσιμη επαρκή πηγή'), 'Missing no-source accessibility count');
  ...
  assert(methodEn.includes('35 canonical tools'), 'English 32-tool research scope missing');
  ...
  await page.waitForFunction(() => document.querySelectorAll('#a11yTableBody tr').length >= 35, null, { timeout: 30000 });
  ...
  assert((await page.locator('[data-eng-finding="35-tools"]').count()) === 1, 'Missing structured ENG finding marker');
  ...
  assert(rowCount === 35, `Expected 35 documented accessibility rows, got ${rowCount}`);
