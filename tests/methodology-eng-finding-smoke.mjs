...
try {
  ...
  assert(methodEl.includes('43 canonical εργαλεία'), 'Missing 32-tool accessibility research scope');
  assert(methodEl.includes('14 τεκμηριωμένα θετικά'), 'Missing positive accessibility count');
  assert(methodEl.includes('18 μερικά/μεικτά'), 'Missing partial accessibility count');
  assert(methodEl.includes('1 τεκμηριωμένη ανησυχία'), 'Missing caution accessibility count');
  assert(methodEl.includes('10 χωρίς εντοπίσιμη επαρκή πηγή'), 'Missing no-source accessibility count');
  ...
  assert(methodEn.includes('43 canonical tools'), 'English 32-tool research scope missing');
  ...
  await page.waitForFunction(() => document.querySelectorAll('#a11yTableBody tr').length >= 43, null, { timeout: 30000 });
  ...
  assert((await page.locator('[data-eng-finding="43-tools"]').count()) === 1, 'Missing structured ENG finding marker');
  ...
  assert(rowCount === 43, `Expected 43 documented accessibility rows, got ${rowCount}`);
