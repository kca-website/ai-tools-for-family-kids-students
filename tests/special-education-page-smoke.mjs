...
async function check(viewport, label) {
  ...
  assert(await page.locator('[data-open-unit]').count() === 3, `${label}: expected 3 available verified learning units`);
  ...
  const healthButton = page.locator('[data-open-unit="eneegyl-b-health-nutrition"]');
  ...
  const noOverflowHealth = await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth + 1);
  assert(noOverflowHealth, `${label}: horizontal overflow after opening health unit`);

  await page.locator('#spUnitMount summary', { hasText: 'Διαγνωστικό' }).click();
  await page.locator('#spQuizStart').click();
  assert(await page.locator('#spQuiz .sp-option').count() === 2, `${label}: health diagnostic did not render options`);
  ...
  assert(noOverflowAfter, `${label}: horizontal overflow after opening mechanics unit`);
  await page.close();
}
